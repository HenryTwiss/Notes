  ...
  \section{Dirichlet Series}
    ...
    \subsection{Perron Formulas}
      ...
      \begin{proof}
        ...
        \] 
        and we see that \(\d(y) = \frac{1}{2}\). This proves everything when \(y = 1\). Now suppose \(y < 1\) and let \(d > c\). Let \(\eta = \eta_{1}+\cdots+\eta_{4}\) be the rectangular contour in \cref{fig:discontinuous_integral_contour_1} where the horizontal lines are along \(t = \pm T\) and the vertical lines are along \(\s = c\) and \(\s = d\).
        ...
        \[
          I(y,T) = O(y^{c}).
        \]
        Combining these two estimates for \(I(y,T)\) proves everything when \(y < 1\). Now suppose \(y > 1\) and let \(d < 0\). We argue as in the case \(y < 1\) except the rectangular contour is now as in \cref{fig:discontinuous_integral_contour_3}.
        ...
        \]
        and so \(\d(y) = 1\). The corresponding semicircular contour is given in \cref{fig:discontinuous_integral_contour_4}.
      ...
      The following result is (smoothed) \textbf{Perron's formula}\index{Perron's formula}:
