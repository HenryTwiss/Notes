...
\begin{document}
  ...
  \section{Integral Lattices}
    Let \(F\) be a characteristic zero field and \(V\) be an \(\F\)-vector space of dimension \(n\) with nondegenerate symmetric inner product \(\<\cdot,\cdot\>\). We say that a subset \(\L\) of \(V\) is a \textit{integral lattice} if \(\L\) is a free \(\Z\)-module. In particular, any integral lattice \(\L\) is of the form
    \[
    ...
    \]
    with \(v_{i,j} \in F\), and define the associated \textit{generator matrix} \(P\) by
    \[
      P = \begin{pmatrix} v_{1,1} & \cdots & v_{n,1} \\ \vdots & & \vdots \\ v_{1,n} & \cdots & v_{n,n} \end{pmatrix}.
    \]
    Then \(P\) is the base change matrix from \(e_{1},\ldots,e_{n}\) to \(v_{1},\ldots,v_{n}\). The \textit{covolume} \(V_{\L}\) of a complete integral lattice \(\L\) is defined to be
    \[
      V_{\L} = |\det(P)|.
    \]
    ...
    If \(\L\) is a complete integral lattice in \(V\) then the \textit{dual} \(\L^{\vee}\) of \(\L\) is defined by
    \[
    ...
    The most important result we will require about integral lattices is \textit{Minkowski's lattice point theorem} which states that, under some mild conditions, a set of sufficiently large volume in \(V\) contains a nonzero point of a complete integral lattice.
