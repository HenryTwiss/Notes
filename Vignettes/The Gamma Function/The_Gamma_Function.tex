  ...
  \section{The Gamma Function}\label{sec:The_Gamma_Function}
    ...
    \subsection{The Gamma Function}
      ...
      \begin{proof}
        ...
        \]
        The change of variables \((x,y) \mapsto (xy,y-xy)\) gives 
        \[
          \int_{0}^{\infty}\int_{0}^{1}e^{-y}y(xy)^{s-1}(y-xy)^{u-1}\,dx\,dy,
        \]
        as the Jacobian determinant is \(y\). Then
        \[
      ...
      It is not yet possible to meromorphically continue the beta function to \(\C^{2}\) because we cannot divide by the gamma function as we do not yet know where it possess zeros. It turns out that the gamma function has no zeros and we will prove this shortly. The essential step is to prove Euler's reflection formula which relates the gamma function to a periodic function of which we know its zeros explicitly. We will make use of \cref{prop:integral_reprepsentation_for_beta_function} and a formula for the beta function which will allow us to compute it by means of the residue theorem. To deduce this formula, apply the change of variables \(x \mapsto \frac{x}{x+1}\) to the beta function to obtain
      \begin{equation}\label{equ:residue_form_beta_function}
        B(s,u) = \int_{0}^{\infty}\frac{x^{s-1}}{(x+1)^{s+u}}\,dx.
      \end{equation}
      This shows that beta function is the Mellin transform of \(\frac{1}{(s+1)^{s+u}}\). We will make use of this fact to prove \textbf{Euler's reflection formula}\index{Euler's reflection formula}.
      ...
      \begin{proof}
        ...
        \noindent Then we must show that this integral is \(\frac{\pi}{\sin(\pi s)}\). Let \(\eta = \sum_{i}\eta_{i}\) be the keyhole contour \cref{fig:reflection_formula_contour} where the large arc has radius \(R\) and the small arc has radius \(\e\). Consider
        \[
      ...
      Upon inspection of the Hadamard factorization, we find that the reciprocal of the gamma function is of order \(1\).
    \subsection{Asymptotics}
      We now turn to analyzing the growth rate of the gamma function itself. In fact, we can obtain a well-known asymptotic formula for the gamma function known as \textbf{Stirling's formula}\index{Stirling's formula}.
      ...
      \begin{proof}
        ...
        \]
        At \(z = 0\), the first derivative is zero and the second derivative is negative. So taking the Taylor series expansion of \(\log(1+z)-z\) about \(z = 0\) and truncating after the third term gives
        \[
        ...
        \]
          Considering the Taylor series expansion of \(e^{z}\) about \(z = 0\) and truncating after the second term, we find that
        \[
