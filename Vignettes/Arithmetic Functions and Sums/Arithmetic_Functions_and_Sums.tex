    ...
    \subsection{Gauss Sums}
      ...
      \begin{proof}
        ...
        \begin{enumerate}[label*=(\roman*)]
          ...
          \item This follows by direct computation.
          \item We may assume \(m > 1\) otherwise the statement is vacuously true. Now let \(c\) be an integer with \((c,m) = 1\) and satisfying \(\chi(c) \neq 1\). Such a \(c\) exists because otherwise \(\chi\) is the principal Dirichlet character modulo \(m\) and thus imprimitive. A short computation shows
          \[
            \chi(c)G(b,\chi) = G(b,\chi),
          \]
          whence \(G(b,\chi) = 0\).
