...
\begin{document}
  ...
  \section{Exponential Sums}
    ...
    \[
      e(z) = e^{2\pi iz}.
    \]
    \subsection{\todo{Exponenetial Sums}}
      \todo{Discuss general exponential sums as well as complete and incomplete sums}
    \subsection{Gauss Sums}
      ...
      \]
      The Gauss sum is a finite sum of \(m\)-th roots of unity which have been twisted by the Dirichlet character \(\chi\). When \(b \mid m\) the summands are all \(1\) and the Dirichlet orthogonality relations imply
      \[
      ...
      Via properties (i) and (ii), the evaluation of the Gauss sum is reduced to that of \(\tau(\chi)\) at least when \(\chi\) is primitive. In this case, it is also not difficult to determine the modulus of \(\tau(\chi)\).
    ...
    \subsection{Character Sums and the P\'olya-Vinogradov Inequality}
      Character sums are one of the simplest type of incomplete exponential sums. Let \(\chi\) be a non-principal Dirichlet character modulo \(m\). The sum \(S_{\chi}(M,N)\) defined by
      \[
        S_{\chi}(M,N) = \sum_{M \le n \le M+N}\chi(n),
      \]
      for nonnegative \(M\) and \(N\), is called the \textbf{character sum}\index{character sum} of \(\chi\). If \(M = 0\), we write \(S_{\chi}(N) = S_{\chi}(0,N)\) for simplicity. An important question is how effectively can \(S_{\chi}(M,N)\) be estimated for various Dirichlet characters \(\chi\) and regimes of \(M\) and \(N\) relative to the modulus of \(\chi\). Since this is an exponential sum, we have the trivial bound
      \[
        S_{\chi}(M,N) \le N-1.
      \]
      ...
      \[
        S_{\chi}(M,N) \le m.
      \]
      This second estimate is only an improvement upon the first in the regime \(m \ll N\). Surprisingly, we can obtain almost half-power savings in this estimate with very little work provided we introduce an implicit constant. We will require a small lemma.
