...
\begin{document}
  ...
  \section{Exponential Sums}
    ...
    \subsection{Gauss Sums}
      ...
      \]
      When \(b = 1\) we simply write \(G(\chi) = G(1,\chi)\). The simplest case of a Gauss sum occurs when \(\chi = \chi_{m,0}\) in which case we write \(R(b,m) = G(b,\chi_{m,0})\) and call this the \textbf{Ramanujan sum}\index{Ramanujan sum} modulo \(m\). It can be expressed as
      \[
