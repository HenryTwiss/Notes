...
\begin{document}
  ...
  \section{Arithmetic Functions}
    ...
    \subsection{Dirichlet Series}
      One method of studying arithmetic functions is to package them into generating series. A (formal) \textbf{Dirichlet series}\index{Dirichlet series} \(D(s)\) is an expression of the form
      \[
        D(s) = \sum_{n \ge 1}\frac{a(n)}{n^{s}},
      \]
      for some complex coefficients \(a(n)\). We view Dirichlet series as elements of \(\C[[n^{-s}]]\) which are formal power series in the variable \(n^{-s}\) with complex coefficients.
      
      We may associate a Dirichlet series to any arithmetic function \(f\). This series \(D_{f}(s)\) is defined by
      \[
        D_{f}(s) = \sum_{n \ge 1}\frac{f(n)}{n^{s}}.
      \]
      Let \(\mc{D}\) denote the subspace of \(\C[[n^{-s}]]\) consisting of all Dirichlet series whose coefficients are given by an arithmetic function. Clearly \(\mc{D}\) is an abelian group. It is also a commutative ring where multiplication of Dirichlet series corresponds to Dirichlet convolution of the corresponding arithmetic functions. Indeed, if \(f\) and \(g\) are two arithmetic functions then a short computation shows
      \[
        D_{f}(s)D_{g}(s) = \sum_{n \ge 1}\frac{\sum_{d \mid n}f(d)g\left(\frac{n}{d}\right)}{n^{s}}.
      \]
      Therefore the product \(D_{f}(s)D_{g}(s)\) is itself the Dirichlet series whose coefficient are given by the Dirichlet convolution of \(f\) and \(g\). In short,
      \[
        D_{f}(s)D_{g}(s) = D_{f \ast g}(s).
      \]
      The identity element is \(D_{\e}(s) = 1\) which is just the constant function. The Dirichlet series corresponding to \(\mathbf{1}\) is called the \textbf{Riemann zeta function}\index{Riemann zeta function} \(\z(s)\) and is given by
      \[
        \z(s) = \sum_{n \ge 1}\frac{1}{n^{s}}.
      \]
      It follows from \cref{prop:Mobius_indicator} that the the inverse of the Riemann zeta function is the Dirichlet series given by the M\"obius function. In other words,
      \[
        \frac{1}{\z(s)} = \sum_{n \ge 1}\frac{\mu(n)}{n^{s}}.
      \]
      Then M\"obius inversion can be expressed in terms of Dirichlet series as
      \[
        D_{g}(s) = D_{f}(s)\z(s) \quad \text{or equivalently} \quad D_{f}(s) = D_{g}(s)\frac{1}{\z(s)}.
      \]  
  \section{Asymptotics}
  ...
  \section{Dirichlet Characters}
    Throughout, for any integers \(a\) and \(m\) the symbol \(\legendre{a}{m}\) will stand for the Kronecker symbol. In particular, it is the Jacobi symbol when \(m\) is a positive odd integer, and the Legendre symbol when \(m = p\) is a prime. If \(a\) is an integer modulo \(m\) with \((a,m) = 1\) we write \(a^{-1}\) for the inverse of \(a\) modulo \(m\). Also, for any complex \(z\) we set
    \[
      e(z) = e^{2\pi iz}.
    \]
    \subsection{Dirichlet Characters}
      ...
      \[
        \sum_{a \tmod{m}}(\chi\conj{\psi})(a) = \vphi(m)\d_{\chi,\psi} \quad \text{and} \quad \sum_{\chi \tmod{m}}\chi(ab^{-1}) = \vphi(m)\d_{a,b}.
      \]
      In particular,
      \[
      ...
      \begin{proof}
        ...
        \]
        in which case the sum vanishes. This proves the first identity. For the second, if \(a = b\) then the second sum is clearly \(\vphi(m)\). If \(a \neq b\), we claim that there exists a Dirichlet character \(\psi\) modulo \(m\) with \(\psi(ab^{-1}) \neq 1\). Set \(g = ab^{-1}\). The cyclic subgroup \(\<g\>\) of \((\Z/m\Z)^{\ast}\) has some order \(d > 1\). Consider the homomorphism
        \[
        ...
        \[
          \psi(ab^{-1})\sum_{\chi \tmod{m}}\chi(ab^{-1}) = \sum_{\chi \tmod{m}}\chi(ab^{-1}),
        \]
  ...
  \section{Exponential Sums}
    ...
    \subsection{Exponential Sums}
      ...
      \]
      where \(A\) is a finite set of integers and \(f\) is a complex function on \(A\). In many instances, \(f\) is an arithmetic function so that \(A\) may be any finite set of positive integers. In any case, it is fruitful to view \(S_{f}\) as a geometric walk in \(\C\) where the \(a\)-th step is in the direction \(e(f(a))\). Taking absolute values, we always have the trivial bound
      \[
        S_{f} \le |A|.
      \]
      ...
      A particular type of exponential sum, where we expect close to square-root cancellation, are those of complete exponential sums. To define them, fix a integer \(m\) and let \(f\) be a complex function on \((\Z/m\Z)\) or \((\Z/m\Z)^{\ast}\). Then the corresponding exponential sum
      \[
    ...
    \subsection{Ramanujan Sums}
      Let \(m\) be a positive integer. For any integer \(b\), the \textbf{Ramanujan sum}\index{Ramanujan sum} \(R(b,m)\) modulo \(m\) is defined by
      \[
        R(b,m) = \sum_{\substack{a \tmod{m} \\ (a,m) = 1}}e\left(\frac{ab}{m}\right).
      \]
      This is a complete exponential sum. When \(b \mid m\) the summands are all \(1\) and the Ramanujan sum has the simple evaluation
      \[
        R(b,m) = \vphi(m).
      \]
      In fact, Ramanujan sums can be computed explicitly by means of the M\"obius function.
    ...
    \subsection{Gauss Sums}
      Let \(\chi\) be a Dirichlet character modulo \(m\). For any integer \(b\), the \textbf{Gauss sum}\index{Gauss sum} \(G(b,\chi)\) associated to \(\chi\) is given by
      \[
        G(b,\chi) = \sum_{a \tmod{m}}\chi(a)e\left(\frac{ab}{m}\right).
      \]
      This is a complete exponential sum. If \(b = 1\) we simply write \(G(\chi) = G(1,\chi)\). Also, when \(b \mid m\) the summands are all \(1\) and the Dirichlet orthogonality relations imply
      \[
        G(b,\chi) = \vphi(m)\d_{\chi,\chi_{m,0}}.
      \]
      In the case \(\chi = \chi_{m,0}\), we have
      \[
        G(b,\chi_{m,0}) = R(b,m),
      \]
      so that the Gauss sum reduces to a Ramanujan sum which can be evaluated via \cref{prop:Ramanujan_sum_evaluation}. This evaluation is not typical of a larger reality. Gauss sums, in general, are extremely difficult to compute. Nevertheless, we can develop many useful properties which help to reduce their evaluation.
      ...
      \begin{proposition}\label{prop:Gauss_sum_reduction}
        ...
        \begin{enumerate}[label*=(\roman*)]
          \item \(\conj{G(b,\cchi)} = \chi(-1)G(b,\chi)\).
          \item If \((b,m) = 1\) then \(G(b,\chi) = \cchi(b)G(\chi)\).
          \item If \((b,m) > 1\) and \(\chi\) is primitive then \(G(b,\chi) = 0\).
          \item If \((m,n) = 1\) then \(G(b,\chi\psi) = \chi(n)\psi(m)G(b,\chi)G(b,\psi)\).
          \item If \(\wtilde{\chi}\) is the primitive Dirichlet character of conductor \(q\) inducing \(\chi\), then
          \[
            G(\chi) = \mu\left(\frac{m}{q}\right)\wtilde{\chi}\left(\frac{m}{q}\right)G(\wtilde{\chi}).
          \]
        \end{enumerate}
      \end{proposition}
      \begin{proof}
        ...
        \begin{enumerate}[label*=(\roman*)]
          \item This is equivalent to the identity
          \[
            \sum_{a \tmod{m}}\chi(a)e\left(-\frac{ab}{m}\right) = \chi(-1)\sum_{a \tmod{m}}\chi(a)e\left(\frac{ab}{m}\right),
          \]
          which is clear upon making the change of variables \(a \mapsto -a\).
          \item This is equivalent to the identity
          \[
            \sum_{a \tmod {m}}\chi(a)e\left(\frac{ab}{m}\right) = \cchi(b)\sum_{a \tmod {m}}e\left(\frac{a}{m}\right),
          \]
          which is clear under the change of variables \(a \mapsto ab^{-1}\).
          \item Observe that if \((b,m) > 1\) then \(m > 1\). We will separate the proof into two cases depending on if \(m\) is prime or not. In the first case, \(m = p\) for a prime \(p\). Then \(p \mid b\) whence
          \[
            G(b,\chi) = \sum_{a \tmod{m}}\chi(a).
          \]
          Therefore \(G(b,\chi) = 0\) by the orthogonality relations because \(\chi\) is primitive. In the second case, we have \(p \mid (b,m)\) for a prime \(p\). Then there exists an integer \(c\) satisfying \(c \equiv 1 \tmod {\frac{m}{p}}\) and \(\chi(c) \neq 1\). Such a \(c\) exists because otherwise \(\chi\) is induced from a Dirichlet character modulo \(\frac{m}{p}\) and thus is imprimitive. A short computation then shows
          \[
            G(b,\chi) = \chi(c)G(b,\chi),
          \]
          which implies \(G(b,\chi) = 0\).
          ...
          \[
            G(\chi) = G(\chi_{\frac{m}{q},0})\wtilde{\chi}\left(\frac{m}{q}\right)G(\wtilde{\chi}).
          \]
          As \(G(\chi_{\frac{m}{q},0}) = r\left(1,\frac{m}{q}\right)\), we use \cref{prop:Ramanujan_sum_evaluation} to compute \(G(\chi_{\frac{m}{q},0}) = \mu\left(\frac{m}{q}\right)\). Whence
          \[
            G(\chi) = \mu\left(\frac{m}{q}\right)\wtilde{\chi}\left(\frac{m}{q}\right)G(\wtilde{\chi}).
          \]
          Now suppose \(\left(\frac{m}{q},q\right) > 1\). In this case the right-hand side is zero because \(\wtilde{\chi}\left(\frac{m}{q}\right) = 0\). So we must show \(G(\chi) = 0\). Now there exists a prime \(p\) with \(p \mid \frac{m}{q}\) and \(p \mid q\). For any \(a\) modulo \(m\) write \(a = a'\frac{m}{p}+a''\) with \(a'\) modulo \(p\) and \(a''\) modulo \(\frac{m}{p}\). Moreover, as \(p \mid \frac{m}{q}\) we know \(q \mid \frac{m}{p}\). These two facts and a short computation together show
          \[
            G(\chi) = \left(\sum_{a' \tmod{p}}e\left(\frac{a'}{p}\right)\right)\left(\sum_{a'' \tmod{\frac{m}{p}}}\wtilde{\chi}(a'')e\left(\frac{a''}{m}\right)\right).
          \] 
          The first sum vanishes since it is the sum of all \(p\)-th roots of unity. This proves \(G(\chi) = 0\).
        \end{enumerate}
      \end{proof}

      Via properties (i) and (ii), the evaluation of the Gauss sum is reduced to that of \(G(\chi)\) at least when \(\chi\) is primitive. It is also not difficult to show that \(G(\chi)\) exhibits square-root cancellation. In fact, we can determine the modulus exactly.
      ...
      \end{theorem}
      \begin{proof}
        If \(\chi\) is the trivial Dirichlet character the claim is obvious since the Gauss sum is \(1\). So assume \(\chi\) is nontrivial whence \(q > 1\). Consider instead \(|G(\chi)|^{2} = G(\chi)\conj{G(\chi)}\). Expanding the Gauss sum \(\conj{G(\chi)}\) and invoking \cref{prop:Gauss_sum_reduction} (ii), a short computation shows
        \[
          |G(\chi)|^{2} = \sum_{a \tmod{q}}G(a,\chi)e\left(-\frac{a}{q}\right).
        \]
        Upon expanding the Gauss sum \(G(a,\chi)\), another short computation gives
        \[
          |G(\chi)|^{2} = \sum_{a' \tmod{q}}\chi(a')\left(\sum_{a \tmod{q}}e\left(\frac{a(a'-1)}{q}\right)\right).
        \]
        If \(a' \equiv 1 \tmod{q}\) the inner sum is \(q\) and otherwise vanishes as it is the sum of all \(q\)-th roots of unity. It follows that the double sum is \(q\) whence \(|G(\chi)|^{2} = q\). This is equivalent to the claim.
      \end{proof}
      ...
      \begin{corollary}\label{cor:gauss_sum_primitive_formula}
        ...
        \[
          G(b,\chi) = \cchi(b)G(\chi).
        \]
        In particular,
        \[
          \chi(b) = \frac{1}{G(\cchi)}\sum_{a \tmod{q}}\cchi(a)e\left(\frac{ab}{q}\right).
        \]
      \end{corollary}
      \begin{proof}
        For the first statement, the identity is obvious if \(\chi\) is the trivial character as the Gauss sum is \(1\). So assume \(\chi\) is nontrivial. If \((b,q) = 1\) then this is exactly \cref{prop:Gauss_sum_reduction} (ii). If \((b,q) > 1\) then the identity follows from \cref{prop:Gauss_sum_reduction} (iii) and that \(\cchi(b) = 0\). This proves the first statement in full. For the second statement, observe that \(G(\chi) \neq 0\) by \cref{thm:Gauss_sum_modulus}. The second identity follows upon replacing \(\chi\) with \(\cchi\), dividing by \(G(\chi)\), and expanding the Gauss sum.
      \end{proof}

      For a Dirichlet character \(\chi\) modulo \(m\), we define the \textbf{epsilon factor}\index{epsilon factor} \(\e_{\chi}\) by
      \[
        \e_{\chi} = \frac{G(\chi)}{\sqrt{m}}.
      \]
      ...
      \begin{proof}
        ...
        \[
          \e_{\cchi}^{-1} = \frac{\conj{G(\chi)}}{{\sqrt{q}}}.
        \]
        On the other hand, \cref{prop:Gauss_sum_reduction} (i) implies
        \[
          \e_{\chi} = \chi(-1)\conj{\frac{G(\chi)}{\sqrt{q}}}.
        \]
      ...
      \]
      This is a complete exponential sum. If \(b = 1\) we write \(g(m) = g(1,m)\). Also, when \(b \mid m\) the summands are all \(1\) and the quadratic Gauss sum evaluates to
      \[
        g(b,m) = m.
      \]
      It turns out that for square-free \(m\) the Gauss sum attached to the quadratic Dirichlet character modulo \(m\) given by the Jacobi symbol is precisely the quadratic Gauss sum. This takes some work to prove. The first step is to reduce to the case when \((b,m) = 1\).
      ...
      \begin{proposition}\label{prop:Gauss_sum_equivalence_for_primes}
        ...
        \[
          G(b,\chi_{m}) = \sum_{a \tmod{m}}\left(1+\legendre{a}{m}\right)e\left(\frac{ab}{m}\right).
        \]
        When \(m = p\) is prime,
        \[
          G(b,\chi_{p}) = g(b,p).
      ...
      \begin{proof}
        ...
        \[
          G(b,\chi_{p}) = 1+\sum_{\substack{a' \tmod{p} \\ a' \neq 0}}e\left(\frac{(a')^{2}b}{p}\right).
        \]
      ...
      \begin{proof}
        ...
        \[
          G(\chi) = \left(\prod_{i < j}\chi_{p_{j}}(p_{i})\chi_{p_{i}}(p_{j})\right)\left(\prod_{i}G(\chi_{p_{i}})\right).
        \]
        By \cref{prop:Gauss_sum_equivalence_for_primes}, we may write
        \[
          \left(\prod_{i < j}\chi_{p_{j}}(p_{i})\chi_{p_{i}}(p_{j})\right)\left(\prod_{i}G(\chi_{p_{i}})\right) = \left(\prod_{i < j}\legendre{p_{i}}{p_{j}}\legendre{p_{j}}{p_{i}}\right)\left(\prod_{i}g(p_{i})\right).
        \]
    ...
    \subsection{Kloosterman and Sali\'e Sums}
      Let \(c\) be a positive integer. For integers \(m\) and \(n\) the \textbf{Kloosterman sum}\index{Kloosterman sum} \(K(m,n,c)\) modulo \(c\) is defined by
      \[
        K(m,n,c) = \sum_{\substack{a \tmod{c} \\ (a,c) = 1}}e^{\frac{2\pi i\left(am+a^{-1}n\right)}{c}}.
      \]
      Notice that if either \(n = 0\) or \(m = 0\) then
      \[
        K(m,0,c) = R(m,c) \quad \text{or} \quad K(0,n,c) = R(n,c),
      \]
      respectively. So the Kloosterman sums reduces to a Ramanujan sum. The most important result about Kloosterman sums is an infamous bound known as the \textbf{Weil bound}\index{Weil bound} for Kloosterman sums.

      \begin{theorem*}[Weil bound]
        Let \(c\) be a positive integer. Then for integers \(m\) and \(n\), we have
        \[
          |K(m,n,c)| \le \s_{0}(c)\sqrt{(m,n,c)c}.
        \]
      \end{theorem*}

      In the case \((m,n,c) = 1\), the Weil bound shows that the Kloosterman sums exhibit square-root cancellation.

      The associated Sali\'e sums are Kloosterman sums with Dirichlet characters. Let \(c\) be a positive integer and \(\chi\) be a Dirichlet character modulo \(c\). For integers \(m\) and \(n\), the \textbf{Sali\'e sum}\index{Sali\'e sum} \(S_{\chi}(m,n,c)\) modulo \(c\) is defined by
      \[
        S_{\chi}(m,n,c) = \sum_{\substack{a \tmod{c} \\ (a,c) = 1}}\chi(a)e^{\frac{2\pi i\left(am+a^{-1}n\right)}{c}}.
      \]
      If either \(n = 0\) or \(m = 0\) then
      \[
        S_{\chi}(m,0,c) = G(m,\chi) \quad \text{or} \quad S_{\chi}(0,n,c) = G(m,\cchi),
      \]
      respectively. So the Sali\'e sum reduces to a Gauss sum.
    \subsection{Character Sums and the P\'olya-Vinogradov Inequality}
     ...
     The aforementioned improvement is known as the \textbf{P\'olya-Vinogradov inequality}\index{P\'olya-Vinogradov inequality} as it was proved independently by P\'olya and Vinogradov in 1918.
      ...
      \begin{proof}
        ...
        \[
          S_{\chi}(M,N) = \frac{1}{G(\cchi)}\sum_{a \tmod{q}}\cchi(a)\sum_{M \le n \le M+N}e\left(\frac{an}{q}\right).
        \]
