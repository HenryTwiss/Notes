  \section{Integral Lattices}
    Let \(F\) be a characteristic zero field and \(V\) be an \(\F\)-vector space of dimension \(n\) with nondegenerate symmetric inner product \(\<\cdot,\cdot\>\). We say that a subset \(\L\) of \(V\) is a \textit{lattice} if \(\L\) is a free abelian group. In particular, any lattice \(\L\) is of the form
    \[
