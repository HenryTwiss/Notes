...
\section{Fourier Series}\label{sec:Fourier_series}
  Let \(\L\) be a complete integral lattice in \(\R^{n}\) with fundamental domain \(\mc{M}\) and denote the dual integral lattice by \(\L^{\vee}\). Suppose \(f(\mathbf{x})\) is \(\L\)-periodic and integrable on \(\mc{M}\). Then we define the \(\l^{\vee}\)-th \textbf{Fourier coefficient}\index{Fourier coefficient} \(\hat{f}(\l^{\vee})\) of \(f(\mathbf{x})\) by
  \[
  ...
  \begin{theorem}\label{thm:Fourier_series_n_dimensions}
  ...
  In the case of \(1\)-periodic functions of a single variable, we can do better as we may merely assume \(f(x)\) is piecewise smooth.

  \begin{theorem}\label{thm:Fourier_series_1_dimensional}
    Suppose \(f(x)\) is a piecewise smooth function on \(\R\) which is \(1\)-periodic. Then the Fourier series of \(f(x)\) converges locally uniformly to \(f(x)\) on every set where \(f(x)\) is continuous. Moreover, at any jump discontinuity, the Fourier series of \(f(x)\) converges to the average of the left-hand and right-hand limits of \(f(x)\).
  \end{theorem}
...
\section{Poisson Summation}\label{sec:Poisson_summation}
  Let \(\L\) be a complete integral lattice in \(\R^{n}\) with fundamental domain \(\mc{M}\) and denote the dual integral lattice by \(\L^{\vee}\). Recall that the \(\l^{\vee}\)-th Fourier coefficient \(\hat{f}(\l^{\vee})\) of \(f(\mathbf{x})\) is
  \[
    \hat{f}(\l^{\vee}) = \int_{\mc{M}}f(\mathbf{x})e^{-2\pi i\<\l^{\vee},\mathbf{x}\>}\,d\mathbf{x},
  \]
  while the Fourier series \(S_{f}(x)\) is
  \[
    S_{f}(\mathbf{x}) = \frac{1}{V_{\L}}\sum_{\l^{\vee} \in \L^{\vee}}\hat{f}(\l^{\vee})e^{2\pi i\<\l^{\vee},\mathbf{x}\>}.
  \]
  There are two natural methods of building a function from \(f(\mathbf{x})\) that is \(\l\)-periodic. The first is to average \(f(\mathbf{x})\) over all translates by elements of \(\L\) while the second is to consider the Fourier series of \(f(\mathbf{x})\). This gives us the two series
  \[
    \sum_{\l \in \L}f(\mathbf{x}+\l) \quad \text{and} \quad \frac{1}{V_{\L}}\sum_{\l^{\vee} \in \L^{\vee}}(\mc{F}f)(\l^{\vee})e^{2\pi i\<\l^{\vee},\mathbf{x}\>}.
  \]
  ...
  \begin{proof}
    ...
    \]
    \(F_{P}(\mathbf{x})\) is smooth and \(1\)-periodic in each component because \(F(\mathbf{x})\) is smooth and invariant under translation by \(\L\). Whence \(F_{P}(\mathbf{x})\) admits a Fourier series converging uniformly everywhere to \(F_{P}(\mathbf{x})\) by \cref{thm:Fourier_series_n_dimensions}. Moreover, \(F_{P}(\mathbf{x})\) is absolutely integrable on \([0,1]^{n}\). We will compute the \(\mathbf{t}\)-th Fourier coefficient of \(F_{P}(\mathbf{x})\) given by
    \[
  ...
  For convenience, we state Poisson summation in the simplified case for the complete integral lattice \(\Z^{n}\) as a corollary since it is how Poisson summation is usually applied.
  ...
  \end{corollary}
  \begin{proof}
    This is Poisson summation for the complete integral lattice \(\Z^{n}\) since \(\Z^{n}\) is self-dual and \(V_{\Z^{n}} = 1\).
  \end{proof}

  We can prove a slightly stronger form of Poisson summation in the single variable setting for the complete integral lattice \(\Z\) since we only require that the average is piecewise smooth.
  ...
  \begin{theorem}
    ...
    \]
    is piecewise smooth, where the \(\ast\) in the sum indicates that \(f(x+n)\) is meant to represent the average of the left-hand and right-hand limits at jump discontinuities. Then
    \[
  ...
  \end{theorem}
  \begin{proof}
    Observe that \(F(x)\) is \(1\)-periodic. As \(F(x)\) is piecewise smooth, it admits a Fourier series converging locally uniformly to \(F(x)\) wherever \(F(x)\) is continuous by \cref{thm:Fourier_series_1_dimensional}. In fact, by the construction of \(F(x)\) and that the Fourier series of \(F(x)\) converges to the average of the left-hand and right-hand limits at jump discontinuities, the Fourier series of \(F(x)\) converges locally uniformly to \(F(x)\) everywhere. Moreover, \(F(x)\) is absolutely integrable on \([0,1]\). We compute the \(t\)-th Fourier coefficient of \(F(x)\) given by
    \[
  ...
  In practical settings, we need a class of functions \(f(\mathbf{x})\) for which the assumptions of Poisson summation hold. We say that \(f(\mathbf{x})\) is of \textbf{Schwarz class}\index{Schwarz class} if \(f \in C^{\infty}(\R^{n})\) and \(f(\mathbf{x})\) along with all of its partial derivatives have rapid decay. If \(f(\mathbf{x})\) is of Schwarz class, the rapid decay implies that \(f(\mathbf{x})\) and all of its derivatives are absolutely integrable over \(\R^{n}\). Moreover, \(F(\mathbf{x}) = \sum_{\mathbf{n} \in \Z^{n}}f(\mathbf{x}+\mathbf{n})\) and all of its derivatives are locally absolutely uniformly convergent by the Weierstrass \(M\)-test. The uniform limit theorem then ensures \(F(\mathbf{x})\) is smooth and thus the conditions of Poisson summation are satisfied. So in most practical applications only only needed to ensure that \(f(\mathbf{x})\) is Schwarz class.
