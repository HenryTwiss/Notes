\chapter{Real Analysis}
  Throughout, all sums over \(\Z\) are assumed to be ordered symmetrically if they are not absolutely convergent.
  \section{The Fourier Transform}\label{sec:the_Fourier_transform}
    ...
    \]
    for real \(\xi\). The integral is absolutely convergent because \(f(x)\) is absolutely integrable. Let us first demonstrate some properties of the Fourier transform.
    ...
    There is also an inverse transform. Let \(F(\xi)\) be an absolutely integrable function on \(\R\). Then the \textbf{inverse Fourier transform}\index{inverse Fourier transform} \((\mc{F}^{-1}F)(x)\) is defined by
    \[
      (\mc{F}^{-1}F)(x) = \int_{\R}F(\xi)e^{2\pi i\xi x}\,d\xi,
    \]
    for real \(x\). It is absolutely convergent because \(F(\xi)\) is absolutely integrable on \(\R\).
    ...
    \begin{theorem*}[Fourier inversion]
      Let \(f(x)\) be a piecewise continuous and absolutely integrable function on \(\R\). Assume that its Fourier transform \(F(\xi)\) is absolutely integrable on \(\R\). Then
      \[
        f(x) = (\mc{F}^{-1}F)(x),
      \]
      where \(f(x)\) is understood to mean the average of the left-hand and right-hand limits at jump discontinuities. Conversely, let \(F(\xi)\) be a piecewise continuous and absolutely integrable function on \(\R\). Assume that its inverse Fourier transform \(f(x)\) is absolutely integrable on \(\R\). Then
      \[
        F(\xi) = (\mc{F}f)(\xi),
      \]
  ...
  \section{Fourier Series}\label{sec:Fourier_series}
    Suppose \(f(x)\) is a \(1\)-periodic function on \(\R\) and integrable on \([0,1]\). Then the \(t\)-th \textbf{Fourier coefficient}\index{Fourier coefficient} \(\hat{f}(t)\) of \(f(x)\) is defined by
    \[
      \hat{f}(t) = \int_{0}^{1}f(x)e^{-2\pi itx}\,dx.
    \]
    Accordingly, the \textbf{Fourier series}\index{Fourier series} \(S_{f}(x)\) of \(f(x)\) is given by
    \[
  ...
  \section{Poisson Summation}\label{sec:Poisson_summation}
    Let \(f(x)\) be an absolutely integrable function on \(\R\). There are two natural methods of building a function from \(f(x)\) which is \(1\)-periodic. The first is to average \(f(x)\) over all translates by elements of \(\Z\) while the second is to consider the Fourier series of \(f(x)\). This gives us the two series
    \[
