...
\section{Poisson Summation}\label{sec:Poisson_summation}
  ...
  \begin{theorem}
    Let \(f(x)\) be an absolutely integrable function on \(\R\) and such that
    \[
      F(x) = \sum_{n \in \Z}f(x+n),
    \]
    is piecewise continuously differentiable on \(\R\). Then
    \[
  ...
  In practical settings, we need a class of functions \(f(x)\) for which the assumptions of Poisson summation hold. We say that \(f(x)\) is of \textbf{Schwarz class}\index{Schwarz class} if \(f(x)\) is smooth and has rapid decay along with all of its derivatives. If \(f(x)\) is of Schwarz class, the rapid decay implies that \(f(x)\) and all of its derivatives are absolutely integrable on \(\R\). Moreover, \(F(x) = \sum_{n \in \Z}f(x+n)\) and all of its derivatives are locally absolutely uniformly convergent by the Weierstrass \(M\)-test. The uniform limit theorem then ensures \(F(x)\) is smooth and thus the conditions of Poisson summation are satisfied. It turns out that in most practical applications we only needed to ensure \(f(x)\) is Schwarz class.

  We now introduce the prototypical Schwarz class function and compute its Fourier transform. This function is \(f(x) = e^{-2\pi x^{2}}\) and is of  particularly interest because it is essentially invariant under the Fourier transform.
