\chapter{Complex Analysis}
  ...
  \section{The Mellin Transform}\label{sec:The_Mellin_transform}
    Let \(\psi(x)\) be a function on \((0,\infty)\) such that \(\psi(x) = O_{\e}(\min(x^{-(a+\e)},x^{-(b+\e)}))\) with \(a < b\). Then the \textbf{Mellin transform}\index{Mellin transform} \((\mc{M}\psi)(s)\) of \(\psi(x)\) is defined by
    \[
      (\mc{M}\psi)(s) = \int_{0}^{\infty}\psi(x)x^{s-1}\,dx.
    \]
    for \(a < \s < b\). This integral is locally absolutely uniformly convergent due to the asymptotic satisfied by \(\psi(x)\) upon writing
    \[
    ...
    There is also an inverse transform. Let \(\Psi(s)\) be a holomorphic function on the vertical strip \(a < \s < b\) and such that \(\Psi(s) = O_{\e}((|t|+3)^{-(1+\e)})\). Then the \textbf{inverse Mellin transform}\index{inverse Mellin transform} \((\mc{M}^{-1}\Psi)(x)\) of \(\Psi(s)\) is defined by
    \[
      (\mc{M}^{-1}\Psi)(x) = \frac{1}{2\pi i}\int_{(c)}\Psi(s)x^{-s}\,ds,
    \]
    for any positive \(x\) and \(a < c < b\). This integral is independent of \(c\) and locally absolutely uniformly convergent because of the asymptotic satisfied by \(\Psi(s)\).
    ...
    \begin{theorem*}[Mellin inversion]
      ...
      \[
        \Psi(s) = (\mc{M}\psi)(s).
      \]
      Conversely, suppose \(\psi(x)\) is a piecewise continuous function on \((0,\infty)\) such that \(\psi(x) = O_{\e}(\min(x^{-(a+\e)},x^{-(b+\e)}))\) with \(a < b\). If \(\Psi(s)\) is the Mellin transform of \(\psi(x)\), then
      \[
        \psi(x) = (\mc{M}^{-1}\Psi)(x),
      \]
