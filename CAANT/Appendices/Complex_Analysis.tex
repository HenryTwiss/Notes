\chapter{Complex Analysis}
  ...
  \section{The Mellin Transform}\label{sec:The_Mellin_transform}
    Let \(\psi(x)\) be a function on \((0,\infty)\) such that \(\psi(x) = O_{\e}(\min(x^{-(a+\e)},x^{-(b+\e)}))\) with \(a < b\). Its \textbf{Mellin transform}\index{Mellin transform} \((\mc{M}\psi)(s)\) is defined by
    \[
      (\mc{M}\psi)(s) = \int_{0}^{\infty}\psi(x)x^{s-1}\,dx.
    \]
    for \(a < \s < b\). This integral is locally absolutely uniformly convergent precisely because of the asymptotic satisfied by \(\psi(x)\) upon writing
    \[
      (\mc{M}\psi)(s) = \int_{0}^{1}\psi(x)x^{s-1}\,dx+\int_{1}^{\infty}\psi(x)x^{s-1}\,dx.
    \]
    Of course, if \(\psi(x)\) exhibits rapid decay then its Mellin transform is locally absolutely uniformly convergent in the half-plane \(\s > a\). In any case, the Mellin transform is holomorphic in the corresponding region.

    One of the most important properties about the Mellin transform is that it can be inverted. Let \(\Psi(s)\) be a holomorphic function on the vertical strip \(a < \s < b\) and such that \(\Psi(s) = O_{\e}((|t|+3)^{-(c+\e)})\) for some \(c \ge 1\). Then its \textbf{inverse Mellin transform}\index{inverse Mellin transform} \((\mc{M}^{-1}\Psi)(x)\) is defined by
    \[
      (\mc{M}^{-1}\Psi)(x) = \frac{1}{2\pi i}\int_{(\s)}\Psi(s)x^{-s}\,ds,
    \]
    for any positive \(x\). This integral is locally absolutely uniformly convergent precisely because of the asymptotic satisfies by \(\Psi(s)\).

    It is useful to know under what conditions the inverse Mellin transform of a Mellin transform returns the original function. This result is known as Mellin inversion and it turns out that the assumptions needed are quite mild.

    \begin{theorem*}[Mellin inversion]
      Let \(\Psi(s)\) be a holomorphic function on the vertical strip \(a < \s < b\) and such that \(\Psi(s) = O_{\e}((|t|+3)^{-(c+\e)})\) for some \(c \ge 1\). If \(\psi(x)\) is the inverse Mellin transform of \(\Psi(x)\), then
      \[
        \Psi(s) = (\mc{M}\psi)(s).
      \]
      Conversely, suppose \(\psi(x)\) is a piecewise continuously differentiable function on \((0,\infty)\) such that \(\psi(x) = O_{\e}(\min(x^{-(a+\e)},x^{-(b+\e)}))\) with \(a < b\). If \(\Psi(s)\) is the Mellin transform of \(\psi(x)\), then
      \[
        \psi(x) = (\mc{M}^{-1}\Psi)(x),
      \]
      where \(\psi(x)\) is understood to mean the average of its left-hand and right-hand limits at jump discontinuities.
    \end{theorem*}
  \section{The Gamma Function}\label{sec:the_gamma_function}
    The \textbf{gamma function}\index{gamma function} \(\G(s)\) is defined by the integral
    \[
      \G(s) = \int_{0}^{\infty}e^{-x}x^{s-1}\,dx.
    \]
    Equivalently, \(\G(s)\) is the Mellin transform of \(e^{-x}\). As \(e^{-x}\) exhibits exponential decay, this defines a holomorphic function for \(\s > 0\). Also note that \(\G(s)\) is real for \(s > 0\). The most basic properties of \(\G(s)\) are collected in the following result:
    ...
    \[
      \G(s+1) = s\G(s),
    \]
    is referred to as the \textbf{functional equation}\index{functional equation} for the gamma function. The functional equation will allow for the meromorphic conduction of the Gamma function to all of \(\C\).
    ...
    \]
    In order to deduce more analytic properties of the gamma function, we will introduce an auxiliary function that will be more amenable to study. The \textbf{beta function}\index{beta function} \(B(s,u)\) is defined by
    \[
    ...
    \end{equation}
    This shows that beta function is the Mellin transform of \(\frac{1}{(s+1)^{s+u}}\). This formula is immensely useful when \(s+u = 1\) because we may then evaluate \(B(s,u)\) by means of the residue theorem. We will make use of this fact to prove \textbf{Euler's reflection formula}\index{Euler's reflection formula}.
    ...
    \begin{proof}
      ...
      \]
      We will evaluate this integral in the limit as \(R \to \infty\) and then as \(\e \to 0\). The contour encloses the only pole of the integrand which is simple at \(z = -1\) and of residue \(e^{\pi i(s-1)}\) (after taking the branch cut \(\log_{0}\) of the logarithm along the positive real line). So, on the one hand, the residue theorem gives
      \[
        \int_{\eta}\frac{z^{s-1}}{z+1}\,dz = 2\pi ie^{\pi i(s-1)},
      \]
      where in the last equality we have used that \(\log_{0}(-1) = i\pi\) (since this is the branch cut of the logarithm along the positive real line). On the other hand, the integral is a sum along all four contours. Clearly
      \[
    ...
    Now knowing that the gamma function has no zeros, we can use the beta function to prove an important multiplicativity property of the gamma function. This is known as \textbf{Legendre's duplication formula}\index{Legendre's duplication formula}.

    \begin{theorem*}[Legendre's duplication formula]
      For any \(s \in \C-\Z_{0-}\), we have
      \[
    ...
    \begin{proof}
      ...
      \]
      We now take the limit as \(n \to \infty\). By \cref{equ:limit_form_of_exponential} and that \(\left(\frac{x}{n}+1\right)^{n}\) is monotonically increasing in \(n\), we may interchange the limit and integral by monotone convergence giving
      \[
    ...
    Upon inspection of the Hadamard factorization, we find that the reciprocal of the gamma function is of order \(1\). We now turn to analyzing the growth rate of the gamma function itself. In fact, we can obtain a well-known asymptotic formula for the gamma function known as \textbf{Stirling's formula}\index{Stirling's formula}.
    ...
    We can use \cref{equ:weaker_Stirling_formula_1} to obtain weaker estimates valid on the negative real line away from the poles of the gamma function.
    ...
    Having discussed the major properties of the gamma function, we now introduce a related function. We call \(\frac{\G'}{\G}(s)\) the \textbf{digamma function}\index{digamma function}. In other words, the digamma function is the logarithmic derivative of the gamma function. From \cref{thm:continuation_of_gamma_function}, it is meromorphic on \(\C\) with simple poles at the nonpositive integers whose residues are all \(-1\). Taking the logarithmic derivative of the functional equation shows
    \[
    ...
    We can use \cref{equ:weaker_approximtion_for_digamma_1} to obtain a weaker estimate valid on the negative real line away from the poles of the gamma function.
