\chapter{Complex Analysis}
  ...
  \section{The Mellin Transform}\label{sec:The_Mellin_transform}
    ...
    \]
    This integral need not be convergent. However, if \(\psi(x) = O_{\e}(\min(x^{-(a+\e)},x^{-(b+\e)}))\) for some \(a < b\) then it is locally absolutely uniformly convergent for \(a < \s < b\) upon writing
    \[
    ...
    \begin{theorem*}[Mellin inversion]
      ...
      \[
        \Psi(s) = (\mc{M}\psi)(s).
      \]
      Conversely, suppose \(\psi(x)\) is a piecewise smooth function on \((0,\infty)\) such that \(\psi(x) = O_{\e}(\min(x^{-(a+\e)},x^{-(b+\e)}))\) with \(a < b\). If \(\Psi(s)\) is the Mellin transform of \(\psi(x)\), then
      \[
        \psi(x) = (\mc{M}^{-1}\Psi)(x),
      \]
