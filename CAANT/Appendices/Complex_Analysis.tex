  ...
  \section{The Mellin Transform}\label{sec:The_Mellin_transform}
    ...
    \begin{theorem*}[Mellin inversion]
      ...
      \[
        \Psi(s) = (\mc{M}\psi)(s).
      \]
      Conversely, suppose \(\psi(x)\) is a piecewise smooth function on \((0,\infty)\) taking the average of its left-hand and right-hand limits at any jump discontinuity and such that \(\psi(x) = O_{\e}(\min(x^{-(a+\e)},x^{-(b+\e)}))\) with \(a < b\). If \(\Psi(s)\) is the Mellin transform of \(\psi(x)\), then
      \[
        \psi(x) = (\mc{M}^{-1}\Psi)(x).
      \]
    \end{theorem*}
