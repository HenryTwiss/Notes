\chapter{Complex Analysis}
  ...
  \section{The Mellin Transform}\label{sec:The_Mellin_transform}
    Let \(\psi(x)\) be a function on \((0,\infty)\). Then the \textbf{Mellin transform}\index{Mellin transform} \((\mc{M}\psi)(s)\) of \(\psi(x)\) is defined by
    \[
      (\mc{M}\psi)(s) = \int_{0}^{\infty}\psi(x)x^{s-1}\,dx.
    \]
    This integral need not be convergent. However, if
    \[
      \psi(x) = \begin{cases} O_{\e}(x^{-a-\e}) & \text{if \(x \ll 1\)}, \\ O_{\e}(x^{-b+\e}) & \text{if \(x \gg 1\)}, \end{cases}
    \]
    for some \(a < b\) then it is locally absolutely uniformly convergent for \(a < \s < b\) upon writing
    \[
      (\mc{M}\psi)(s) = \int_{0}^{1}\psi(x)x^{s-1}\,dx+\int_{1}^{\infty}\psi(x)x^{s-1}\,dx.
    \]
    Of course, if we may take \(a\) or \(b\) arbitrary large then the Mellin transform is locally absolutely uniformly convergent in the half-plane \(\s < b\) or \(\s > a\) respectively. In any case, the Mellin transform is holomorphic in the corresponding region.

    There is also an inverse transform. Let \(\Psi(s)\) be a holomorphic function on the vertical strip \(a < \s < b\). Then the \textbf{inverse Mellin transform}\index{inverse Mellin transform} \((\mc{M}^{-1}\Psi)(x)\) of \(\Psi(s)\) is defined by
    \[
      (\mc{M}^{-1}\Psi)(x) = \frac{1}{2\pi i}\int_{(c)}\Psi(s)x^{-s}\,ds,
    \]
    for any \(a < c < b\). This integral need not converge or even be independent of \(c\). Yet if \(\Psi(s) = O_{\e}((|t|+3)^{-(1+\e)})\) then the integral is locally absolutely uniformly convergent and independent of \(c\).
    ...
    \begin{theorem*}[Mellin inversion]
      Let \(\Psi(s)\) be a holomorphic function on the vertical strip \(a < \s < b\) and such that \(\Psi(s) = O_{\e}((|t|+3)^{-(1+\e)})\). If \(\psi(x)\) is the inverse Mellin transform of \(\Psi(x)\), then
      \[
        \Psi(s) = (\mc{M}\psi)(s).
      \]
  ...
  \section{The Gamma Function}\label{sec:the_gamma_function}
    ...
    \]
    Equivalently, \(\G(s)\) is the Mellin transform of \(e^{-x}\). As \(e^{-x}\) exhibits exponential decay and is bounded near zero, this defines a holomorphic function for \(\s > 0\). Also note that \(\G(s)\) is real for \(s > 0\). The most basic properties of \(\G(s)\) are collected in the following result:
    ...
    \begin{proof}
      ...
      \]
      so it suffices to show that the remaining integral is \(\frac{\pi}{\sin(\pi s)}\). Let \(\eta = \sum_{i}\eta_{i}\) be the keyhole contour \cref{fig:reflection_formula_contour} where the large arc has radius \(R\) and the small arc has radius \(\e\).
