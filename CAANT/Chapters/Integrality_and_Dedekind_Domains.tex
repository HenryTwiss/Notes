\chapter{Integrality and Dedekind Domains}
  ...
  \section{Dedekind Domains}
    ...
    More work will be necessary obtain reverse inclusion. We being by showing that every prime \(\mf{p}\) is invertible. Note that unlike integral ideals, \(1 \in \mf{p}^{-1}\) so that \(\mf{p}^{-1}\) contains units.
  ...
  \section{\todo{Orders}}
    Let \(\mc{O}/\smc{O}\) be a Dedekind extension for a degree \(n\) separable extension \(L/K\). We will often consider integral domains that are in-between \(\smc{O}\) and \(\mc{O}\) but which are not themselves Dedekind domains as they need not be integrally closed domains. An \textbf{order}\index{order} \(\L\) of a Dedekind extension \(\mc{O}/\smc{O}\) for a degree \(n\) separable extension \(L/K\) is a finitely generated \(\smc{O}\)-module of \(\mc{O}\) which contains a basis for \(L/K\). Accordingly, we call \(\mc{O}\) the \textbf{maximal order}\index{maximal order} of \(\mc{O}/\smc{O}\). Necessarily, the order \(\L\) satisfies
    \[
      \smc{O} \subseteq \L \subseteq \mc{O}.
    \]
    A few useful facts about \(\L\) are also immediate. As \(\mc{O}\) is the integral closure of \(\smc{O}\) in \(L\), \(\L\) is integral over \(\smc{O}\). Moreover, the field of fractions of \(\L\) contains \(K\) and as \(\L\) contains a basis of \(L/K\), the field of fractions must be \(L\) itself. Also, \(\L\) is noetherian and every prime is maximal. Indeed, any integral ideal of \(\mf{A}\) of \(\L\) is a finitely generated \(\smc{O}\)-module, as \(\L\) itself is, whence a finitely generated \(\L\)-module. This shows \(\L\) is noetherian. Now suppose \(\mf{P}\) is a prime of \(\L\). We claim \(\mf{P} \cap \smc{O}\) is a prime of \(\smc{O}\). It is clearly an ideal and is prime because \(\mf{P}\) is. To see that it is not the zero ideal, let \(\l \in \mf{P}\) be nonzero. As \(\l\) is integral over \(\smc{O}\), we have
    \[
      \l^{n}+\a_{n-1}\l^{n-1}+\cdots+\a_{0} = 0,
    \]
    for some positive integer \(n\) and \(\a_{i} \in \smc{O}\). Taking \(n\) minimal ensures \(\a_{0} \neq 0\). Isolating \(\a_{0}\) shows \(\a_{0} \in \mf{P}\) whence \(\a_{0} \in \mf{P} \cap \smc{O}\). Moreover, it follows that \(\a_{0}\L \subset \mf{P} \subseteq \L\). Therefore \(\mf{P}\) has the same rank as \(\L\) as \(\smc{O}\)-modules. Then the integral domain \(\L/\mf{P}\) is necessarily finite and thus a field proving \(\mf{P}\) is maximal. These facts show that while orders need not be integrally closed domains, they satisfy every other property of being a Dedekind domain. In practice, orders generally arise in the form
    \[
      \L = \smc{O}[\a_{1},\ldots,\a_{n}],
    \]
