\chapter{Integrality and Dedekind Domains}
  ...
  \section{\todo{Orders}}
    ...
    \[
      \Pic(\smc{O}) = \Cl(\smc{O}).
    \]
    It is possible to describe the Picard group in terms of principal fractional ideals for the localizations \(\smc{O}_{\mf{p}}\) at primes \(\mf{p}\). To prove this, we need some results specific to orders and more generally one-dimensional noetherian domains. Our first property is that localization preserves orders.
    ...
    \begin{proposition}
      ...
      \[
        I(\mf{o}) \cong \bigop_{\mf{p}}P(\mf{o}_{\mf{p}}),
      \]
      induced by natural inclusion. In particular, upon identifying \(P(\mf{o})\) with its image under this isomorphism, we have
      \[
        \Pic(\mf{o}) \cong \left(\bigop_{\mf{p}}P(\mf{o}_{\mf{p}})\right)/P(\mf{o}).
      \]
    \end{proposition}
    \begin{proof}
      \todo{xxx}
