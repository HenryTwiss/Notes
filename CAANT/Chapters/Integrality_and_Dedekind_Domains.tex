\chapter{Integrality and Dedekind Domains}
  ...
  \section{Localization}
    ...
    \begin{proof}
      ...
      \]
      for some positive integer \(n\) and \(\frac{\a_{i}}{\d_{i}} \in \smc{O}D^{-1}\). Letting \(\d = \prod_{i}\d_{i}\) and multiplying by \(\d^{n}\), we obtain
      \[
  ...
  \section{\todo{Orders}}
    ...
    \[
      \Pic(\smc{O}) = \Cl(\smc{O}).
    \]
    It is possible to describe the Picard group in terms of principal fractional ideals for the localizations \(\smc{O}_{\mf{p}}\) at primes \(\mf{p}\). To prove this, we need some results specific to orders and more generally one-dimensional noetherian domains. Our first property is that localization preserves orders and their conductors.

    \begin{proposition}\label{prop:localization_preserves_orders}
      Let \(\smc{O}\) be an order with conductor \(\mf{q}(\smc{O})\). Then for any multiplicative subset \(D\) of \(\smc{O}\), \(\smc{O}D^{-1}\) is an order with conductor \(\mf{q}(\smc{O})D^{-1}\).
    \end{proposition}
    \begin{proof}
    ...
    \begin{proposition}\label{prop:localized_Chinese_remainder_theorem}
      ...
      \[
        \smc{O}/\mf{a} \cong \bigop_{\mf{p} \subseteq \mf{a}}\smc{O}_{\mf{p}}/\mf{a}_{\mf{p}},
      \]
      induced by natural inclusion.
    \end{proposition}
    \begin{proof}
      ...
      \]
      induced by natural inclusion. It remains to demonstrate that natural inclusion induces an isomorphism \(\smc{O}/\wtilde{\mf{a}}_{\mf{p}} \cong \smc{O}_{\mf{p}}/\mf{a}_{\mf{p}}\). To see this, consider the homomorphism
      \[
    ...
    Our third property is that when \(\smc{O}\) is a one-dimensional noetherian domain, a fractional ideal of \(\smc{O}\) is invertible if and only if its localization at a prime \(\mf{p}\) is principal in \(\smc{O}_{\mf{p}}\).

    \begin{proposition}\label{prop:invertible_fractional_ideal_using_localization}
      Let \(\smc{O}\) be a one-dimensional noetherian domain with field of fractions \(K\). Then a fractional ideal \(\mf{f}\) of \(\smc{O}\) is invertible if and only if the fractional ideal \(\mf{f}_{\mf{p}}\) of \(\smc{O}_{\mf{p}}\) is principal for every prime \(\mf{p}\) of \(\smc{O}\).
    \end{proposition}
    \begin{proof}
      ...
      \[
        \sum_{i}\a_{i}\b_{i} = 1.
      \]
      Moreover, not all of the \(\a_{i}\b_{i}\) can lie in the unique maximal ideal \(\mf{p}_{\mf{p}}\) of the local ring \(\smc{O}_{\mf{p}}\) for otherwise this ideal is not proper. Without loss of generality, we may assume \(\a_{1}\b_{1} \notin \mf{p}_{\mf{p}}\) and so it is a unit in \(\smc{O}_{\mf{p}}\). As \(\mf{a}_{\mf{p}}\mf{b} = \smc{O}_{\mf{p}}\) by the invertibility of \(\mf{a}\), it follows that \(\mf{a}_{\mf{p}} = \a_{1}\smc{O}_{\mf{p}}\). This shows \(\mf{a}_{\mf{p}}\) is principal. For the reverse implication, suppose \(\mf{a}_{\mf{p}}\) is principle for every prime \(\mf{p}\) of \(\smc{O}\). Whence \(\mf{a}_{\mf{p}} = \a_{\mf{p}}\smc{O}_{\mf{p}}\) for some nonzero \(\a_{\mf{p}} \in K\). As \(K\) is the field of fractions of \(\smc{O}_{\mf{p}}\), we may multiply by a nonzero element of \(\smc{O}_{\mf{p}}\), if necessary, to ensure that \(\a_{\mf{p}} \in \mf{a}\). Now if \(\mf{a}\) were not invertible, then there would exist a prime \(\mf{p}\) such that
      \[
        \mf{a}\mf{a}^{-1} \subseteq \mf{p},
      \]
      since \(\smc{O}\) is one-dimensional. Let \(\a_{1},\ldots,\a_{r}\) be generators of \(\mf{a}\) as an \(\smc{O}\)-module. As \(\a_{i} \in \mf{a}_{\mf{p}}\), we may write \(\a_{i} = \a_{\mf{p}}\frac{\eta_{i}}{\d_{i}}\) with \(\frac{\eta_{i}}{\d_{i}} \in \smc{O}_{\mf{p}}\). Then \(\d_{i}\a_{i} \in \a_{\mf{p}}\smc{O}\) and so \(\d_{i}\a_{i}\a_{\mf{p}}^{-1} \in \smc{O}\). Setting \(\d = \prod_{i}\d_{i}\), it follows that \(\d\a_{\mf{p}}^{-1}\mf{a} \subseteq \smc{O}\). Thus \(\d\a_{\mf{p}}^{-1} \in \mf{a}^{-1}\) from which we find \(\d \in \mf{a}\mf{a}^{-1}\) whence \(\d \in \mf{p}\). This is a contradiction and so \(\mf{a}\) is invertible.
    \end{proof}
    ...
    \begin{proposition}
      ...
      \]
      induced by natural inclusion. In particular, upon identifying \(P(\smc{O})\) with its image under this isomorphism, we have
      \[
    ...
    \iffalse
    We are now ready to introduce orders. Let \(\mc{O}/\smc{O}\) be a Dedekind extension for a degree \(n\) separable extension \(L/K\). We will often consider integral domains that are in-between \(\smc{O}\) and \(\mc{O}\) but which are not themselves Dedekind domains as they need not be integrally closed domains. An \textbf{order}\index{order} \(\mf{o}\) of a Dedekind extension \(\mc{O}/\smc{O}\) for a degree \(n\) separable extension \(L/K\) is a finitely generated \(\smc{O}\)-module of \(\mc{O}\) which contains a basis for \(L/K\). Accordingly, we call \(\mc{O}\) the \textbf{maximal order}\index{maximal order} of \(\mc{O}/\smc{O}\). Necessarily, the order \(\mf{o}\) satisfies
    \[
      \smc{O} \subseteq \mf{o} \subseteq \mc{O}.
    \]
    A few useful facts about \(\mf{o}\) are also immediate. As \(\mc{O}\) is the integral closure of \(\smc{O}\) in \(L\), \(\mf{o}\) is integral over \(\smc{O}\). Moreover, the field of fractions of \(\mf{o}\) contains \(K\) and as \(\mf{o}\) contains a basis of \(L/K\), the field of fractions must be \(L\) itself. Also, \(\mf{o}\) is a one-dimensional noetherian domain. Indeed, any integral ideal of \(\mf{A}\) of \(\mf{o}\) is a finitely generated \(\smc{O}\)-module, as \(\mf{o}\) itself is, whence a finitely generated \(\mf{o}\)-module. This shows \(\mf{o}\) is noetherian. Now suppose \(\mf{P}\) is a prime of \(\mf{o}\). We claim \(\mf{P} \cap \smc{O}\) is a prime of \(\smc{O}\). It is clearly an ideal and is prime because \(\mf{P}\) is. To see that it is not the zero ideal, let \(\l \in \mf{P}\) be nonzero. As \(\l\) is integral over \(\smc{O}\), we have
    \[
      \l^{n}+\a_{n-1}\l^{n-1}+\cdots+\a_{0} = 0,
    \]
    for some positive integer \(n\) and \(\a_{i} \in \smc{O}\). Taking \(n\) minimal ensures \(\a_{0} \neq 0\). Isolating \(\a_{0}\) shows \(\a_{0} \in \mf{P}\) whence \(\a_{0} \in \mf{P} \cap \smc{O}\). Whence \(\a_{0}\mf{o} \subset \mf{P} \subseteq \mf{o}\) which shows that \(\mf{P}\) has the same rank as \(\mf{o}\) as \(\smc{O}\)-modules. Then the integral domain \(\mf{o}/\mf{P}\) is necessarily finite and thus a field proving \(\mf{P}\) is maximal. These facts show that while orders need not be integrally closed domains, they satisfy every other property of being a Dedekind domain. In practice, orders generally arise in the form
    \[
      \mf{o} = \smc{O}[\a_{1},\ldots,\a_{n}],
    \]
    where \(\a_{1},\ldots,\a_{n}\) satisfy \(L = K(\a_{1},\ldots,\a_{n})\).
