\chapter{Integrality and Dedekind Domains}
  ...
  \section{Localization}
    ...
    \begin{proposition}\label{prop:invertible_fractional_ideal_using_localization}
      Let \(\smc{O}\) be a one-dimensional noetherian domain with field of fractions \(K\). Then a fractional ideal \(\mf{f}\) of \(\smc{O}\) is invertible if and only if \(\mf{f}_{\mf{p}}\) is a principal fractional ideal of \(\smc{O}_{\mf{p}}\) for every prime \(\mf{p}\) of \(\smc{O}\).
    \end{proposition}
    \begin{proof}
      \todo{xxx}
    \end{proof}
