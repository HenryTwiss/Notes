\chapter{Integrality and Dedekind Domains}
  ...
  \section{Dedekind Domains}
    ...
    \]
    Note that  \(\d \in \mf{f}^{-1}\) whence \(\d\mf{f}^{-1}\) is an integral ideal so that \(\mf{f}^{-1}\) is a fractional ideal. We say that \(\mf{f}\) is \textbf{invertible}\index{invertible} if
    \[
      \mf{f}\mf{f}^{-1} = \smc{O}.
    \]
    ...
    \begin{proof}
      ...
      \]
      Since \(\mf{p}\) is maximal, it follows that \(\mf{p}^{-1}\mf{p}\) is either \(\mf{p}\) or \(\smc{O}\). So it suffices to show that the first case cannot hold. Assume by contradiction that \(\mf{p}^{-1}\mf{p} = \mf{p}\). Let \(\w_{1},\ldots,\w_{r}\) be generators of \(\mf{p}\) as an \(\smc{O}\)-module and let \(\a \in \mf{p}^{-1}-\smc{O}\). Then \(\a\w_{i} \in \mf{p}^{-1}\mf{p}\) whence \(\a\mf{p} \subseteq \mf{p}^{-1}\mf{p}\) and \(\a\mf{p} \subseteq \mf{p}\). But then
      \[
  ...
  \section{Localization}
    ...
    There is a generalization of the Chinese remainder theorem provided \(\smc{O}\) is a one-dimensional noetherian domain.
    ...
    Also when \(\smc{O}\) is a one-dimensional noetherian domain, a fractional ideal of \(\smc{O}\) is invertible if and only if its localizations at all primes is principal in \(\smc{O}_{\mf{p}}\).
    ...
    We will now turn to the setting when \(\smc{O}\) is an integrally closed domain. In this case, localization respects integral closure.
  ...
  \section{\todo{Orders}}
    ...
    There is a generalization of the ideal class group to orders as well. Since \(\mf{o}\) is not necessarily a Dedekind domain, fractional ideals need not be invertible. The natural restriction is to then consider all fractional ideals which are invertible. Let \(I(\mf{o})\) denote the set of invertible fractional ideals of \(\mf{o}\) which are invertible. Then it is easy to see that this is an abelian group.

    \begin{theorem}\label{thm:order_ideal_group_is_a_group}
      Let \(\mc{O}/\smc{O}\) be a Dedekind extension for a degree \(n\) separable extension \(L/K\) and let \(\mf{o}\) be an order of \(\mc{O}/\smc{O}\). Then \(I(\mf{o})\) is an abelian group with identity \(\mf{o}\).
    \end{theorem}
    \begin{proof}
      Associativity and commutativity of \(I(\mf{o})\) are obvious. The identity is \(\mf{o}\) because every invertible fractional ideal is a finitely generated \(\mf{o}\)-submodule of \(L\). Lastly, every fractional ideal is invertible by definition of \(I(\mf{o})\).
    \end{proof}
    
    Let \(P(\mf{o})\) denote the subgroup of principal fractional ideals of \(I(\mf{o})\). In fact, \(P(\mf{o})\) contains every principal fraction ideal of \(\mf{o}\) because every principal fraction ideal is invertible. As \(I(\mf{o})\) is abelian by the previous result, \(P(\mf{o})\) is normal. The \textbf{Picard group}\index{Picard group} \(\Pic(\mf{o})\) of \(\mf{o}\) is defined to be the quotient
    \[
      \Pic(\mf{o}) = I(\mf{o})/P(\mf{o}).
    \]
    Of course, in the case of the maximal order \(\mc{O}\), the Picard group is precisely the ideal class group which is to say
    \[
      \Pic(\mc{O}) = \Cl(\mc{O}).
    \]
    It is possible to describe the Picard group in terms of principal fractional ideals for the localizations of \(\mf{o}\) at primes.

    \begin{proposition}
      Let \(\mc{O}/\smc{O}\) be a Dedekind extension for a degree \(n\) separable extension \(L/K\) and let \(\mf{o}\) be an order of \(\mc{O}/\smc{O}\). Then there is an isomorphism
      \[
  ...
  \iffalse
    Another important fact about orders which we will see is that their localizations at primes behave much the same as for Dedekind domains for almost all primes. To begin to describe the similarity, we will prove a generalization of the Chinese remainder theorem.
