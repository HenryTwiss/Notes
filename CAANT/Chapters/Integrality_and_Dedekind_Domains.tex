\chapter{Integrality and Dedekind Domains}
  ...
  \section{Localization}
    ...
    \begin{theorem}\label{thm:Dedekind_if_and_only_if_all_localizations_are_discrete_valuation_rings}
      Let \(\smc{O}\) be a noetherian domain. Then \(\smc{O}\) is a Dedekind domain if and only if \(\smc{O}_{\mf{p}}\) is a discrete valuation ring.
    \end{theorem}
    \begin{proof}
  ...
  \section{\todo{Orders}}
    ...
    \begin{proof}
      ...
      \]
      for primes \(\mf{q}\) with \(\mf{q} \neq \mf{p}\). Whence \(\frac{\a}{\d} \in \mf{f}_{\mf{p}}\) and \cref{prop:ring_is_intersection_of_all_localizations} implies \(\frac{\a}{\d} \in \mf{f}\). But then \(\k_{\mf{p}} \in \mf{f}_{\mf{p}}\) and the reverse inclusion holds. Altogether, this means \(\phi\) is surjective. To see that it is injective, suppose \(\mf{f} \in I(\smc{O})\) is such that \(\mf{f}_{\mf{p}} = \smc{O}_{\mf{p}}\). Then \cref{prop:ring_is_intersection_of_all_localizations} implies \(\mf{a} \subseteq \smc{O}\). This forces \(\mf{a} = \smc{O}\). For otherwise \(\mf{a}\) is proper and so it is contained in a maximal ideal which is necessarily prime. As \(\smc{O}\) is one-dimensional, there would exist a prime \(\mf{p}\) for which \(\mf{a} \subseteq \mf{p}\). But then \(\mf{a}_{\mf{p}} \subseteq \mf{p}_{\mf{p}}\) which is a contradiction as \(\mf{p}_{\mf{p}}\) is the unique maximal ideal of \(\smc{O}_{\mf{p}}\) and therefore cannot be \(\smc{O}_{\mf{p}}\). Therefore \(\mf{a} = \smc{O}\) which means \(\phi\) is injective. This establishes the first isomorphism. The second isomorphism is immediate by the definition of the Picard group.
    \end{proof}
