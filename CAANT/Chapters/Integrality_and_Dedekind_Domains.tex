\chapter{Integrality and Dedekind Domains}
  ...
  \section{Dedekind Domains}
    ...
    \[
      \mf{f}\mf{f}^{-1} = \smc{O}.
    \]
    A priori, a fractional ideal need not be invertible nor should we expect it to be. In order to guarantee invertibility, we need to consider a more restrictive setting than mere integral domains. This will allow for integral ideals to factor uniquely into a product of primes and for the fractional ideals to form a group under multiplication in which every fractional ideal is invertible and \(\smc{O}\) is the identity. This will essentially permit us to treat integral ideas as we would integers.
    ...
    It follows from this result that every fractional ideal is invertible in a Dedekind domain. We will use this fact so often that, rather than reference it, we will suppress it for legibility.
    
    It is an interesting fact that for a Dedekind domain, being a principal ideal domain is equivalent to being a unique factorization domain.
    ...
    We will now discuss applications of the Chinese remainder theorem in the context of integral ideals. With it we can prove some other interesting results. Suppose \(\mf{a}\) is an integral ideal with prime factorization
    \[
      \mf{a} = \mf{p}_{1}^{e_{1}} \cdots \mf{p}_{r}^{e_{r}}.
    \]
  ...
  \section{Localization}
    ...
    Also when \(\smc{O}\) is a one-dimensional noetherian domain, a fractional ideal is invertible if and only if its localizations at all primes is principal.
    ...
    \end{proposition}
    \begin{proof}
      Write \(\mf{f} = \frac{1}{\d}\mf{a}\) for some nonzero \(\d \in \smc{O}\) and integral ideal \(\mf{a}\). Then it suffices to show the claim for the integral ideal \(\mf{a}\). For legibility, write \(\mf{b} = \mf{a}^{-1}\).For the forward implication, suppose \(\mf{a}\) is invertible so that \(\mf{a}\mf{b} = \smc{O}\). Then
      \[
        \sum_{i}\a_{i}\b_{i} = 1,
      \]
      for some \(\a_{i} \in \mf{a}\) and \(\b_{i} \in \mf{b}\). Moreover, not all of the \(\a_{i}\b_{i}\) can lie in the unique maximal ideal \(\mf{p}_{\mf{p}}\) of the local ring \(\smc{O}_{\mf{p}}\) for otherwise this ideal is not proper. Without loss of generality, we may then assume \(\a_{1}\b_{1}\) is a unit in \(\smc{O}_{\mf{p}}\). As \(\mf{a}_{\mf{p}}\mf{b} = \smc{O}_{\mf{p}}\) because \(\mf{a}\) is invertible, it follows that \(\mf{a}_{\mf{p}} = \a_{1}\smc{O}_{\mf{p}}\). This shows \(\mf{a}_{\mf{p}}\) is principal. For the reverse implication, suppose \(\mf{a}_{\mf{p}}\) is principle for every prime \(\mf{p}\) of \(\smc{O}\). Whence \(\mf{a}_{\mf{p}} = \a_{\mf{p}}\smc{O}_{\mf{p}}\) for some nonzero \(\a_{\mf{p}} \in K\). As \(K\) is the field of fractions of \(\smc{O}_{\mf{p}}\), we may multiply by a nonzero element of \(\smc{O}_{\mf{p}}\), if necessary, to ensure that \(\a_{\mf{p}} \in \mf{a}\). Now if \(\mf{a}\) were not invertible, then there would exist a prime \(\mf{p}\) such that
      \[
  ...
  \section{\todo{Orders}}
    ...
    \]
    A few useful facts about \(\mf{o}\) are also immediate. As \(\mc{O}\) is the integral closure of \(\smc{O}\) in \(L\), \(\mf{o}\) is integral over \(\smc{O}\). Moreover, the field of fractions of \(\mf{o}\) contains \(K\) and as \(\mf{o}\) contains a basis of \(L/K\), the field of fractions must be \(L\) itself. Also, \(\mf{o}\) is noetherian and every prime is maximal. Indeed, any integral ideal of \(\mf{A}\) of \(\mf{o}\) is a finitely generated \(\smc{O}\)-module, as \(\mf{o}\) itself is, whence a finitely generated \(\mf{o}\)-module. This shows \(\mf{o}\) is noetherian. Now suppose \(\mf{P}\) is a prime of \(\mf{o}\). We claim \(\mf{P} \cap \smc{O}\) is a prime of \(\smc{O}\). It is clearly an ideal and is prime because \(\mf{P}\) is. To see that it is not the zero ideal, let \(\l \in \mf{P}\) be nonzero. As \(\l\) is integral over \(\smc{O}\), we have
    \[
      \l^{n}+\a_{n-1}\l^{n-1}+\cdots+\a_{0} = 0,
    \]
    for some positive integer \(n\) and \(\a_{i} \in \smc{O}\). Taking \(n\) minimal ensures \(\a_{0} \neq 0\). Isolating \(\a_{0}\) shows \(\a_{0} \in \mf{P}\) whence \(\a_{0} \in \mf{P} \cap \smc{O}\). Moreover, it follows that \(\a_{0}\mf{o} \subset \mf{P} \subseteq \mf{o}\). Therefore \(\mf{P}\) has the same rank as \(\mf{o}\) as \(\smc{O}\)-modules. Then the integral domain \(\mf{o}/\mf{P}\) is necessarily finite and thus a field proving \(\mf{P}\) is maximal. These facts show that while orders need not be integrally closed domains, they satisfy every other property of being a Dedekind domain. In practice, orders generally arise in the form
    \[
