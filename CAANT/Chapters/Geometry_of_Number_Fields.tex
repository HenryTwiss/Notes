\chapter{Geometry of Number Fields}
  ...
  \section{\todo{Minkowski Space}}
    ...
    \]
    for all \((z_{\s})_{\s} \in K_{\R}\). We define the \textbf{canonical embedding}\index{canonical embedding} \(j\) of \(K\) to be the \(\Q\)-embedding
    \[
    ...
    \begin{proof}
      ...
      \]
      As both maps are \(\R\)-linear, it follows that the original map induces a component-wise isomorphism between \(K_{\R}\) and \(\R^{n}\) proving the first statement. For the second statement, let \((z_{\s})_{\s},(z'_{\s})_{\s} \in K_{\R}\) and let \((x_{\s})_{\s},(x'_{\s})_{\s} \in \R^{n}\) be the corresponding elements in \(\R^{n}\) under this isomorphism respectively. If \(\s = \rho\) then
      \[
    ...
    \]
    The Minkowski embedding is a \(\Q\)-embedding of \(K\) into \(\R^{n}\) since it is the composition of the canonical embedding and the isomorphism established in the previous result.
    ...
    \iffalse
    \begin{proof}
      The second identity follows from the first by taking \(\mf{f} = \mc{O}_{K}\) so it remains to prove the first two statements. Let \(\k_{1},\ldots,\k_{n}\) be a basis for \(\mf{f}\) and let \(\s_{1},\ldots,\s_{n}\) be the elements of \(\Hom_{\Q}(K,\conj{\Q})\). Then the associated generator matrix for \(j(\mf{f})\) is the embedding matrix \(M(\k_{1},\ldots,\k_{n})\). Hence
      \[
        V_{j(\mf{f})} = |\det(M(\k_{1},\ldots,\k_{n}))|,
      \]
      proving the first statement. To prove the remaining identity, it suffices to show
      \[
        |\det(M(\k_{1},\ldots,\k_{n}))| = \Norm_{K}(\mf{f})|\det(M(\a_{1},\ldots,\a_{n}))|,
      \]
      for any integral basis \(\a_{1},\ldots,\a_{n}\) in view of \cref{equ:embedding_matrix_discriminant_relation}. \todo{xxx} As \(\mf{f}\) is a fractional ideal, there exists a nonzero \(\d \in \mc{O}_{K}\) and an integral ideal \(\mf{a}\) such that
      \[
        \mf{f} = \frac{1}{\d}\mf{a}.
      \]
