\chapter{Geometry of Number Fields}
  Throughout, all rings will be understood to be commutative and with unity. We will also use the theory of integral lattices in \cref{sec:integral_lattices}.
  \section{\todo{Minkowski Space}}
    ...
    To give a more explicit description of Minkowski space, let \(N_{\s} = 1,2\) according to if \(\s\) is real or complex respectively. Also let \(\rho\) and \(\t\) run over the real \(\Q\)-embeddings and a complete set of representatives for pairs of complex \(\Q\)-embeddings for \(\Hom_{\Q}(K,\conj{\Q})\). Note that \(N_{\rho} = 1\) and \(N_{\tau} = 2\). Then
    \[
    ...
    \end{proposition}
    \begin{proof}
      Upon writing \(\mf{f} = \frac{1}{d}\mf{a}\) for some nonzero \(d \in \Z\) and integral ideal \(\mf{a}\) and \cref{prop:norm_and_field_norms_are_identical}, it suffices to prove the statements for an integral ideal \(\mf{a}\).
      As \(\mf{a}\) is complete, let \(\k_{1},\ldots,\k_{n}\) be a basis for \(L/K\) contained in \(\mf{a}\). Also let \(\s_{1},\ldots,\s_{n}\) be the elements of \(\Hom_{\Q}(K,\conj{\Q})\). Then the associated generator matrix for \(j(\mf{a})\) is the embedding matrix \(M(\k_{1},\ldots,\k_{n})\). Hence
      \[
      ...
      \]
      for any integral basis \(\a_{1},\ldots,\a_{n}\) for \(K\). Note that \(\k_{1},\ldots,\k_{n}\) is necessarily a basis for \(\mf{a}\) as a free \(\Z\)-module by \cref{thm:integral_basis_AKBL} and write
      \[
    ...
    \end{proof}
  \fi\section{\todo{Finiteness of the Class Number}}\iffalse
