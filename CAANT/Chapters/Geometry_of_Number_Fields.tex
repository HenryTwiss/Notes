\chapter{Geometry of Number Fields}
  ...
  \section{Finiteness of the Class Number}
    ...
    \begin{proof}
      ...
      \[
        X' = \left\{(\mathbf{x}_{1},\mathbf{r},\boldsymbol{\t}) \in \R^{r_{1}} \op B(0,n)^{r_{2}}:\sum_{i}|x_{i}|+2\sum_{j}r_{j} \le n\right\}.
      \]
      ...
      where
      \[
        X'' = \left\{(\mathbf{x}_{1},\mathbf{r}) \in \R_{+}^{r_{1}} \op B(0,n)^{r_{2}}:\sum_{i}x_{i}+2\sum_{j}r_{j} \le n\right\}.
      \]
      To compute the remaining integral, for nonnegative integers \(\ell\) and \(k\) and positive \(t\), let
      \[
        X''_{\ell,k}(t) = \left\{(\mathbf{x}_{1},\mathbf{r}) \in \R_{+}^{\ell} \op B(0,n)^{k}:\sum_{i}x_{i}+2\sum_{j}r_{j} \le n\right\},
      \]
      and set
      \[
  ...
  \section{\todo{Dirichlet's Unit Theorem}}\iffalse
