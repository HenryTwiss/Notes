  ...
  \section{Zero Density}
    ...
    \begin{lemma}\label{lem:powerful_L-function_approximation_lemma}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \]
        for bounded \(\s\) provided \(s\) is at least distance \(\e\) away from \(s = 0\) and \(s = 1\) and the nontrivial zeros of \(L(s)\). In addition, we may discard any finite number of terms in the sum if \(\s > 1\).
      \end{enumerate}
    \end{lemma}
    \begin{proof}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \item First suppose \(s\) in the vertical strip \(-\frac{1}{2} \le \s \le 2\) and at least distance \(\e\) away from \(s = 0\) and \(s = 1\). Taking the real part of the formula in \cref{prop:explicit_formula_log_derivative} and using \cref{equ:digamma_gamma_factor_analytic_conductor_estimate} and (i) gives
        \[
          -\Re\left(\frac{L'}{L}(s)\right) = -\sum_{\rho \neq 0,1}\Re\left(\frac{1}{s-\rho}\right)+O_{\e}(\log{\mf{q}(s,L)}).
        \]
