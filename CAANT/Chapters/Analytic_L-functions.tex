\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{The Lindel\"of Hypothesis and Estimates on the Critical Line}
    ...
    \begin{theorem}\label{thm:critical_line_estimate}
      ...
      \[
        L\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}\left(\frac{1}{2}+it,L\right)^{\e}\sup_{X \asymp \sqrt{\mf{q}\left(\frac{1}{2}+it,L\right)}}\left(\frac{|A_{\w}(t,X,L)|}{\mf{q}\left(\frac{1}{2}+it,L\right)^{\frac{1}{4}}}\right).
      \]
    \end{theorem}
    \begin{proof}
      ...
      \begin{align*}
        L\left(\frac{1}{2}+it\right) &\ll \left|\sum_{n \ge 1}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}V_{\frac{1}{2}+it}\left(\frac{n}{\sqrt{q_{L}}}\right)\right|+\left|\sum_{n \ge 1}\frac{\conj{a_{L}(n)}n^{it}}{\sqrt{n}}V_{\frac{1}{2}-it}\left(\frac{n}{\sqrt{q_{L}}}\right)\right|+e^{-|t|}.
      \end{align*}
      We now turn to estimating the remaining sums which will be accomplished by applying the smooth dyadic weight. Consider the first sum. By our choice of \(\Phi(u)\), the estimates in \cref{prop:V_function_decay} are valid which implies that the sum converges absolutely. Now write 
      \[
        V_{s}(y) = \sum_{k \in \Z}\w\left(\frac{y}{2^{k}}\right)V_{s}(y).
      \]
      Apply this identity to the sum and interchange the resulting two sums by absolute convergence to obtain
      \[
      ...
      \]
      By compact support of the smooth dyadic weight, for each \(k\) the inner sum over \(n\) is supported on the dyadic block \(n \asymp X_{k}\) where \(X_{k} = 2^{k}\sqrt{q_{L}}\) and the summands are zero for sufficiently negative \(k\). Therefore the summands which contribute must satisfy \(k \ll \log\mf{q}_{\infty}\left(\frac{1}{2}+it,L\right)\). Whence our double sum is
      \[
        \sum_{k \ll \log\mf{q}_{\infty}\left(\frac{1}{2}+it,L\right)}\sum_{n \asymp X_{k}}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}\w\left(\frac{n}{X_{k}}\right)V_{\frac{1}{2}+it}\left(\frac{n}{\sqrt{q_{L}}}\right).
      \]
      Now set \(V_{k}(y) = \frac{1}{\sqrt{y}}V_{\frac{1}{2}+it}\left(\frac{y}{\sqrt{q_{L}}}\right)\). By Abel summation, the inner sum is \(O\) of
      \[
      ...
      \[
        \sup_{X \asymp \sqrt{\mf{q}\left(\frac{1}{2}+it,L\right)}}\left(\frac{|A_{\w}(t,X,L)|}{\mf{q}\left(\frac{1}{2}+it,L\right)^{\frac{1}{4}}}\right).
      \]
      As \(q_{L} \asymp \mf{q}\left(\frac{1}{2}+it,L\right)\) and there are \(O_{\e}\left(\mf{q}\left(\frac{1}{2}+it,L\right)^{\e}\right)\) many such \(k\), this bound implies that our sum is \(O_{\e}\) of
      \[
        \mf{q}\left(\frac{1}{2}+it,L\right)^{\e}\sup_{X \asymp \sqrt{\mf{q}\left(\frac{1}{2}+it,L\right)}}\left(\frac{|A_{\w}(t,X,L)|}{\mf{q}\left(\frac{1}{2}+it,L\right)^{\frac{1}{4}}}\right).
      \]
    ...
    \begin{theorem}\label{thm:central_value_estimate}
      Let \(L(s)\) be an analytic \(L\)-function and let \(\w(y)\) be a smooth dyadic weight. Then
      \[
        L\left(\frac{1}{2}\right) \ll_{\e} \mf{q}\left(\frac{1}{2},L\right)^{\e}\max_{X \asymp \sqrt{\mf{q}\left(\frac{1}{2},L\right)}}\left(\frac{|A_{\w}(0,X,L)|}{\mf{q}\left(\frac{1}{2},L\right)^{\frac{1}{4}}}\right).
      \]
    \end{theorem}
    \begin{proof}
