\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Analytic Data}
    ...
    \begin{enumerate}[align=left]
      ...
      \[
        \L(s,L) = q_{L}^{\frac{s}{2}}\g(s,L)L(s) \quad \text{and} \quad \g(s,L) = \pi^{-\frac{d_{L}s}{2}}\prod_{j}\G\left(\frac{s+\mu_{j}}{2}\right),
      \]
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{equation}\label{equ:gamma_factor_s_and_u_estimate}
      \frac{\g(s+u,L)}{\g(s,L)} \ll_{\e} \frac{\mf{q}_{\infty}(s+u)^{\frac{\s+\tau}{2}-\frac{1}{4}}}{\mf{q}_{\infty}(s)^{\frac{\s}{2}-\frac{1}{4}}}e^{-d_{L}\frac{\pi}{2}(|t+r|-|t|)},
    \end{equation}
    for \(s\) in the critical strip and bounded \(\tau\) provided \(s+u\) is at least distance \(\e\) away from the poles of the gamma factor. Since \(\Phi(u)\) is bounded in the vertical strip \(|\tau| < a+1\), the integrand exhibits exponential decay. Therefore the integral is locally absolutely uniformly bounded and hence \(V_{s}(y,L)\) is smooth. The function \(V_{s}(y,L)\) is the smooth cutoff function mentioned previously. We will also let \(\e(s,L)\) be given by
    \[
    ...
    \begin{proof}
      ...
      \]    
      Stirling's formula (see \cref{sec:the_gamma_function}) shows that \(\g(s+u,L)\) exhibits exponential decay while \(L(s+u)\) has at most polynomial growth by \cref{prop:L_function_bounded_in_vertical_strips}. Since \(\Phi(u)\) is bounded in the vertical strip \(|\tau| < a+1\), it follows that the integrand exhibits exponential decay. Therefore the integral is locally absolutely uniformly bounded. We will evaluate the integral in two ways. On the one hand, we can expand \(L(s+u)\) inside the integrand as a Dirichlet series and by absolute boundedness of the integral we may interchange the sum and integral. A short computation shows that this is
      \[
    ...
    With our choice of \(\Phi(u)\), this result shows that \(V_{s}(y,L)\) is essentially \(1\) up to some admissible error for \(y \ll \sqrt{\mf{q}_{\infty}(s,L)}\) and exhibits rapid decay thereafter as we can take \(M\) (and hence \(N\)) to be arbitrarily large.
    ...
    \]
    defined for \(y > 0\) where \(a > 1\). By our choice of \(\psi(y)\), its inverse Mellin transform \(\Psi(s)\) has rapid decay. Since \(L(s)\) has at most polynomial growth by \cref{prop:L_function_bounded_in_vertical_strips}, the integrand has rapid decay as well. Therefore the integral is locally absolutely uniformly bounded and hence \(\psi(y,L)\) is smooth for \(y > 0\). We will also set
    \[
      R_{L} = \Res_{s = 1}L(s).
    \]
  ...
  \section{Logarithmic Derivatives}
    ...
    \end{equation}
    which we refer to as the \textbf{functional equation}\index{functional equation} of \(\xi(s,L)\). Observe that \(\xi(s,L)\) is just \(\L(s,L)\) with the potential poles at \(s = 0\) and \(s = 1\) removed. This means \(\xi(s,L)\) is entire. In fact, it admits a Hadamard factorization. To see this, first observe that \(\xi(s,L)\) is of order \(1\) by Stirling's formula (see \cref{sec:the_gamma_function}) and that \((s-1)^{m_{L}}L(s)\) is of order \(1\). Then the Hadamard factorization theorem implies
    \[
  ...
  \section{Zero Density}
    ...
    \begin{lemma}\label{lem:powerful_L-function_approximation_lemma}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \item We may write
        \[
          -\Re\left(\frac{L'}{L}(s)\right) = -\sum_{\rho \neq 0,1}\Re\left(\frac{1}{s-\rho}\right)+O_{\e}(\log{\mf{q}(s,L)}),
        \]
        for bounded \(\s\) provided \(s\) is at least distance \(\e\) away from \(s = 0\), \(s = 1\), the nontrivial zeros of \(L(s)\), and the poles of the gamma factor. In addition, we may discard any finite number of terms in the sum if \(\s > 1\).
      \end{enumerate}
    \end{lemma}
    \begin{proof}
      We will prove the properties separately.
      \begin{enumerate}[label*=(\roman*)]
        \item Using the Hadamard factorization of \(\xi(s,L)\) to take the logarithmic derivative of both sides of the functional equation gives
        \[
        ...
        \[
          2\Re(B_{L}) = -2\sum_{\rho \neq 0,1}\Re\left(\frac{1}{\rho}\right),
        \]
        which is equivalent to the claim.
        \item Let \(s = 3+iT\) so that the logarithmic derivative of \(L(s)\) is absolutely bounded. Taking the real part of the formula in \cref{prop:explicit_formula_log_derivative} and using \cref{equ:digamma_gamma_factor_analytic_conductor_estimate} along with (i) gives
        \[
        ...
        \[
          \sum_{\rho \neq 0,1}\frac{1}{1+(T-\g)^{2}} = O(\log{\mf{q}(iT,L)}).
        \]
        This proves the first statement. The second statement follows at once since all of the terms in the sum are positive at any least \(\frac{1}{2}\).
        \item Let \(\s > 1\). Then these estimates follow from direct computation since any nontrivial zero \(\rho\) satisfies \(\b \le 1\) while any gamma parameter \(\mu_{j}\) satisfies \(\Re(\mu_{j}) \ge 0\).
        \item First suppose \(s\) in the vertical strip \(-\frac{1}{2} \le \s \le 2\) and is at least distance \(\e\) away from \(s = 0\), \(s = 1\), the nontrivial zeros of \(L(s)\), and the poles of the gamma factor. Taking the real part of the formula in \cref{prop:explicit_formula_log_derivative} and using \cref{equ:digamma_gamma_factor_analytic_conductor_estimate} and (i) gives
        \[
          -\Re\left(\frac{L'}{L}(s)\right) = -\sum_{\rho \neq 0,1}\Re\left(\frac{1}{s-\rho}\right)+O_{\e}(\log{\mf{q}(s,L)}).
        \]
        By (iii), any term is the sum is positive and bounded provided \(\s > 1\) so that we may discard finitely many.
      \end{enumerate}
    \end{proof}

    With this lemma we can deduce a result which estimates the number of nontrivial zeros of \(L(s)\) in a box. For any positive \(T\), define
    \[
      N(T,L) = |\{\rho = \b+i\g \in \C:L(\rho) = 0 \text{ with } 0 \le \b \le 1 \text{ and } |\g| \le T\}|.
    \]
    In other words, \(N(T,L)\) is the number of nontrivial zeros of \(L(s)\) whose ordinate belongs to \([-T,T]\). We will use the argument principle to deduce an asymptotic formula for \(N(T,L)\) which will describe how the nontrivial zeros are distributed in the critical strip.
    ...
    \begin{proof}
      ...
      As the nontrivial zeros occur in pairs \(\rho\) and \(1-\conj{\rho}\), it follows that
      \[
        N(T,L) = 2N'(T,L)+O(\log{\mf{q}(f)}),
      \]
      where \(O(\log{\mf{q}(f)})\) accounts for the possible real nontrivial zeros of which there are finitely many. We will estimate \(N'(T,L)\) and in doing so we may assume \(L(s)\) does not vanish on the line \(t = T\) by varying \(T\) by a sufficiently small constant, if necessary, and observing that \(N'(T,L)\) is modified by a quantity of size \(O(\log{\mf{q}(iT,L)})\) by \cref{lem:powerful_L-function_approximation_lemma} (ii). Let \(\d > 0\) be sufficiently small such that \(\L(s,L)\) has no nontrivial zeros for \(-\d \le t < 0\). Also let \(\eta = \sum_{1 \le i \le 6}\eta_{i}\) be the rectangular contour \cref{fig:zero_counting_contour} where the horizontal lines are along \(t = T\) and \(t = -\d\) and the vertical lines are along \(\s = -\e\) and \(\s = 1+\e\).
      ...
      Then by our previous comments and the argument principle, we may write
      \[
      ...
      \]
      For convenience, set \(\eta_{L} = \eta_{1}+\eta_{2}+\eta_{3}\) and \(\eta_{R} = \eta_{4}+\eta_{5}+\eta_{6}\). \iffalse Recall that we have already shown \(\xi(\conj{s},L) = \conj{(\xi(s,L))}\). Using this fact along with the functional equation and that \(-\arg(s) = \arg(\conj{s})\), we compute
      \begin{align*}
      ...
      \]
      For the factor \((s(1-s))^{m_{L}}\), we first have
      \[
      ...
      \begin{equation}\label{equ:zero_counting_1}
        \D_{\eta_{L}}(s(1-s))^{m_{L}} = O\left(\frac{1}{T}\right).
      \end{equation}
      For the factor \(q_{L}^{\frac{s}{2}}\), we use that \(\arg(s) = \Im(\log(s))\) and compute
      \begin{equation}\label{equ:zero_counting_2}
        \D_{\eta_{L}}\arg{q_{L}^{\frac{s}{2}}} = \log{q_{L}}\left(\frac{T}{2}+\frac{\d}{2}\right) = \frac{T}{2}\log{q_{L}}+O(1).
      \end{equation}
      ...
      \]
      By \cref{equ:log_derivative_bound_conductor}, the integral is \(O(\log{q_{L}})\) on \(\eta_{2}\). On \(\eta_{1}\) and \(\eta_{3}\), \cref{lem:powerful_L-function_approximation_lemma} (ii) and (iv) together imply that the integral is \(O(\log{\mf{q}(iT,L)})\). It follows that
      \begin{equation}\label{equ:zero_counting_5}
        \D_{\eta_{L}}\arg(L(s)) = O(\log{\mf{q}(iT,L)}).
      \end{equation}
  ...
  \fi\iffalse\section{A Zero-free Region}
    ...
    \begin{lemma}\label{lem:non-vanshing_at_1_lemma}
      Let \(L(s)\) be an analytic \(L\)-function such that \(\Re(\L_{L}(n)) \ge 0\) provided \((n,Q) = 1\). Also suppose that \(|\a_{j}(p)| \le \frac{p}{2}\) for ramified primes \(p\). Then \(L(1,f) \neq 0\) and hence \(m_{L} \ge 0\). Moreover, there exists a constant \(c > 0\) such that \(L(s)\) has at most \(m_{L}\) real zeros in the region
      \[
        \s \ge 1-\frac{c}{d_{L}(m_{L}+1)\log{\mf{q}(f)}}.
      \]
    \end{lemma}
    \begin{proof}
      ...
      \[
        \sum_{j}\frac{1}{\s-\b_{j}} < \frac{m_{L}}{\s-1}+\Re\left(\frac{L'}{L}(\s,f)\right)+O(\log{\mf{q}(f)}).
      \]
      ...
      \[
        \sum_{j}\frac{1}{\s-\b_{j}} < \frac{m_{L}}{\s-1}+O(\log{d_{L}\mf{q}(f)}).
      \]
      From this inequality we see that \(\b_{j} \neq 1\). For if some \(\b_{j} = 1\), \(m_{L} < 0\) and the right-hand side is negative for \(\s\) sufficiently close to \(1\) contradicting the positivity of the left-hand side. Thus \(L(1,f) \neq 0\) and hence \(m_{L} \ge 0\). As there are finitely many \(\b_{j}\), there exists a \(c > 0\) such that the \(\b_{j}\) satisfy
      \[
        \b_{j} \ge 1-\frac{c}{d_{L}(m_{L}+1)\log{\mf{q}(f)}}.
      \]
      ...
      \[
        \frac{nd_{L}\log{\mf{q}(f)}}{2c+\frac{c}{(m_{L}+1)}} < \left(\frac{m_{L}}{2c}+O(1)\right)d_{L}\log{\mf{q}(f)}.
      \]
      Isolating \(n\), we see that
      \[
        n < m_{L}+\frac{m_{L}}{2(m_{L}+1)}+O(c),
      \]
      and taking \(c\) smaller, if necessary, we have \(n \le m_{L}\). As \(L(s)\) is nonzero for \(\s > 1\), it follows that there are at most \(m_{L}\) real zeros satisfying
      \[
        \s \ge 1-\frac{c}{d_{L}(m_{L}+1)\log{\mf{q}(f)}}.
      \]
    ...
    \begin{theorem}\label{thm:zero_free_region_generic}
      Let \(L(s)\) be an analytic \(L\)-function with at most a simple pole at \(s = 1\), \(\Re(\L_{L}(n)) \ge 0\) provided \((n,Q) = 1\), and \(|\a_{j}(p)| \le \frac{p}{2}\) for ramified primes \(p\). Then there exists a constant \(c > 0\) such that \(L(s)\) has no zeros in the region
      \[
  ...
  \fi\iffalse\section{The Explicit Formula}
    ...
    \begin{theorem*}[Explict Formula]
      ...
      \begin{align*}
        \sum_{n \ge 1}\left(\L_{L}(n)\psi(n)+\L_{\conj{f}}(n)\phi(n)\right) &= \psi(1)\log{q_{L}}+m_{L}\Psi(1) \\
        &+ \frac{1}{2\pi i}\int_{\left(\frac{1}{2}\right)}\left(\frac{\g'}{\g}(s,L)+\frac{\g}{\g}(1-s,L)\right)\Psi(s)\,ds-\sum_{\rho}\Psi(\rho).
      \end{align*}
    \end{theorem*}
    \begin{proof}
      ...
      \[
        \sum_{n \ge 1}\L_{L}(n)\psi(n) = m_{L}\Psi(1)-\sum_{\rho}\Psi(\rho)+\frac{1}{2\pi i}\int_{(-\d)}-\frac{L'}{L}(s)\Psi(s)\,ds.
      \]
      For the latter integral, the functional equation and that \(q(\conj{f}) = Q\) together imply
      \[
        -\frac{L'}{L}(s) = \log{q_{L}}+\frac{\g'}{\g}(s,L)+\frac{\g}{\g}(1-s,L)+\frac{L'}{L}(1-s,L).
      \]
      Thus
      \[
        \frac{1}{2\pi i}\int_{(-\d)}-\frac{L'}{L}(s)\Phi(s)\,ds = \frac{1}{2\pi i}\int_{(-\d)}\left(\log{q_{L}}+\frac{\g'}{\g}(s,L)+\frac{\g}{\g}(1-s,L)+\frac{L'}{L}(1-s,L)\right)\Psi(s)\,ds.
      \]
