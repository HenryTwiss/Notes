\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Logarithmic Derivatives}
    ...
    \end{equation}
    which we refer to as the \textbf{functional equation}\index{functional equation} for \(\xi(s,L)\). Now \(\xi(s,L)\) is just \(\L(s,L)\) with the potential poles at \(s = 0\) and \(s = 1\) removed. This means \(\xi(s,L)\) is entire. In fact, it admits a Hadamard factorization. To see this, first observe that \(\xi(s,L)\) is of order \(1\) by Stirling's formula and that \((s-1)^{m_{L}}L(s)\) is of order \(1\). Then the Hadamard factorization theorem implies
    \[
  ...
  \section{Zero Density Estimates}
    ...
    \begin{lemma}\label{lem:powerful_L-function_approximation_lemma}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \[
          \sum_{\rho \neq 0,1}\frac{1}{1+(T-\g)^{2}} = O(\log{\mf{q}(T,L)}).
        \]
        In particular, the number of nontrivial zeros \(\rho = \b+i\g\) with \(\rho \neq 0,1\) and such that \(|T-\g| \le 1\) is \(O(\log{\mf{q}(T,L)})\).
        \item Suppose \(s = \s+iT\) for bounded \(\s\) and real \(T\). Then
        \[
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \[
          \sum_{\rho \neq 0,1}\Re\left(\frac{1}{s-\rho}\right) = O(\log{\mf{q}(T,L)}),
        \]
    ...
    \begin{proof}
      ...
      \[
        \Im\left(\int_{\eta_{1} \cup \eta_{3}}\frac{L'}{L}(s)\,ds\right) = \sum_{\substack{\rho \neq 0,1 \\ |T-\g| \le 1}}\Im\left(\int_{\eta_{1} \cup \eta_{3}}\frac{1}{s-\rho}\,ds\right)+O(\log\mf{q}(T,L)),
      \]
  ...
  \section{The Explicit Formula}
    ...
    \begin{theorem*}[Explict Formula]
      ...
      \[
        \psi^{\ast}(X,L) = m_{L}X-\sum_{\rho \neq 0,1 }\frac{X^{\rho}}{\rho}+\sum_{j}\sum_{n \ge 1}\frac{X^{-(\mu_{j}+2n)}}{\mu_{j}+2n}-\Res_{s = 0}\left(\frac{X^{s}}{s}\frac{L'}{L}(s)\right).
      \]
    \end{theorem*}
    \begin{proof}
      ...
      In view of \cref{prop:explicit_formula_log_derivative}, the contour encloses a simple pole of the integrand at \(s = 1\) of residue \(m_{L}X\), a simple pole at \(s = \rho\) for every nontrivial zero \(\rho\) whose ordinate \(\g\) satisfies \(|\g| < T\) of residue \(-\frac{X^{\rho}}{\rho}\), a simple pole at \(s = -(\mu_{j}+2n)\) with \(n < \frac{U-\Re(\mu_{j})}{2}\) of residue \(\frac{X^{-(\mu_{j}+2n)}}{\mu_{j}+2n}\), and a pole at \(s = 0\). Thus
      \[
        \frac{1}{2\pi i}\int_{\eta}-\frac{L'}{L}(s)X^{s}\,\frac{ds}{s} = m_{L}X-\sum_{\substack{\rho \neq 0,1 \\ |\g| < T}}\frac{X^{\rho}}{\rho}+\sum_{j}\sum_{n < \frac{U-\Re(\mu_{j})}{2}}\frac{X^{-(\mu_{j}+2n)}}{\mu_{j}+2n}-\Res_{s = 0}\left(\frac{X^{s}}{s}\frac{L'}{L}(s)\right).
      \]
      ...
      \[
        -\frac{L'}{L}(s) = \log(q_{L})+\frac{\g'}{\g}(s,L)+\frac{\g'}{\g}(1-s,L)+\frac{\conj{L}'}{\conj{L}}(1-s).
      \]
      Let \(s\) be such that \(\s < -c\) and suppose \(s\) bounded away from the trivial zeros. Using \cref{digamma_gamma_factor_analytic_conductor_estimate} and that the logarithmic derivative on the right-hand side converges absolutely, we obtain a bound
      \[
