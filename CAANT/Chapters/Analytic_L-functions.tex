\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{The Explicit Formula}
    ...
    There is a much more useful variant which is obtained by applying truncated Perron's formula. For this, we need to introduce two related functions. For any analytic \(L\)-function \(L(s)\), we define \(\psi(x,L)\) by
    \[
    ...
    \begin{proof}
      ...
      \[
        \psi^{\ast}(X,L)-J(X,T) \ll X^{c}\sum_{\substack{n \ge 1 \\ n \neq X}}\frac{|\L_{L}(n)|}{n^{c}}\min\left(1,\frac{1}{T\left|\log\left(\frac{X}{n}\right)\right|}\right)+\d_{X}|\L_{L}(X)|\frac{c}{T},
      \]
      where
      \[
        J(X,T) = \frac{1}{2\pi i}\int_{c-iT}^{c+iT}-\frac{L'}{L}(s)X^{s}\,\frac{ds}{s},
      \]
      for any \(c > 1+\t_{L}\) and \(\d_{X} = 0\) unless \(X\) is a prime power. We will begin by bounding the above sum on the right-hand side of the previous estimate. \todo{xxx}
    \end{proof}
