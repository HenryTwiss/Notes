\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Zero Density Estimates}
    ...
    \begin{lemma}\label{lem:powerful_L-function_approximation_lemma}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \item Suppose \(s = \s+iT\) for bounded \(\s\) and \(T \ge 0\). Then
        \[
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \item Let \(s = \s+iT\) with \(\s\) bounded and \(T \ge 0\). Also suppose \(s\) is at least distance \(\e\) away from the nontrivial zeros of \(L(s)\) and the poles of the gamma factor and \(s \neq 0,1\). Invoking \cref{prop:explicit_formula_log_derivative} and using \cref{equ:digamma_gamma_factor_analytic_conductor_estimate} gives
        \[
  ...
  \section{The Explicit Formula}
    ...
    \begin{proof}
      ...
      \]
      In the range \(n \asymp X\), the Taylor expansion of the logarithm gives \(\log\left(\frac{X}{n}\right) \asymp \frac{X-n}{X}\) so that
      \[
      ...
      \]
      We will now estimate the integral \(J(X,T)\) by appealing to the residue theorem. Let \(T\) not coincide with the ordinate of a nontrivial zero and fix a positive \(U\) not coinciding with a trivial zero.
      ...
      \]
      For the integrals \todo{we estimate as follows}
