\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{The Explicit Formula}
    ...
    \begin{proof}
      ...
      \]
      for any \(c > 1+\t_{L}\) and \(\d_{X} = 0\) unless \(X\) is a prime power. We will now by bounding the above sum. In the ranges \(n \ll X\) and \(n \gg X\), we have \(\log\left(\frac{X}{n}\right) \gg 1\) so that
      \[
      ...
      \]
      Taking \(c = 1+\t_{L}+\frac{1}{\log(X)}\) implies \(X^{c} = eX^{1+\t_{L}}\) so that the the second term on the right-hand side above dominates in view of the fact \(\L_{L}(X) \ll X^{\t_{L}}\log(X)\) for the \(\d_{X}\frac{|\L_{L}(X)|}{T}\) term. Whence
      \[
        \psi^{\ast}(X,L)-J(X,T) \ll \frac{X^{1+\t_{L}}\log^{2}(X)}{T}.
      \]
      We will now estimate the integral \(J(X,T)\). \todo{xxx}
    \end{proof}
