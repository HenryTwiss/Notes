\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Analytic Data}
    ...
    \[
      \mf{q}(t,L) = \mf{q}\left(\frac{1}{2}+it,L\right) \quad \text{and} \quad \mf{q}_{\infty}(t,L) = \mf{q}_{\infty}\left(\frac{1}{2}+it,L\right).
    \]
    as well as
    \[
      \mf{q}(L) = \mf{q}\left(\frac{1}{2},L\right) \quad \text{and} \quad \mf{q}_{\infty}(L) = \mf{q}_{\infty}\left(\frac{1}{2},L\right),
    \]
  ...
  \section{The Lindel\"of Hypothesis and Estimates on the Critical Line}
    ...
    \[
      L\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}\left(t,L\right)^{\frac{1}{4}+\e}.
    \]
    ...
    \begin{conjecture*}[Lindel\"of hypothesis, \(L(s)\)]
      ...
      \[
        L\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}\left(t,L\right)^{\e}.
      \]
    \end{conjecture*}
    ...
    \[
      L\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}\left(t,L\right)^{\d+\e},
    \]
    ...
    \[
      L(s) \ll_{\e} \mf{q}\left(t,L\right)^{\frac{1}{4}+\e},
    \]
    ...
    \[
      L^{(k)}\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}\left(t,L\right)^{\frac{1}{4}+\e}.
    \]
    ...
    \]
    for all positive \(y\). This last condition means \(\left(\w\left(\frac{y}{2^{k}}\right)\right)_{k \in \Z}\) is a partition of unity on \((0,\infty)\). To see that dyadic weights exist, let \(\psi(y)\) be a smooth weight that is identically \(1\) on \([1,2]\) and supported in \([1-\d,2+\d]\). Write
    \[
    ...
    \begin{theorem}\label{thm:critical_line_estimate}
      ...
      \[
        L\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}\left(t,L\right)^{\e}\sup_{X \ll \sqrt{\mf{q}\left(t,L\right)}}\left(\frac{|A_{\w}(t,X,L)|}{\sqrt{X}}\right).
      \]
    \end{theorem}
    \begin{proof}
      ...
      \begin{align*}
        L\left(\frac{1}{2}+it\right) &= \sum_{n \ge 1}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}V_{t}\left(\frac{n}{\sqrt{q_{L}}}\right) \\
        &+ \e_{L}(t)\sum_{n \ge 1}\frac{\conj{a_{L}(n)}n^{it}}{\sqrt{n}}V_{t}\left(\frac{n}{\sqrt{q_{L}}}\right)+\frac{R\left(\frac{1}{2}+it,1,L\right)}{q_{L}^{\frac{1}{4}+i\frac{t}{2}}\g_{L}\left(\frac{1}{2}+it\right)}.
      \end{align*}
      ...
      \begin{align*}
        O\left(\sum_{n \ge 1}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}V_{t}\left(\frac{n}{\sqrt{q_{L}}}\right)\right)+O\left(\sum_{n \ge 1}\frac{\conj{a_{L}(n)}n^{it}}{\sqrt{n}}V_{-t}\left(\frac{n}{\sqrt{q_{L}}}\right)\right)+O(e^{-|t|}).
      \end{align*}
      We now turn to estimating the remaining sums which will be accomplished by applying the smooth dyadic weight. Consider the first sum. By our choice of \(\Phi(u)\), the estimates in \cref{prop:V_function_decay} are valid which implies that the summands exhibit rapid decay for \(n \gg \sqrt{\mf{q}\left(t,L\right)}\). Whence the sum is say
      \[
        \sum_{n \ll \sqrt{\mf{q}\left(t,L\right)}}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}V_{t}\left(\frac{n}{\sqrt{q_{L}}}\right)+O_{\e}\left(\mf{q}\left(t,L\right)^{\e}\right)
      \]
      Now write 
      \[
      ...
      \[
        \sum_{k \in \Z}\sum_{n \ll \sqrt{\mf{q}\left(t,L\right)}}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}\w\left(\frac{n}{2^{k}\sqrt{q_{L}}}\right)V_{t}\left(\frac{n}{\sqrt{q_{L}}}\right).
      \]
      By compact support of the smooth dyadic weight, for each \(k\) the inner sum over \(n\) is supported on the dyadic block \(n \asymp X_{k}\) where \(X_{k} = 2^{k}\sqrt{q_{L}}\). As \(n \ll \sqrt{\mf{q}\left(t,L\right)}\), the summands which contribute must satisfy \(k \ll \log\mf{q}_{\infty}\left(t,L\right)\). Therefore our double sum can be expressed as
      \[
        \sum_{k \ll \log\mf{q}_{\infty}\left(t,L\right)}\sum_{n \asymp X_{k}}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}\w\left(\frac{n}{X_{k}}\right)V_{t}\left(\frac{n}{\sqrt{q_{L}}}\right).
      \]
      Let \(V_{k}(y) = \frac{1}{\sqrt{y}}V_{t}\left(\frac{y}{\sqrt{q_{L}}}\right)\). By Abel summation, the inner sum is \(O\) of
      \[
        \sup_{n \asymp X_{k}}|A_{\w}(X_{k},t,L)|\left|V_{k}(n)\right|+\int_{u \asymp X_{k}}|A_{\w}(u,t,L)||V_{k}'(u)|\,du.
      \]
      In view of \cref{prop:V_function_decay}, we have the estimates
      \[
        V_{k}(y) = O\left(\frac{1}{\sqrt{y}}\right) \quad \text{and} \quad V'_{k}(y) = O\left(\frac{1}{y^{\frac{3}{2}}}\right),
      \]
      for \(y \ll \sqrt{\mf{q}\left(t,L\right)}\). Whence our previous expression is \(O\) of
      \[
        \sup_{X \asymp X_{k}}\left(\frac{|A_{\w}(t,X,L)|}{\sqrt{X}}\right).
      \]
      As there are \(O_{\e}\left(\mf{q}\left(t,L\right)^{\e}\right)\) many such \(k\), the bound above implies that our sum is \(O_{\e}\) of
      \[
        \mf{q}\left(t,L\right)^{\e}\sup_{X \ll \sqrt{\mf{q}\left(t,L\right)}}\left(\frac{|A_{\w}(t,X,L)|}{\sqrt{X}}\right).
      \]
    ...
    \begin{equation}\label{equ:coefficient_sum_trivial_bound}
      \sup_{X \ll \sqrt{\mf{q}\left(t,L\right)}}\frac{|A_{\w}(t,X,L)|}{\sqrt{X}} \ll_{\e} \mf{q}\left(t,L\right)^{\frac{1}{4}+\e},
    \end{equation}
    which gives the convexity bound. As we believe \(L(s)\) satisfies the Lindel\"of hypothesis, we should expect lots of cancellation in the sum \(A_{\w}(t,X,L)\). In fact, the expected cancellation should be \(O\left(\mf{q}\left(t,L\right)^{\frac{1}{4}}\right)\) in light of the aforementioned bound. This means we expect \(A_{\w}(t,X,L)\) to exhibit square-root cancellation in the sense
    \[
    ...
    \begin{theorem}\label{thm:central_value_estimate}
      Let \(L(s)\) be an analytic \(L\)-function and let \(\w(y)\) be a smooth dyadic weight. Then
      \[
        L\left(\frac{1}{2}\right) \ll_{\e} \mf{q}\left(L\right)^{\e}\max_{X \ll \sqrt{\mf{q}\left(L\right)}}\left(\frac{|A_{\w}(X,L)|}{\sqrt{X}}\right).
      \]
    \end{theorem}
    \begin{proof}
