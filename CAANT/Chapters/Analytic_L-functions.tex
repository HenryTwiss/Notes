\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Zero Density Estimates}
    ...
    \begin{proof}
      ...
      \]
      as \(s\) is bounded away from \(0\) and \(1\). Each summand is \(\D_{\eta_{1} \cup \eta_{3}}\arg(s-\rho) = O(1)\). As there are \(O(\log\mf{q}(iT,L))\) many terms by \cref{lem:powerful_L-function_approximation_lemma} (i), it follows that
      \[
  ...
  \section{Zero-free Regions}
    ...
    \end{lemma}
    \begin{proof}
      There are finitely many nontrivial real zeros \(\b\) belonging to the interval \(\frac{1}{2} \le s \le 1\). Now take \(s > 1\) and observe that the real parts of the terms \(\frac{m_{L}}{s}\) and \(\frac{1}{s-\rho}\) in \cref{lem:powerful_L-function_approximation_lemma} (ii) are positive. So upon taking the real part of this formula, we may discard all of these terms except those corresponding to the real nontrivial zeros. This result in
      \[
        \sum_{\b}\Re\left(\frac{1}{\s-\b}\right) = \frac{m_{L}}{\s-1}+\Re\left(\frac{L'}{L}(\s)\right)+O(\log{\mf{q}(L)}),
      \]
      where the left-hand side is positive. To estimate \(\Re\left(\frac{L'}{L}(\s)\right)\), recall that \(\Re(\L_{L}(n)) \ge 0\) provided \((n,q_{L}) = 1\) by assumption. Whence
      \[
        \Re\left(\frac{L^{(q_{L})'}}{L^{(q_{L})}}(\s)\right) \le 0.
      \]
      This provides an estimate for the contribution of the Euler factors of \(L(s)\) corresponding to unramified primes. For the contribution corresponding to ramified primes, a short computation with an application of the bound \(|\a_{j}(p)| \le \frac{p}{2}\) shows
      \[
        \Re\left(\frac{L_{(q_{L})}'}{L_{(q_{L})}}(\s)\right) \le d_{L}\log(q_{L}).
      \]
      These estimates together imply
      \[
        \sum_{\b}\Re\left(\frac{1}{\s-\b}\right) < \frac{m_{L}}{\s-1}+O(\log(d_{L}\mf{q}(L))).
      \]
      From this inequality we see that \(\b \neq 1\). For if some \(\b = 1\), we have \(m_{L} < 0\) whence the right-hand side is negative for \(\s\) sufficiently close to \(1\) contradicting positivity of the left-hand side. Thus \(L(1) \neq 0\) and hence \(m_{L} \ge 0\). As there are finitely many \(\b\), there exists a positive constant \(c\) such that the \(\b\) satisfy
      \[
        \b \ge 1-\frac{c}{d_{L}(m_{L}+1)\log{\mf{q}(L)}}.
      \]
      Setting \(\s = 1+\frac{2c}{d_{L}\log{\mf{q}(L)}}\) \iffalse and choosing \(c\) smaller, if necessary, we guarantee \(1 < \s \le 2\). Then the two inequalities above together imply
      \[
      ...
      \[
        \s \ge 1-\frac{c}{d_{L}(m_{L}+1)\log{\mf{q}(L)}}.
      \]
      This completes the proof.\fi
    \end{proof}

    \iffalse We can now prove our zero-free region result:
