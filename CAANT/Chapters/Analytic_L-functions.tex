\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{theorem*}[Approximate functional equation]
      Suppose \(L(s)\) is an analytic \(L\)-function, \(\Phi(u)\) is an even holomorphic function bounded in the vertical strip \(|\tau| < c+1\) for some \(c > 1\) and such that \(\Phi(0) = 1\), and let \(X > 0\). Then for \(s\) in the critical strip, we have
      \[
  ...
  \section{The Explicit Formula}
    ...
    \begin{theorem*}[Explict Formula]
      For any analytic \(L\)-function \(L(s)\) and \(X > 0\), we have
      \[
    ...
    \begin{proof}
      ...
      \]
      for all nontrivial zeros \(\rho\) as they occur in pairs \(\rho\) and \(1-\conj{\rho}\). Also, fix a positive \(U\) such that \(-U\) bounded away from the trivial zeros. Let \(\eta = \sum_{i}\eta_{i}\) be the rectangular contour in \cref{fig:explict_formula_contour} where the horizontal lines are along \(t = \pm T\) and the vertical lines are along \(\s = 1+c\) and \(\s = -U\).
      ...
      \]
      For the integrals along \(\eta_{2}\) and \(\eta_{4}\), we require distinct estimates when \(-c \le \s \le 1+c\) and \(\s < -c\). For the former, \cref{lem:powerful_L-function_approximation_lemma} (ii) gives
      \[
        \frac{L'}{L}(s) = \sum_{|\g\mp T| < 1}\frac{1}{s-\rho}+O(\log\mf{q}(T,L)),
      \]
      for \(s = \s\pm iT\) with \(-c \le \s \le 1+c\). By our choice of \(T\), \(|s-\rho| \gg \frac{1}{\log\mf{q}(T,L)}\) so that each term in the sum is \(O(\log\mf{q}(T,L))\). There are at most \(O(\log\mf{q}(T,L))\) such terms by \cref{lem:powerful_L-function_approximation_lemma} (i) and so
      \[
      ...
      \[
        -\frac{L'}{L}(s) = \log(q_{L})+\frac{\g'}{\g}(s,L)+\frac{\g'}{\g}(1-s,L)+\frac{\conj{L}'}{\conj{L}}(1-s).
      \]
      Let \(s\) be such that \(\s < -c\) and suppose \(s\) bounded away from the trivial zeros. Using \cref{equ:digamma_gamma_factor_analytic_conductor_estimate} and that the logarithmic derivative on the right-hand side converges absolutely, we obtain a bound
      \[
        \frac{L'}{L}(s) = O(\log\mf{q}(s,L)),
      \]
      in this region. The previous two estimates imply
      \[
        \frac{1}{2\pi i}\int_{\eta_{2} \cup \eta_{4}}-\frac{L'}{L}(s)X^{s}\,\frac{ds}{s} \ll \frac{\log^{2}\mf{q}(T,L)}{T}\int_{-c}^{1+c}X^{\s}\,d\s+\frac{\log\mf{q}(T,L)}{T}\int_{-U}^{-c}X^{\s}\,d\s,
      \]
      whence
      \[
        \frac{1}{2\pi i}\int_{\eta_{2} \cup \eta_{4}}-\frac{L'}{L}(s)X^{s}\,\frac{ds}{s} \ll \frac{X^{1+c}\log^{2}\mf{q}(T,L)}{T\log(X)}+\frac{\log\mf{q}(T,L)}{TX^{c}\log(X)}.
      \]
      As for the vertical integral along \(\eta_{3}\), the previous estimate is logarithmic derivative of \(L(s)\) is also valid and we obtain
      \[
        \todo{xxx}
      \]
      ...
      We now set \(c = 1+\t_{L}+\frac{1}{\log(X)}\) so that \(X^{c} = eX^{1+\t_{L}}\) and then take the limit as \(T \to \infty\). It follows that
      \[
        \psi^{\ast}(X,L) = m_{L}X-\sum_{\rho }\frac{X^{\rho}}{\rho}+\sum_{j}\sum_{n \ge 0}\frac{X^{-(\mu_{j}+2n)}}{\mu_{j}+2n}-\Res_{s = 0}\left(\frac{X^{s}}{s}\frac{L'}{L}(s)\right),
      \]
      which proves the explicit formula.
    \end{proof}