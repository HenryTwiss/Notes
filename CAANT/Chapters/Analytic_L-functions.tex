\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Zero Density Estimates}
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label*=(\roman*)]
        \item Let \(s = \s+iT\) for some \(\s > 1+\t\) so that the logarithmic derivative of \(L(s)\) is absolutely bounded. Taking the real part of the formula in \cref{prop:explicit_formula_log_derivative} and using \cref{equ:digamma_gamma_factor_analytic_conductor_estimate,equ:B_L_real_part} gives
        \[
