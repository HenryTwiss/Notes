\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \]
  for the Pochhammer symbol. Also, all sums and products over zeros of a complex function will be counted with multiplicity and ordered with respect to the size of the ordinate.
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{proof}
      ...
      \]    
      Stirling's formula shows that \(\g(s+u,L)\) exhibits exponential decay while \(L(s+u)\) has at most polynomial growth by \cref{prop:L_function_bounded_in_vertical_strips}. Since \(\Phi(u)\) is bounded in the vertical strip \(|\tau| < a+1\), it follows that the integrand exhibits exponential decay. Therefore the integral is locally absolutely uniformly bounded. We will evaluate the integral in two ways. On the one hand, we can expand \(L(s+u)\) inside the integrand as a Dirichlet series and by absolute boundedness of the integral we may interchange the sum and integral. A short computation shows that this is
      \[
  ...
  \section{Logarithmic Derivatives}
    ...
    \end{equation}
    which we refer to as the \textbf{functional equation}\index{functional equation} of \(\xi(s,L)\). Observe that \(\xi(s,L)\) is just \(\L(s,L)\) with the potential poles at \(s = 0\) and \(s = 1\) removed. This means \(\xi(s,L)\) is entire. In fact, it admits a Hadamard factorization. To see this, first observe that \(\xi(s,L)\) is of order \(1\) by Stirling's formula and that \((s-1)^{m_{L}}L(s)\) is of order \(1\). Then the Hadamard factorization theorem implies
    \[
  ...
  \section{Zero Density}
    ...
    \begin{lemma}\label{lem:powerful_L-function_approximation_lemma}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \[
          \sum_{\rho \neq 0,1}\frac{1}{1+(T-\g)^{2}} \ll \log{\mf{q}(iT,L)}.
        \]
        ...
        \item We may write
        \begin{align*}
          -\Re\left(\frac{L'}{L}(s)\right) &= \Re\left(\frac{m_{L}}{s}\right)+\Re\left(\frac{m_{L}}{s-1}\right) \\
          &- \sum_{|s+\mu_{j}| < 1}\Re\left(\frac{1}{s+\mu_{j}}\right)-\sum_{\substack{|s-\rho| < 1 \\ \rho \neq 0,1}}\Re\left(\frac{1}{s-\rho}\right)+O(\log{\mf{q}(s,L)}),
        \end{align*}
        for any \(s\) in the vertical strip \(-\frac{1}{2} \le \s \le 2\) provided \(s \neq 0,1\). In addition, we may discard any term in either sum if \(1 < \s < \le 2\).
      \end{enumerate}
    \end{lemma}
    \begin{proof}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \[
          \sum_{\rho \neq 0,1}\Re\left(\frac{1}{s-\rho}\right) \ll \log{\mf{q}(iT,L)}.
        \]
        ...
        \item These estimates follow from direct computation since any nontrivial zero \(\rho\) satisfies \(\b \le 1\) while any gamma parameter \(\mu_{j}\) satisfies \(\Re(\mu_{j}) \ge 0\).
        \item Taking the real part of the formula in \cref{prop:explicit_formula_log_derivative} and using (i) gives
        \begin{align*}
          -\Re\left(\frac{L'}{L}(s)\right) &= \Re\left(\frac{m_{L}}{s}\right)+\Re\left(\frac{m_{L}}{s-1}\right) \\
          &+ \Re\left(\frac{\g'}{\g}(s,L)\right)-\sum_{\rho \neq 0,1}\frac{1}{s-\rho}+O(\log{q_{L}}).
        \end{align*}
        \iffalse Applying \(\frac{\G'}{\G}(s+1) = \frac{\G'}{\G}(s)+\frac{1}{s}\) to \(\frac{\g'}{\g}(s,L)\) and using \cref{equ:digamma_gamma_factor_analytic_conductor_estimate} gives
        \[
          \frac{\g'}{\g}(s,L) = -\sum_{|s+\mu_{j}| < 1}\frac{1}{s+\mu_{j}}+O(\log{\mf{q}_{\infty}(s,L)}).
        \]
        Then we obtain
        \[
          -\frac{L'}{L}(s) = \frac{m_{L}}{s}+\frac{m_{L}}{s-1}-\sum_{|s+\mu_{j}| < 1}\frac{1}{s+\mu_{j}}-\sum_{\substack{|s-\rho| < 1 \\ \rho \neq 0,1}}\frac{1}{s-\rho}+O(\log{\mf{q}(s,L)}),
        \]
        which is the first statement of (iv). For second statement, tak the real part of this estimate and write it as
        \[
          \sum_{|s+\mu_{j}| < 1}\Re\left(\frac{1}{s+\mu_{j}}\right)+\sum_{\substack{|s-\rho| < 1 \\ \rho \neq 0,1}}\Re\left(\frac{1}{s-\rho}\right) \le \Re\left(-\frac{L'}{L}(s)\right)+\Re\left(\frac{m_{L}}{s}\right)+\Re\left(\frac{m_{L}}{s-1}\right)+O(\log{\mf{q}(s,L)}).
        \]
        Now observe that we can discard any of the terms in either sum provided \(1 < \s \le 2\) by (iii).\fi
      \end{enumerate}
    \end{proof}

    \iffalse With \cref{lem:powerful_L-function_approximation_lemma} in hand, we can deduce a result which estimates the number of nontrivial zeros in a box. Accordingly, for any \(T \ge 0\) we define
    \[
    ...
    \begin{theorem}\label{thm:zero_counting}
      ...
      \[
        N(T,f) = \frac{T}{\pi}\log\left(\frac{QT^{d_{L}}}{(2\pi e)^{d_{L}}}\right)+O(\log{\mf{q}(iT,L)}).
      \]
      In particular,
      \[
    ...
    \begin{proof}
      ...
      \[
        \xi(s,L) = (s(1-s))^{m_{L}}Q^{\frac{s}{2}}\pi^{-\frac{d_{L}s}{2}}\prod_{j}\G\left(\frac{s+\mu_{j}}{2}\right)L(s).
      \]
      ...
      and therefore
      \[
        N'(T,f) = \frac{T}{2\pi}\log\left(\frac{T^{d_{L}}}{(2\pi e)^{d_{L}}}\right)+O(\log{\mf{q}(iT,L)}),
      \]
