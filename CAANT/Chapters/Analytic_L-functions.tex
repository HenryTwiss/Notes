\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Estimates on the Critical Line}
    ...
    \]
    which is to say we may take \(\d = 0\). As the coefficients \(a_{L}(n)\) may very well be real, the expected cancellation must come from the terms \(n^{-it} = e^{-it\log(n)}\) which act on the coefficients \(a_{L}(n)\) by rotation. In effect, we should think of the terms \(a_{L}(n)n^{-it}\) as modeling identically distributed random variables. In order to exploit this cancellation via analytic methods, one can make use of smoothed Perron's formula. Letting \(\W(s)\) be the Mellin transform of \(\w(y)\), smooth Perron's formula gives
    \[
