\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \]
  for the Pochhammer symbol. We will make use of the gamma function developed in \cref{sec:the_gamma_function} especially Stirling's formula as well as the Mellin transform and the corresponding inversion theorem developed in \cref{sec:The_Mellin_transform}. Also, all sums and products over zeros of a function will be counted with multiplicity and ordered symmetrically with respect to the size of the ordinate if they are not absolutely convergent.
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{theorem*}[Approximate functional equation]
      Suppose \(L(s)\) is an analytic \(L\)-function, \(\Phi(u)\) is an even holomorphic function bounded in the vertical strip \(|\tau| < c+1\) for some \(c > 1\) and such that \(\Phi(0) = 1\), and let \(X \ge 1\). Then for \(s\) in the critical strip, we have
      \[
    ...
    \[
      \psi(y,L) = \frac{1}{2\pi i}\int_{(c)}q_{L}^{s}\frac{\g(s,L)}{\g(1-s,L)}y^{-s}\Psi(1-s)\,ds,
    \]
    defined for \(y > 0\) where \(c > 1\). By our choice of \(\psi(y)\), its inverse Mellin transform \(\Psi(s)\) has rapid decay via \cref{prop:smoothing_function_Mellin_inverse_vertical_strips}. Since \(L(s)\) has at most polynomial growth by \cref{prop:L_function_bounded_in_vertical_strips}, the integrand has rapid decay as well. Therefore the integral is locally absolutely uniformly convergent and hence \(\psi(y,L)\) is smooth for \(y > 0\). We will also set
    \[
      R_{L} = \Res_{s = 1}L(s).
    \]
    ...
    \begin{proof}
      ...
      \[
        \sum_{n \ge 1}a_{L}(n)\psi(n) = \frac{1}{2\pi i}\int_{(c)}L(s)\Psi(s)\,ds,
      \]
      provided \(c > 1\). By our choice of \(\psi(y)\), its inverse Mellin transform \(\Psi(s)\) has rapid decay via \cref{prop:smoothing_function_Mellin_inverse_vertical_strips}. Since \(L(s)\) has at most polynomial growth by \cref{prop:L_function_bounded_in_vertical_strips}, the integrand has rapid decay as well. Therefore the integral is locally absolutely uniformly convergent which permits us to shift the line of integration. Shifting to \((1-c)\), we pass by a potential pole at \(s = 1\) and obtain
      \[
        \sum_{n \ge 1}a_{L}(n)\psi(n) = \frac{1}{2\pi i}\int_{(1-c)}L(s)\Psi(s)\,ds+R_{L}\Psi(1).
      \]
      Apply the functional equation to rewrite this equality in the form
      \[
        \sum_{n \ge 1}a_{L}(n)\psi(n) = \frac{1}{2\pi i}\int_{(1-c)}\e_{L}q_{L}^{\frac{1}{2}-s}\frac{\g(1-s,L)}{\g(s,L)}\conj{L}(1-s)\Psi(s)\,ds+R_{L}\Psi(1).
      \]
      Performing the change of variables \(s \mapsto 1-s\) in this latter integral gives
      \[
        \sum_{n \ge 1}a_{L}(n)\psi(n) = \frac{1}{2\pi i}\int_{(c)}\e_{L}q_{L}^{s-\frac{1}{2}}\frac{\g(s,L)}{\g(1-s,L)}\conj{L}(s)\Psi(1-s)\,ds+R_{L}\Psi(1).
      \]
      The proof is complete upon expanding \(\conj{L}(s)\) as a Dirichlet series, interchanging the sum and integral by absolute boundedness of the integral, and factoring out \(\frac{\e_{L}}{\sqrt{q_{L}}}\).
    \end{proof}
  \section{The Riemann Hypothesis and Nontrivial Zeros}
    The zeros of an analytic \(L\)-functions \(L(s)\) has interesting behavior which are classified into two categories: trivial and nontrivial zeros. To describe them, recall that the completion
    \[
  ...
  \section{The Lindel\"of Hypothesis and Moments}
  ...
  \section{Zero Density Estimates}
    ...
    \begin{lemma}\label{lem:powerful_L-function_approximation_lemma}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \]
        In particular, the number of nontrivial zeros of \(L(s)\) such that \(|T-\g| \le 1\) is \(O(\log{\mf{q}(T,L)})\).
    ...
    \begin{proof}
      ...
      Let \(\eta = \sum_{i}\eta_{i}\) be the rectangular contour in \cref{fig:zero_counting_contour} where the horizontal lines are along \(t = T\) and \(t = -\d\) and the vertical lines are along \(\s = 1+\t_{L}+\d\) and \(\s = -(\t_{L}+\d)\). Then by our previous comments and the argument principle, we may write
      \[
  ...
  \section{The Explicit Formula}
    There is a formula, somewhat analogous to the approximate functional equation, that relates a weighted sum of the von Mangoldt coefficients of any analytic \(L\)-function \(L(s)\) to a sum over its nontrivial zeros.

    \begin{theorem*}
      Let \(\psi(y)\) be a smooth weight where \(\Psi(s)\) is its Mellin transform and set \(\phi(y) = y^{-1}\psi(y^{-1})\) so that its Mellin transform \(\Phi(s)\) satisfies \(\Phi(s) = \Psi(1-s)\). Then for any analytic \(L\)-function \(L(s)\), we have
      \begin{align*}
    ...
    There is a much more useful variant which is obtained by applying truncated Perron's formula and performing various estimates. For this, we need to introduce two related functions. For any analytic \(L\)-function \(L(s)\), we define \(\psi(x,L)\) by
    \[
      \psi(X,L) = \sum_{n \le X}\L_{L}(n),
    \]
    for \(X > 0\). In other words, \(\psi(X,L)\) is just the sum of the von Mangoldt coefficients of \(L(s)\) up to \(X\). It turns out that many useful applications of analytic \(L\)-functions can be derived from sufficiently nice estimates for \(\psi(X,L)\). This is primarily due to the fact that \(\psi(X,L)\) is closely related a sum over the nontrivial zeros of \(L(s)\) which is strongly suggested by the previous result. Since we will be taking inverse Mellin transforms, it will be useful to work with a slightly modified version of \(\psi(X,L)\). Accordingly, we define Define \(\psi^{\ast}(x,L)\) by
    \[
    ...
    \begin{theorem*}[Explict Formula]
      For any analytic \(L\)-function \(L(s)\) and \(T \ge X > 1\), we have
      \begin{align*}
    ...
    \begin{proof}
      ...
      \begin{align*}
        \frac{1}{2\pi i}\int_{\eta}-\frac{L'}{L}(s)X^{s}\,\frac{ds}{s} &= m_{L}X-\sum_{\substack{\rho \\ |\g| < T}}\frac{X^{\rho}}{\rho} \\
        &+ \sum_{j}\sum_{0 \le n < \frac{U-\Re(\mu_{j})}{2}}\frac{X^{-(\mu_{j}+2n)}}{\mu_{j}+2n}-\Res_{s = 0}\left(\frac{X^{s}}{s}\frac{L'}{L}(s)\right).
      \end{align*}
      ...
      \begin{align*}
        J(X,T) &= m_{L}X-\sum_{\substack{\rho \\ |\g| < T}}\frac{X^{\rho}}{\rho}+\sum_{j}\sum_{n \ge 0}\frac{X^{-(\mu_{j}+2n)}}{\mu_{j}+2n}-\Res_{s = 0}\left(\frac{X^{s}}{s}\frac{L'}{L}(s)\right) \\
        &+ O\left(\frac{X^{c}\log^{2}\mf{q}(T,L)}{T\log(X)}\right),
      \end{align*}
      ...
      \begin{align*}
        \psi^{\ast}(X,L) &= m_{L}X-\sum_{\substack{\rho \\ |\g| < T}}\frac{X^{\rho}}{\rho}+\sum_{j}\sum_{n \ge 0}\frac{X^{-(\mu_{j}+2n)}}{\mu_{j}+2n}-\Res_{s = 0}\left(\frac{X^{s}}{s}\frac{L'}{L}(s)\right) \\
        &+ O\left(\frac{X^{c}}{T}+\frac{X^{1+\t_{L}}\log^{2}(X)}{T}+\frac{X^{c}\log^{2}\mf{q}(T,L)}{T\log(X)}\right).
      \end{align*}
      We now set \(c = 1+\t_{L}+\frac{1}{\log(X)}\) so that \(X^{c} = eX^{1+\t_{L}}\) and we may write
      \begin{align*}
        \psi^{\ast}(X,L) &= m_{L}X-\sum_{\substack{\rho \\ |\g| < T}}\frac{X^{\rho}}{\rho}+\sum_{j}\sum_{n \ge 0}\frac{X^{-(\mu_{j}+2n)}}{\mu_{j}+2n}-\Res_{s = 0}\left(\frac{X^{s}}{s}\frac{L'}{L}(s)\right) \\
        &+ O\left(\frac{X^{c}\log^{2}\mf{q}(T,L)}{T}\right).
      \end{align*}
