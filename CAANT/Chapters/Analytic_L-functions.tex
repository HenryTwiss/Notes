\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Zero Density}
    ...
    \]
    In other words, \(N(T,L)\) is the number of nontrivial zeros of \(L(s)\) with ordinate in \([-T,T]\). We will prove an asymptotic formula for \(N(T,L)\).
    ...
    \begin{proof}
      Let \(T \ge 1\) and set
      \[
        N'(T,f) = \left|\{\rho = \b+i\g \in \C:L(\rho) = 0 \text{ with } 0 \le \b \le 1 \text{ and } 0 < \g \le T\}\right|.
      \]
      ...
      \[
        N(T,L) = N'(T,f)+N_{L}'(T)+O(\log{\mf{q}(f)}),
      \]
