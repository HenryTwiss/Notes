\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{The Approximate Functional Equation}
    ...
    \]
    This term appears as a factor in the dual sum accounting for the functional equation data. We now prove the \textbf{approximate function equation}\index{approximate function equation}:
  ...
  \section{The Lindel\"of Hypothesis and Estimates on the Critical Line}
    ...
    \begin{theorem}\label{thm:central_value_estimate}
      Let \(L(s)\) be an analytic \(L\)-function and let \(\w(y)\) be a smooth dyadic weight. Then
      \[
        L\left(\frac{1}{2}\right) \ll_{\e} \mf{q}\left(\frac{1}{2},L\right)^{\e}\max_{X \ll \sqrt{\mf{q}\left(\frac{1}{2},L\right)}}\left(\frac{|A_{\w}(0,X,L)|}{\sqrt{X}}\right).
      \]
    \end{theorem}
    \begin{proof}
  ...
  \section{Logarithmic Derivatives}
    ...
    \]
    In fact, \(\xi(s,L)\) is just \(\L(s,L)\) with the potential poles at \(s = 0\) and \(s = 1\) removed. This means \(\xi(s,L)\) is entire. In fact, it admits a Hadamard factorization. To see this, first observe that \(\xi(s,L)\) is of order \(1\) by Stirling's formula and that \((s-1)^{r_{L}}L(s)\) is of order \(1\). Then the Hadamard factorization theorem implies
    \[
    ...
    \begin{proposition}\label{prop:explicit_formula_log_derivative}
      ...
      \[
        -\frac{L'}{L}(s) = \frac{r_{L}}{s}+\frac{r_{L}}{s-1}+\frac{1}{2}\log{q_{L}}+\frac{\g'}{\g}(s,L)-B_{L}-\sum_{\rho \neq 0,1}\left(\frac{1}{s-\rho}+\frac{1}{\rho}\right).
      \] 
    \end{proposition}
    \begin{proof}
      ...
      \[
        \frac{\xi'}{\xi}(s,L) = \frac{r_{L}}{s}+\frac{r_{L}}{s-1}+\frac{1}{2}\log{q_{L}}+\frac{\g'}{\g}(s,L)+\frac{L'}{L}(s).
      \]
      ...
      \[
        B_{L}+\sum_{\rho \neq 0,1}\left(\frac{1}{s-\rho}+\frac{1}{\rho}\right) = \frac{r_{L}}{s}+\frac{r_{L}}{s-1}+\frac{1}{2}\log{q_{L}}+\frac{\g'}{\g}(s,L)+\frac{L'}{L}(s).
      \]
  ...
  \iffalse\section{Zero Density}
    ...
    \begin{lemma}\label{lem:powerful_L-function_approximation_lemma}
      Let \(L(s)\) be an \(L\)-function. The following properties hold:
      \begin{enumerate}[label*=(\roman*)]
        \item The constant \(B_{L}\) satisfies
        \[
          \Re(B_{L}) = -\sum_{\rho \neq 0,1}\Re\left(\frac{1}{\rho}\right).
        \]
        \item For any nonnegative \(T\), the number of nontrivial zeros \(\rho = \b+i\g\) with \(\rho \neq 0,1\) and such that \(|T-\g| \le 1\) is \(O(\log{\mf{q}(iT,L)})\).
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label*=(\roman*)]
        \item Taking the logarithmic derivative of the Hadamard factorization of \(\xi(s,L)\) gives
        \[
          B_{L} = \frac{\xi'}{\xi}(s,L)-\sum_{\rho \neq 0,1}\left(\frac{1}{s-\rho}+\frac{1}{\rho}\right).
        \]
        Now let \(s > 1\). Then \(s\) is bounded away from all of the nontrivial zeros and it follows that \(\frac{1}{(s-\rho)}+\frac{1}{(1-s-\conj{\rho})} \ll \frac{1}{\rho^{2}}\) and \(\frac{1}{\rho}+\frac{1}{\conj{\rho}} \ll \frac{1}{\rho^{2}}\). Therefore the sums
        \[
        ...
        \begin{equation}\label{equ:log_conductor_gamma_factor_bound_analytic_conductor}
          \frac{1}{2}\log{q_{L}}+\frac{\g'}{\g}(s,L) \ll \log{\mf{q}(s,L)},
        \end{equation}
        ...
        \[
          -\frac{L'}{L}(s) = \frac{r_{L}}{s}+\frac{r_{L}}{s-1}+\frac{\g'}{\g}(s,L)-\sum_{\substack{|s-\rho| < 1 \\ \rho \neq 0,1}}\frac{1}{s-\rho}+O(\log{\mf{q}(s,L)}).
        \]
        Applying \(\frac{\G'}{\G}(s+1) = \frac{\G'}{\G}(s)+\frac{1}{s}\) to \(\frac{\g'}{\g}(s,L)\) and using \cref{equ:digamma_gamma_factor_analytic_conductor_estimate} gives
        \[
