\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Zero Density Estimates}
    ...
    \begin{lemma}\label{lem:powerful_L-function_approximation_lemma}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \[
          -\frac{L'}{L}(s) = -\sum_{\substack{\rho \neq 0,1 \\ |T-\g| \le 1}}\frac{1}{s-\rho}+O_{\e}(\log{\mf{q}(s,L)}),
        \]
        provided \(s\) is at least distance \(\e\) away from \(s = 0\), \(s = 1\), the nontrivial zeros of \(L(s)\), and the poles of the gamma factor.
      \end{enumerate}
    \end{lemma}
    \begin{proof}
      We will prove the properties separately.
      \begin{enumerate}[label*=(\roman*)]
        \item Let \(s = \s+iT\) for some \(\s > 1+\t\) so that the logarithmic derivative of \(L(s)\) is absolutely bounded. Taking the real part of the formula in \cref{prop:explicit_formula_log_derivative} and using \cref{equ:digamma_gamma_factor_analytic_conductor_estimate} along with (i) gives
        \[
        ...
        \item Let \(s = \s+iT\) with \(\s\) bounded and nonnegative \(T\). Also suppose \(s\) is at least distance \(\e\) away from \(s = 0\), \(s = 1\), the nontrivial zeros of \(L(s)\), and the poles of the gamma factor. Invoking \cref{prop:explicit_formula_log_derivative} and using \cref{equ:digamma_gamma_factor_analytic_conductor_estimate} gives
        \[
          -\frac{L'}{L}(s) = -\sum_{\rho \neq 0,1}\left(\frac{1}{s-\rho}+\frac{1}{\rho}\right)+O_{\e}(\log{\mf{q}(iT,L)}).
        \]
        ...
        \]
        The first \(O\)-estimate on the right-hand side is \(O(1)\) by \cref{Hadamard_zero_sum_for_xi}. As we are restricting to nontrivial zeros such that \(|T-\g| > 1\) in the second sum, it is \(O(\log{\mf{q}(s,L)})\) by (i). Also, (i) implies
        \[
    ...
    \begin{proof}
      ...
      Let \(\eta = \sum_{i}\eta_{i}\) be the rectangular contour \cref{fig:zero_counting_contour} where the horizontal lines are along \(t = T\) and \(t = -\d\) and the vertical lines are along \(\s = -(\t+\d)\) and \(\s = 1+\t+\d\). Then by our previous comments and the argument principle, we may write
      \[
      ...
      \]
      respectively
      For the product of gamma factors, first observe that the logarithm of Stirling's formula (see \cref{sec:the_gamma_function}) gives
      \[
      ...
      \]
      and it follows that the contribution for the product of gamma factors is
      \[
        \D_{\eta_{R}}\arg\left(\prod_{j}\G\left(\frac{s+\mu_{j}}{2}\right)\right) = \frac{T}{2}\log\left(\frac{T^{d_{L}}}{(2e)^{d_{L}}}\right)+O(1).
      \]
      For the contribution from \(L(s)\), write
      \[
        \D_{\eta_{R}}\arg(L(s)) = \Im\left(\int_{\eta_{R}}\frac{L'}{L}(s)\,ds\right).
      \]
      As the logarithmic derivative of \(L(s)\) is absolutely bounded for \(\s = 1+\t+\d\), the integral is \(O(1)\) on \(\eta_{2}\). On \(\eta_{1}\) and \(\eta_{3}\), use \cref{lem:powerful_L-function_approximation_lemma} (ii) to write
      \[
      ...
      \]
      Each summand is \(\D_{\eta_{1} \cup \eta_{3}}\arg(s-\rho) = O(1)\). As there are \(O(\log\mf{q}(iT,L))\) many terms by \cref{lem:powerful_L-function_approximation_lemma} (i), it follows that
      \[
      ...
      \]
      and the asymptotic formula follows. The asymptotic equivalence follows immediately since it is a strictly weaker asymptotic.
    \end{proof}
    ...
    \]
    In other words, \(N_{\text{unf}}(T,f)\) is the number of unfolded nontrivial zeros of \(L(s)\) whose ordinate belongs to \([-W,W]\). The point of considering unfolded nontrivial zeros is that they are uniformly dense along the critical strip.
  ...
  \iffalse\section{Zero-free Regions}
    Although the Riemann hypothesis for any analytic \(L\)-function remains out of reach, some progress has been made to understand regions inside of the critical strip for which there are no nontrivial zeros except for possibly one real exception. Such a region is said to be a \textbf{zero-free region}\index{zero-free region}. We will derive a standard zero-free region for any analytic \(L\)-function under some mild assumptions. First we require a lemma.

    \begin{lemma}\label{lem:non-vanshing_at_1_lemma}
      Let \(L(s)\) be an analytic \(L\)-function such that \(\Re(\L_{L}(n)) \ge 0\) provided \((n,q_{L}) = 1\). Also suppose \(|\a_{j}(p)| \le \frac{p}{2}\) for ramified primes \(p\). Then \(L(1) \neq 0\) whence \(m_{L} \ge 0\). Moreover, there exists a constant \(c > 0\) such that \(L(s)\) has at most \(m_{L}\) real zeros in the region
      \[
        \s \ge 1-\frac{c}{d_{L}(m_{L}+1)\log{\mf{q}(L)}}.
      \]
    \end{lemma}
    \begin{proof}
      There are finitely many nontrivial real zeros \(\b\) belonging to the interval \(\frac{1}{2} \le s \le 1\). Fix \(\s > 1\). Applying \cref{lem:powerful_L-function_approximation_lemma} (iv) while discarding all the terms in the sum except those corresponding to these real zeros, we obtain the inequality
      \[
        \sum_{\b}\Re\left(\frac{1}{\s-\b}\right) = \Re\left(\frac{L'}{L}(\s,f)\right)+O(\log{\mf{q}(L)}).
      \]
      To estimate \(\Re\left(\frac{L'}{L}(\s,f)\right)\), we first note that as \(\Re(\L_{L}(n)) \ge 0\) provided \((n,q_{L}) = 1\) by assumption, the Dirichlet series of \(\frac{L'^{(q_{L})}}{L^{(q_{L})}}(s)\) shows that
      \[
        \Re\left(\frac{L^{(q_{L})'}}{L^{(q_{L})}}(\s,f)\right) \le 0.
      \]
      ...
      \[
        \Re\left(\frac{L'_{q_{L}}}{L_{q_{L}}}(\s,f)\right) \le \left|\frac{L'_{q_{L}}}{L_{q_{L}}}(\s,f)\right| = \left|\sum_{p \mid q_{L}}\sum_{j}\frac{\a_{j}(p)\log(p)}{(1-\a_{j}(p)p^{-\s})p^{\s}}\right| \le d_{L}\sum_{p \mid q_{L}}\log(p) \le d_{L}\log(q_{L}),
      \]
      ...
      \[
        \sum_{j}\frac{1}{\s-\b_{j}} < \frac{m_{L}}{\s-1}+O(\log{d_{L}\mf{q}(L)}).
      \]
      From this inequality we see that \(\b_{j} \neq 1\). For if some \(\b_{j} = 1\), \(m_{L} < 0\) and the right-hand side is negative for \(\s\) sufficiently close to \(1\) contradicting the positivity of the left-hand side. Thus \(L(1) \neq 0\) and hence \(m_{L} \ge 0\). As there are finitely many \(\b_{j}\), there exists a \(c > 0\) such that the \(\b_{j}\) satisfy
      \[
        \b_{j} \ge 1-\frac{c}{d_{L}(m_{L}+1)\log{\mf{q}(L)}}.
      \]
      Setting \(\s = 1+\frac{2c}{d_{L}\log{\mf{q}(L)}}\) and choosing \(c\) smaller, if necessary, we guarantee \(1 < \s \le 2\). Then the two inequalities above together imply
      \[
        \frac{nd_{L}\log{\mf{q}(L)}}{2c+\frac{c}{(m_{L}+1)}} < \left(\frac{m_{L}}{2c}+O(1)\right)d_{L}\log{\mf{q}(L)}.
      \]
    ...
    \begin{theorem}\label{thm:zero_free_region_generic}
      Let \(L(s)\) be an analytic \(L\)-function with at most a simple pole at \(s = 1\), \(\Re(\L_{L}(n)) \ge 0\) provided \((n,q_{L}) = 1\), and \(|\a_{j}(p)| \le \frac{p}{2}\) for ramified primes \(p\). Then there exists a constant \(c > 0\) such that \(L(s)\) has no zeros in the region
      \[
        \s \ge 1-\frac{c}{d_{L}^{2}\log(\mf{q}(L)(|t|+3))},
      \]
    ...
    \begin{proof}
      ...
      \[
        \mf{q}(g) \le \mf{q}(L)^{6}\mf{q}(it,f)^{8}\mf{q}(2it,f)^{2} \le \mf{q}(L)^{16d_{L}}(|t|+3)^{10d_{L}} < (\mf{q}(L)(|t|+3))^{16d_{L}}.
      \]
      We claim that \(\Re(\L_{g}(n)) \ge 0\) for \((n,q_{L}) = 1\). To see this, let \(p\) be an unramified prime. The local parameters of \(L(s,g)\) at \(p\) are \(\a_{j}(p)\) and \(\conj{\a_{j}(p)}\) both with multiplicity three, \(\a_{j}(p)p^{-it}\) and \(\conj{\a_{j}(p)}p^{-it}\) both with multiplicity four, and \(\a_{j}(p)p^{-2it}\) and \(\conj{\a_{j}(p)}p^{-2it}\) both with multiplicity one. So for any \(k \ge 1\), the sum of \(k\)-th powers of these local parameters is
      \[
    ...
    \[
      1-\frac{c}{d_{L}^{2}\log(3\mf{q}(L))} \le \b_{f} < 1
    \]
  ...
  \fi\iffalse\section{The Explicit Formula}
    ...
    \begin{proof}
      ...
      \]
      For the latter integral, the functional equation and that \(q(\conj{f}) = q_{L}\) together imply
      \[
