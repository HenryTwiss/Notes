\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{theorem*}[Approximate functional equation]
      Let \(L(s)\) be an analytic \(L\)-function, let \(\Phi(u)\) be an even holomorphic function bounded in the vertical strip \(|\tau| < c+1\) for some \(c > 1\) and such that \(\Phi(0) = 1\), and let \(X \ge 1\). Then for \(s\) in the critical strip, we have
      \[
    ...
    \begin{proposition}\label{prop:V_function_decay}
      Let \(L(s)\) be an analytic \(L\)-function, set \(\Phi(u) = \cos^{-4d_{L}M}\left(\frac{\pi u}{4M}\right)\) for some positive integer \(M\), in the inverse Mellin transform \(V_{s}(y,L)\). Then for \(s\) in the critical strip, any nonnegative integer \(k\), and positive integer \(N\) with \(N < 2M\), \(V_{s}(y,L)\) satisfies the estimates
      \[
  ...
  \section{The Riemann Hypothesis and Nontrivial Zeros}
    ...
    \begin{conjecture*}[Riemann hypothesis]
      The nontrivial zeros of the analytic \(L\)-function \(L(s)\) lie on the vertical line \(\s = \frac{1}{2}\).
    \end{conjecture*}
