\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Analytic Data}
    ...
    \begin{enumerate}[align=left]
      ...
      \[
        \L_{L}(s) = q_{L}^{\frac{s}{2}}\g_{L}(s)L(s) \quad \text{and} \quad \g_{L}(s) = \pi^{-\frac{d_{L}s}{2}}\prod_{j}\G\left(\frac{s+\mu_{j}}{2}\right),
      \]
      called the \textbf{completion}\index{completion} of \(L(s)\) and the \textbf{gamma factor}\index{gamma factor} of \(L(s)\) respectively, satisfying
      \[
        \L_{L}(s) = \e_{L}\conj{\L_{L}(1-\conj{s})}.
      \]
      This is called the \textbf{functional equation}\index{functional equation} of \(L(s)\). The tuple \((\e_{L},q_{L},\mu_{1},\ldots,\mu_{d_{L}})\) is called the \textbf{functional equation data}\index{functional equation data} of \(L(s)\).
      \item[\textbf{Euler product:}] For every prime \(p\), there exist complex numbers \((\a_{j}(p))_{j \le d_{L}}\) called the \textbf{Euler parameters}\index{Euler parameters} of \(L(s)\) at \(p\) satisfying \(|\a_{j}(p)| \le p^{\theta}\) for some \(0 \le \theta < 1\) and are such that \(L(s)\) admits the Euler product
      \[
    ...
    To an analytic \(L\)-function \(L(s)\), we associate its \textbf{analytic conductor}\index{analytic conductor} \(\mf{q}_{L}(s)\) defined by
    \[
      \mf{q}_{L}(s) = q_{L}\mf{q}_{L,\infty}(s),
    \]
    where
    \[
      \mf{q}_{L,\infty}(s) = \prod_{j}(|s+\mu_{j}|+3).
    \]
    The choice of \(3\) in \(|s+\mu_{j}|+3\) is a matter of convenience as could use any positive constant. In particular, it is useful when taking logarithms as \(\log(|s+\mu_{j}|+3) \ge 1\). For legibility, we will also write
    \[
      \mf{q}_{L}(t) = \mf{q}_{L}\left(\frac{1}{2}+it\right) \quad \text{and} \quad \mf{q}_{L,\infty}(t) = \mf{q}_{L,\infty}\left(\frac{1}{2}+it\right),
    \]
    when it is clear \(s = \frac{1}{2}+it\). Estimates for the analytic conductor follow from those of the gamma function. Recall from Stirling's formula that
    ...
    for bounded \(\s\) provided \(s\) is at least distance \(\e\) away from the poles of \(\G(1-s)\). Then the estimates
     \begin{equation}\label{equ:gamma_factor_analytic_conductor_estimate}
      \frac{\g_{L}(1-s)}{\g_{L}(s)} \ll_{\e} \mf{q}_{L,\infty}(s)^{\frac{1}{2}-\s} \quad \text{and} \quad q_{L}^{\frac{1}{2}-s}\frac{\g_{L}(1-s)}{\g_{L}(s)} \ll_{\e} \mf{q}_{L}(s)^{\frac{1}{2}-\s},
    \end{equation}
    ...
    \begin{equation}\label{equ:digamma_gamma_factor_analytic_conductor_estimate}
      \frac{\g_{L}'}{\g_{L}}(s) \ll_{\e} \log\mf{q}_{L,\infty}(s) \quad \text{and} \quad \log{q_{L}}+\frac{\g_{L}'}{\g_{L}}(s) \ll_{\e} \log\mf{q}_{L}(s),
    \end{equation}
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{proof}
      ...
      \[
        L(s) \ll_{\e} \mf{q}_{L}(s)^{\frac{1}{2}-\s}\conj{L(1-\conj{s})}.
      \]
    ...
    \end{equation}
    for \(s\) in the critical strip and bounded \(\tau\) provided \(s+u\) is at least distance \(\e\) away from the poles of the gamma factor. Since \(\Phi(u)\) is bounded in the vertical strip \(|\tau| < a+1\), the integrand exhibits exponential decay. Therefore the integral is locally absolutely uniformly bounded and hence \(V_{L,s}(y)\) is smooth. The function \(V_{L,s}(y)\) is the smooth cutoff function mentioned previously. We will also let \(\e_{L}(s)\) be given by
    \[
      \e_{L}(s) = \e_{L}q_{L}^{\frac{1}{2}-s}\frac{\g_{L}(1-s)}{\g_{L}(s)}.
    \]
    For legibility, we write
    \[
      \e_{L}(t) = \e_{L}\left(\frac{1}{2}+it\right),
    \]
    when it is clear \(s = \frac{1}{2}+it\). This term appears as a factor in the dual sum accounting for the functional equation data. We now prove the \textbf{approximate function equation}\index{approximate function equation}:
    ...
    \begin{theorem*}[Approximate functional equation]
      ...
      \[
        L(s) = \sum_{n \ge 1}\frac{a_{L}(n)}{n^{s}}V_{L,s}\left(\frac{n}{\sqrt{q_{L}}X}\right)+\e_{L}(s)\sum_{n \ge 1}\frac{\conj{a_{L}(n)}}{n^{1-s}}V_{L,1-s}\left(\frac{nX}{\sqrt{q_{L}}}\right)+\frac{R_{L}(s,X)}{q_{L}^{\frac{s}{2}}\g_{L}(s)},
      \]
      where \(R_{L}(s,X)\) is given by
      \[
        R_{L}(s,X) = \Res_{u = 1-s}\frac{\L_{L}(s+u)\Phi(u)X^{u}}{u}+\Res_{u = -s}\frac{\L_{L}(s+u)\Phi(u)X^{u}}{u}.
      \]
    \end{theorem*}
    \begin{proof}
      Consider the integral
      \[
        \frac{1}{2\pi i}\int_{(a)}\L_{L}(s+u)\Phi(u)X^{u}\,\frac{du}{u}.
      \]    
      ...
      \[
        q_{L}^{\frac{s}{2}}\g_{L}(s)\sum_{n \ge 1}\frac{a_{L}(n)}{n^{s}}V_{L,s}\left(\frac{n}{\sqrt{q_{L}}X}\right).
      \]
      ...
      \[
        \frac{1}{2\pi i}\int_{(-a)}\L_{L}(s+u)\Phi(u)X^{u}\,\frac{du}{u}+\L_{L}(s)+R_{L}(s,X).
      \]
      Apply the functional equation and perform the change of variables \(u \mapsto -u\) to rewrite this as
      \[
        -\e_{L}\frac{1}{2\pi i}\int_{(a)}\conj{\L_{L}(1-\conj{s+u})}\Phi(u)X^{-u}\,\frac{du}{u}+\L_{L}(s)+R_{L}(s,X).
      \]
      ...
      \[
        -\e_{L}q_{L}^{\frac{1-s}{2}}\g_{L}(1-s)\sum_{n \ge 1}\frac{\conj{a_{L}(n)}}{n^{1-s}}V_{L,s}\left(\frac{nX}{\sqrt{q_{L}}}\right)+\L_{L}(s)+R_{L}(s,X).
      \]
      Equating these two evaluations and isolating the completion gives
      \begin{align*}
        \L_{L}(s) &= q_{L}^{\frac{s}{2}}\g_{L}(s)\sum_{n \ge 1}\frac{a_{L}(n)}{n^{s}}V_{L,s}\left(\frac{n}{\sqrt{q_{L}}X}\right) \\
        &+ \e_{L}q_{L}^{\frac{1-s}{2}}\g_{L}(1-s)\sum_{n \ge 1}\frac{\conj{a_{L}(n)}}{n^{1-s}}V_{L,1-s}\left(\frac{nX}{\sqrt{q_{L}}}\right)+R_{L}(s,X).
      \end{align*}
      Diving by \(q_{L}^{\frac{s}{2}}\g_{L}(s)\) completes the proof.
    \end{proof}

    Let us now show how \(V_{L,s}(y)\) has the effect of dampening the two dual sums appearing on the right-hand side of the approximate functional equation. In practice, it is common to choose \(\Phi(u)\) such that it has exponential decay and we can make the vertical strip on which it is bounded arbitrarily wide. For example, let
    \[
    ...
    \end{equation}
    for \(|\tau| < (2M-1)+1\) provided \(u\) is at least distance \(\e\) away from the poles of \(\Phi(u)\). Therefore \(\Phi(u)\) admits exponential decay. For this choice of \(\Phi(u)\), \(V_{L,s}(y)\) and its derivatives will possess rapid decay.

    \begin{proposition}\label{prop:V_function_decay}
      Let \(L(s)\) be an analytic \(L\)-function, set \(\Phi(u) = \cos^{-4d_{L}M}\left(\frac{\pi u}{4M}\right)\) for some positive integer \(M\), and let \(V_{L,s}(y)\) be the inverse Mellin transform defined by
      \[
        V_{L,s}(y) = \frac{1}{2\pi i}\int_{(2M-1)}\frac{\g_{L}(s+u)}{\g_{L}(s)}\Phi(u)y^{-u}\frac{du}{u}.
      \]
      Then for \(s\) in the critical strip, any nonnegative integer \(k\), and positive integer \(N\) with \(N < 2M\), \(V_{L,s}(y)\) satisfies the estimates
      \[
        (-y)^{k}V_{L,s}^{(k)}(y) = \begin{cases} \d_{k,0}+O_{\e}\left(\left(\frac{y}{\sqrt{\mf{q}_{L,\infty}(s)}}\right)^{N}\right) & \text{if \(y \ll \sqrt{\mf{q}_{L,\infty}(s)}\)}, \\ O_{\e}\left(\left(\frac{y}{\sqrt{\mf{q}_{L,\infty}(s)}}\right)^{-N}\right) & \text{if \(y \gg \sqrt{\mf{q}_{L,\infty}(s)}\)}. \end{cases}
      \]
      In particular,
      \[
        (-y)^{k}V_{L,s}^{(k)}(y) \ll_{\e} \left(1+\frac{y}{\sqrt{\mf{q}_{L,\infty}(s)}}\right)^{-N}.
      \]
    \end{proposition}
    \begin{proof}
      As we have already seen, the integrand defining \(V_{L,s}(y)\) admits exponential decay. This permits us to differentiate under the integral sign and shift the line of integration. The former shows
      \[
        (-y)^{k}V_{L,s}^{(k)}(y) = \frac{1}{2\pi i}\int_{(2M-1)}\frac{\g_{L}(s+u)}{\g_{L}(s)}\Phi(u)(u)_{k}y^{-u}\frac{du}{u}.
      \]
      Shifting to \((-N)\), we pass by a simple pole at \(u = 0\) of residue \(1\) if and only if \(k = 0\). This gives
      \[
        (-y)^{k}V_{L,s}^{(k)}(y) = \d_{k,0}+\frac{1}{2\pi i}\int_{(-N)}\frac{\g_{L}(s+u)}{\g_{L}(s)}\Phi(u)(u)_{k}y^{-u}\frac{du}{u}.
      \]
      ...
      \[
        (-y)^{k}V_{L,s}^{(k)}(y) = \d_{k,0}+O_{\e}\left(\left(\frac{y}{\sqrt{\mf{q}_{L,\infty}(s)}}\right)^{N}\right).
      \]
      If we instead shift to \((N)\), we do not pass by any poles and obtain
      \[
        (-y)^{k}V_{L,s}^{(k)}(y) = \frac{1}{2\pi i}\int_{(N)}\frac{\g_{L}(s+u)}{\g_{L}(s)}\Phi(u)(u)_{k}y^{-u}\frac{du}{u}.
      \]
      An analogous argument to estimate the remaining integral shows
      \[
        (-y)^{k}V_{L,s}^{(k)}(y) = O_{\e}\left(\left(\frac{y}{\sqrt{\mf{q}_{L,\infty}(s)}}\right)^{-N}\right).
      \]
      From these \(O\)-estimates we obtain nontrivial bounds in the ranges \(y \ll \sqrt{\mf{q}_{L,\infty}(s)}\) and \(y \gg \sqrt{\mf{q}_{L,\infty}(s)}\) respectively. Combining both of these estimates produces the bound
      \[
        (-y)^{k}V_{L,s}^{(k)}(y) \ll_{\e} \left(1+\frac{y}{\sqrt{\mf{q}_{L,\infty}(s)}}\right)^{-N}.
      \]
    \end{proof}

    With our choice of \(\Phi(u)\), this result shows that \(V_{L,s}(y)\) is essentially \(1\) up to some admissible error for \(y \ll \sqrt{\mf{q}_{L,\infty}(s)}\) and exhibits rapid decay thereafter as we can take \(M\) (and hence \(N\)) to be arbitrarily large.
    
    In a similar spirit to the approximate functional equation, a useful summation formula can be derived from the functional equation. Let \(\psi(y)\) be a smooth weight where \(\Psi(s)\) is its Mellin transform. Then we will let \(\psi_{L}(y)\) be the inverse Mellin transform
    \[
      \psi_{L}(y) = \frac{1}{2\pi i}\int_{(a)}q_{L}^{s}\frac{\g_{L}(s)}{\g_{L}(1-s)}y^{-s}\Psi(1-s)\,ds,
    \]
    defined for \(y > 0\) where \(a > 1\). By our choice of \(\psi(y)\), its inverse Mellin transform \(\Psi(s)\) has rapid decay. Since \(L(s)\) has at most polynomial growth by \cref{prop:L_function_bounded_in_vertical_strips}, the integrand has rapid decay as well. Therefore the integral is locally absolutely uniformly bounded and hence \(\psi_{L}(y)\) is smooth for \(y > 0\). Our result is the following:

    \begin{theorem}
      Let \(L(s)\) be an analytic \(L\)-function and let \(\psi(y)\) be a smooth weight where \(\Psi(s)\) is its Mellin transform. Then
      \[
        \sum_{n \ge 1}a_{L}(n)\psi(n) = \frac{\e_{L}}{\sqrt{q_{L}}}\sum_{n \ge 1}\conj{a_{L}(n)}\psi_{L}(n)+R_{L}\Psi(1),
      \]
      where \(R_{L}\) is given by
      \[
        R_{L} = \Res_{s = 1}L(s).
      \]
    \end{theorem}
    \begin{proof}
      ...
      \[
        \sum_{n \ge 1}a_{L}(n)\psi(n) = \frac{1}{2\pi i}\int_{(1-a)}\e_{L}q_{L}^{\frac{1}{2}-s}\frac{\g_{L}(1-s)}{\g_{L}(s)}\conj{L(1-\conj{s})}\Psi(s)\,ds+R_{L}\Psi(1).
      \]
      Performing the change of variables \(s \mapsto 1-s\) in this latter integral gives
      \[
        \sum_{n \ge 1}a_{L}(n)\psi(n) = \frac{1}{2\pi i}\int_{(a)}\e_{L}q_{L}^{s-\frac{1}{2}}\frac{\g_{L}(s)}{\g_{L}(1-s)}\conj{L(\conj{s})}\Psi(1-s)\,ds+R_{L}\Psi(1).
      \]
  ...
  \section{The Riemann Hypothesis and Nontrivial Zeros}
    ...
    \[
      L(s) = \e_{L}q_{L}^{\frac{1}{2}-s}\frac{\g_{L}(1-s)}{\g_{L}(s)}\conj{L(1-\conj{s})}.
    \] 
    So for \(\s < 0\), we see that \(\conj{L(1-\conj{s})}\) is nonzero. Moreover, \(\g_{L}(1-s)\) is as well. Together this means that for \(\s < 0\) the poles of \(\g_{L}(s)\) are zeros of \(L(s)\). Such a zero is called a \textbf{trivial zero}\index{trivial zero} of \(L(s)\). From the definition of the gamma factor, they are all simple and of the form \(s = -(\mu_{j}+2n)\) for some gamma parameter \(\mu_{j}\) and some nonnegative integer \(n\).
  ...
  \section{The Lindel\"of Hypothesis and Estimates on the Critical Line}
    ...
    \begin{theorem}\label{thm:convexity_bound_for_L_function_in_critial_strip}
      ...
      \[
        L(s) \ll_{\e} \mf{q}_{L}(s)^{\frac{1-\s}{2}+\e},
      \]
    ...
    \begin{proof}
      ...
      \[
        (s-1)^{r_{L}}L(s) \ll_{\e} (s-1)^{r_{L}}\mf{q}_{L}(s)^{\frac{1}{2}+\e}.
      \]
      By the Phragm\'en-Lindel\"of convexity principle, we obtain
      \[
        (s-1)^{r_{L}}L(s) \ll_{\e} (s-1)^{r_{L}}\mf{q}_{L}(s)^{\frac{1-\s}{2}+\e},
      \]
    ...
    \[
      L\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}_{L}(t)^{\frac{1}{4}+\e}.
    \]
    ...
    \begin{conjecture*}[Lindel\"of hypothesis, \(L(s)\)]
      ...
      \[
        L\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}_{L}(t)^{\e}.
      \]
    \end{conjecture*}
    ...
    \[
      L\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}_{L}(t)^{\d+\e},
    \]
    ...
    \[
      L(s) \ll_{\e} \mf{q}_{L}(t)^{\frac{1}{4}+\e},
    \]
    ...
    \[
      L^{(k)}\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}_{L}(t)^{\frac{1}{4}+\e}.
    \]
    ...
    \[
      A_{L,\w}(t,X) = \sum_{n \ge 1}a_{L}(n)n^{-it}\w\left(\frac{n}{X}\right).
    \]
    and write
    \[
      A_{L,\w}(X) = A_{L,\w}(0,X).
    \]
    Our result estimates \(L(s)\) at \(s = \frac{1}{2}+it\) in terms of \(A_{L,\w}(t,X)\).
    ...
    \begin{theorem}\label{thm:critical_line_estimate}
      ...
      \[
        L\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}_{L}(t)^{\e}\sup_{X \ll \sqrt{\mf{q}_{L}(t)}}\left(\frac{|A_{L,\w}(t,X)|}{\sqrt{X}}\right).
      \]
    \end{theorem}
    \begin{proof}
      ...
      \begin{align*}
        L\left(\frac{1}{2}+it\right) &= \sum_{n \ge 1}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}V_{L,t}\left(\frac{n}{\sqrt{q_{L}}}\right) \\
        &+ \e_{L}(t)\sum_{n \ge 1}\frac{\conj{a_{L}(n)}n^{it}}{\sqrt{n}}V_{L,t}\left(\frac{n}{\sqrt{q_{L}}}\right)+\frac{R_{L}\left(\frac{1}{2}+it,1\right)}{q_{L}^{\frac{1}{4}+i\frac{t}{2}}\g_{L}\left(\frac{1}{2}+it\right)}.
      \end{align*}
      \cref{equ:gamma_factor_analytic_conductor_estimate} implies \(\e_{L}(t) \ll 1\) while \cref{equ:vertical_strip_gamma_estimates,equ:choice_for_V_decay_estimate} together imply that the residue term exhibits exponential decay. This implies that right-hand side of the approximate functional equation is
      \begin{align*}
        O\left(\sum_{n \ge 1}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}V_{L,t}\left(\frac{n}{\sqrt{q_{L}}}\right)\right)+O\left(\sum_{n \ge 1}\frac{\conj{a_{L}(n)}n^{it}}{\sqrt{n}}V_{L,-t}\left(\frac{n}{\sqrt{q_{L}}}\right)\right)+O(e^{-|t|}).
      \end{align*}
      We now turn to estimating the remaining sums which will be accomplished by applying the smooth dyadic weight. Consider the first sum. By our choice of \(\Phi(u)\), the estimates in \cref{prop:V_function_decay} are valid which implies that the summands exhibit rapid decay for \(n \gg \sqrt{\mf{q}_{L}(t)}\). Whence the sum is say
      \[
        \sum_{n \ll \sqrt{\mf{q}_{L}(t)}}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}V_{L,t}\left(\frac{n}{\sqrt{q_{L}}}\right)+O_{\e}\left(\mf{q}_{L}(t)^{\e}\right)
      \]
      Now write 
      \[
        V_{L,s}(y) = \sum_{k \in \Z}\w\left(\frac{y}{2^{k}}\right)V_{L,s}(y).
      \]
      Apply this identity to the sum and interchange the resulting two sums by local finiteness to obtain
      \[
        \sum_{k \in \Z}\sum_{n \ll \sqrt{\mf{q}_{L}(t)}}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}\w\left(\frac{n}{2^{k}\sqrt{q_{L}}}\right)V_{L,t}\left(\frac{n}{\sqrt{q_{L}}}\right).
      \]
      By compact support of the smooth dyadic weight, for each \(k\) the inner sum over \(n\) is supported on the dyadic block \(n \asymp X_{k}\) where \(X_{k} = 2^{k}\sqrt{q_{L}}\). As \(n \ll \sqrt{\mf{q}_{L}(t)}\), the summands which contribute must satisfy \(k \ll \log\mf{q}_{L,\infty}(t)\). Therefore our double sum can be expressed as
      \[
        \sum_{k \ll \log\mf{q}_{L,\infty}(t)}\sum_{n \asymp X_{k}}\frac{a_{L}(n)n^{-it}}{\sqrt{n}}\w\left(\frac{n}{X_{k}}\right)V_{L,t}\left(\frac{n}{\sqrt{q_{L}}}\right).
      \]
      Let \(V_{L,k}(y) = \frac{1}{\sqrt{y}}V_{L,t}\left(\frac{y}{\sqrt{q_{L}}}\right)\). By Abel summation, the inner sum is \(O\) of
      \[
        \sup_{n \asymp X_{k}}|A_{L,\w}(X_{k},t)|\left|V_{L,k}(n)\right|+\int_{u \asymp X_{k}}|A_{L,\w}(u,t)||V_{L,k}'(u)|\,du.
      \]
      In view of \cref{prop:V_function_decay}, we have the estimates
      \[
        V_{L,k}(y) = O\left(\frac{1}{\sqrt{y}}\right) \quad \text{and} \quad V'_{k}(y) = O\left(\frac{1}{y^{\frac{3}{2}}}\right),
      \]
      for \(y \ll \sqrt{\mf{q}_{L}(t)}\). Whence our previous expression is \(O\) of
      \[
        \sup_{X \asymp X_{k}}\left(\frac{|A_{L,\w}(t,X)|}{\sqrt{X}}\right).
      \]
      As there are \(O_{\e}\left(\mf{q}_{L}(t)^{\e}\right)\) many such \(k\), the bound above implies that our sum is \(O_{\e}\) of
      \[
        \mf{q}_{L}(t)^{\e}\sup_{X \ll \sqrt{\mf{q}_{L}(t)}}\left(\frac{|A_{L,\w}(t,X)|}{\sqrt{X}}\right).
      \]
      The estimate for the second sum is treated analogously. Thus
      \[
        L\left(\frac{1}{2}+it\right) \ll_{\e} \mf{q}_{L}(t)^{\e}\sup_{X \ll \sqrt{\mf{q}_{L}(t)}}\left(\frac{|A_{L,\w}(t,X)|}{\sqrt{X}}\right),
      \]
      upon absorbing smaller order error terms.
    \end{proof}

    As any useful estimate for the sum \(A_{L,\w}(t,X)\) will be in terms of powers of \(X\), the supremum in \cref{thm:central_value_estimate} will automatically pick out the largest possible bound when \(X\) is the square root of the analytic conductor. This means that the savings of \(\sqrt{X}\) in the denominator is essentially as large as possible. For example, we obtain the convexity bound as an application. Since the Dirichlet series of \(L(s)\) is absolutely convergent for \(\s < 1\), we have a bound of shape
    \[
      \sum_{n \le X}|a_{L}(n)| \ll_{\e} X^{1+\e}.
    \]
    Then \(A_{L,\w}(t,X) \ll_{\e} X^{1+\e}\) as well and we obtain
    \begin{equation}\label{equ:coefficient_sum_trivial_bound}
      \sup_{X \ll \sqrt{\mf{q}_{L}(t)}}\frac{|A_{L,\w}(t,X)|}{\sqrt{X}} \ll_{\e} \mf{q}_{L}(t)^{\frac{1}{4}+\e},
    \end{equation}
    which gives the convexity bound. As we believe \(L(s)\) satisfies the Lindel\"of hypothesis, we should expect lots of cancellation in the sum \(A_{L,\w}(t,X)\). In fact, the expected cancellation should be \(O\left(\mf{q}_{L}(t)^{\frac{1}{4}}\right)\) in light of the aforementioned bound. This means we expect \(A_{L,\w}(t,X)\) to exhibit square-root cancellation in the sense
    \[
      A_{L,\w}(t,X) \ll_{\e} X^{\frac{1}{2}+\e}.
    \]
    ...
    \begin{theorem}\label{thm:central_value_estimate}
      Let \(L(s)\) be an analytic \(L\)-function and let \(\w(y)\) be a smooth dyadic weight. Then
      \[
        L\left(\frac{1}{2}\right) \ll_{\e} \mf{q}\left(L\right)^{\e}\max_{X \ll \sqrt{\mf{q}\left(L\right)}}\left(\frac{|A_{L,\w}(X)|}{\sqrt{X}}\right).
      \]
    \end{theorem}
    \begin{proof}
  ...
  \section{Logarithmic Derivatives}
    ...
    \]
    We call \(\L_{L}(n)\) the \textbf{Von Mangoldt coefficients}\index{Von Mangoldt coefficients} of \(L(s)\).

    There is an incredibly useful formula for the logarithmic derivative of \(L(s)\) which is often the starting point for deeper analytic investigations. To deduce it, we require a more complete understanding of the completion \(\L_{L}(s)\). First observe that the zeros \(\rho\) of \(\L_{L}(s)\) are contained within the critical strip. Indeed, by our earlier discussion of zeros, \(\L_{L}(s)\) is nonzero for \(\s > 1\) and the functional equation implies that \(\L_{L}(s)\) is nonzero for \(\s < 0\) too. This means that the zeros of \(\L_{L}(s)\) are exactly nontrivial zeros of \(L(s)\). Now let us setup some notation. We define \(\xi_{L}(s)\) by
    \[
      \xi_{L}(s) = (s(1-s))^{r_{L}}\L_{L}(s).
    \]
    Then \(\xi_{L}(s)\) is just \(\L_{L}(s)\) with the potential poles at \(s = 0\) and \(s = 1\) removed. This means \(\xi_{L}(s)\) is entire. The functional equation also implies
    \[
      \xi_{L}(s) = \e_{L}\conj{\xi_{L}(1-\conj{s})}.
    \]
    Our result will compute the Hadamard factorization of \(\xi_{L}(s)\) and taking its logarithmic derivative will give a formula relating the logarithmic derivative of \(L(s)\) to the zeros of \(L(s)\).
    ...
    \end{proposition}
    \begin{proof}
      Recall that \(\xi_{L}(s)\) is entire and its zeros are exactly the nontrivial zeros of \(L(s)\). We claim \(\xi_{L}(s)\) is of order \(1\). This follows from Stirling's formula and that \((s-1)^{r_{L}}L(s)\) is of order \(1\). Then the Hadamard factorization theorem implies
      \[
        \xi_{L}(s) = e^{A_{L}+B_{L}s}\prod_{\rho \neq 0,1}\left(1-\frac{s}{\rho}\right)e^{\frac{s}{\rho}},
      \]
      for some constants \(A_{L}\) and \(B_{L}\) and that the desired sum converges. This proves the first statement. We will compute the logarithmic derivative of \(\xi_{L}(s)\) in two ways to prove the second statement. On the one hand, taking the logarithmic derivative of the definition of \(\xi_{L}(s)\) yields
      \[
        \frac{\xi_{L}'}{\xi_{L}}(s) = \frac{r_{L}}{s}+\frac{r_{L}}{s-1}+\frac{1}{2}\log{q_{L}}+\frac{\g_{L}'}{\g_{L}}(s)+\frac{L'}{L}(s).
      \]
      On the other hand, taking the logarithmic derivative of the Hadamard factorization gives
      \[
        \frac{\xi_{L}'}{\xi_{L}}(s) = B_{L}+\sum_{\rho \neq 0,1}\left(\frac{1}{s-\rho}+\frac{1}{\rho}\right).
      \]
  ...
  \iffalse\section{Zero Density}
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \]
        where we have made use of the fact that the nontrivial zeros occur is pairs \(\rho\) and \(1-\conj{\rho}\) where the latter is also a nontrivial zero of \(L(1-s)\). Now fix \(s\) such that it does not coincide with the ordinate of a nontrivial zero. Then \(s\) is bounded away from all of the nontrivial zeros and it follows that \(\frac{1}{(s-\rho)}+\frac{1}{(1-s-\conj{\rho})} \ll \frac{1}{\rho^{2}}\) and \(\frac{1}{\rho}+\frac{1}{\conj{\rho}} \ll \frac{1}{\rho^{2}}\). Therefore the sums
        \[
        ...
        \begin{equation}\label{equ:log_conductor_gamma_factor_bound_analytic_conductor}
          \frac{1}{2}\log{q_{L}}+\frac{\g_{L}'}{\g_{L}}(s) \ll \log{\mf{q}_{L}(s)},
        \end{equation}
        ...
        \[
          -\frac{L'}{L}(s) = -\frac{L'}{L}(s)+\frac{L'}{L}(3+it,f)+O(\log{\mf{q}_{L}(s)}).
        \]
        ...
        \[
          -\frac{L'}{L}(s) = \frac{r_{L}}{s}+\frac{r_{L}}{s-1}+\frac{\g_{L}'}{\g_{L}}(s)-\sum_{\rho \neq 0,1}\left(\frac{1}{s-\rho}-\frac{1}{3+it-\rho}\right)+O(\log{\mf{q}_{L}(s)}).
        \]
        ...
        \[
          \frac{\g_{L}'}{\g_{L}}(s) = -\sum_{|s+\mu_{j}| < 1}\frac{1}{s+\mu_{j}}+O(\log{\mf{q}_{L,\infty}(s)}).
        \]
        Then we obtain
        \[
          -\frac{L'}{L}(s) = \frac{r_{L}}{s}+\frac{r_{L}}{s-1}-\sum_{|s+\mu_{j}| < 1}\frac{1}{s+\mu_{j}}-\sum_{\substack{|s-\rho| < 1 \\ \rho \neq 0,1}}\frac{1}{s-\rho}+O(\log{\mf{q}_{L}(s)}),
        ...
        \[
          \sum_{|s+\mu_{j}| < 1}\Re\left(\frac{1}{s+\mu_{j}}\right)+\sum_{\substack{|s-\rho| < 1 \\ \rho \neq 0,1}}\Re\left(\frac{1}{s-\rho}\right) \le \Re\left(-\frac{L'}{L}(s)\right)+\Re\left(\frac{r_{L}}{s}\right)+\Re\left(\frac{r_{L}}{s-1}\right)+O(\log{\mf{q}_{L}(s)}).
        \]
    ...
    \begin{proof}
      ...
      \]
      As the nontrivial zeros occur in pairs \(\rho\) and \(1-\conj{\rho}\) where the latter is also a nontrivial zero of \(L(s)\), it follows that
      \[
        N(T,f) = N'(T,f)+N_{L}'(T)+O(\log{\mf{q}(f)}),
      \]
      where \(O(\log{\mf{q}(f)})\) accounts for the possible real nontrivial zeros. There are finitely many such nontrivial zeros because the interval \(0 \le s \le 1\) is compact. We will estimate \(N'(T,f)\) and in doing so we may assume \(L(s)\) does not vanish on the line \(t = T\) by varying \(T\) by a sufficiently small constant, if necessary, and observing that \(N(T,f)\) is modified by a quantity of size \(O(\log{\mf{q}(iT,f)})\) by \cref{lem:powerful_L-function_approximation_lemma} (i). Since the nontrivial zeros are isolated, let \(\d > 0\) be small enough such that \(\L_{L}(s)\) has no nontrivial zeros for \(-\d \le t < 0\). Then by our previous comments and the argument principle,
      \[
        N'(T,f) = \frac{1}{2\pi i}\int_{\eta}\frac{\xi_{L}'}{\xi_{L}}(s)\,ds+O(\log{\mf{q}(iT,f)}),
      \]
      ...
      \[
        \frac{1}{2\pi i}\int_{\eta}\frac{\xi_{L}'}{\xi_{L}}(s)\,ds = \frac{1}{2\pi i}\int_{\eta}\frac{d}{ds}\log{|\xi_{L}(s)|}\,ds+\frac{1}{2\pi }\int_{\eta}\frac{d}{ds}\arg\xi_{L}(s)\,ds = \frac{1}{2\pi}\D_{\eta}\arg(\xi_{L}(s)),
      \]
      where the last equality holds by noting that \(\eta\) is closed so that the first integral vanishes. For convenience, set \(\eta_{L} = \eta_{1}+\eta_{2}+\eta_{3}\) and \(\eta_{R} = \eta_{4}+\eta_{5}+\eta_{6}\). Recall that we have already shown \(\xi_{L}(\conj{s}) = \conj{(\xi_{L}(s))}\). Using this fact along with the functional equation and that \(-\arg(s) = \arg(\conj{s})\), we compute
      \begin{align*}
        \D_{\eta_{L}}\arg(\xi_{L}(s)) &= \D_{\eta_{L}}\arg\left(\e_{L}\xi_{L}(1-s)\right) \\
        &= -\D_{\conj{\eta_{R}}}\arg\left(\e_{L}\xi_{L}(s)\right) \\
        &= \D_{\conj{\eta_{R}}}\arg\left(\conj{\e_{L}}\xi(\conj{s},f)\right) \\
        &= \D_{\eta_{R}}\arg\left(\conj{\e_{L}}\xi_{L}(s)\right) \\
        &= \D_{\eta_{R}}\arg\left(\conj{\e_{L}}\right)+\D_{\eta_{R}}\arg(\xi_{L}(s)) \\
        &= \D_{\eta_{R}}\arg(\xi_{L}(s)).
      \end{align*}
      In other words, the change in argument along \(\eta_{L}\) is equal to the change in argument along \(\eta_{R}\) and so
      \[
        \frac{1}{2\pi i}\int_{\eta}\frac{\xi_{L}'}{\xi_{L}}(s)\,ds = \frac{1}{\pi}\D_{\eta_{L}}\arg\xi_{L}(s).
      \]
      Thus to estimate the integral, we estimate the change in argument along \(\eta_{L}\) of each factor in
      \[
        \xi_{L}(s) = (s(1-s))^{r_{L}}Q^{\frac{s}{2}}\pi^{-\frac{d_{L}s}{2}}\prod_{j}\G\left(\frac{s+\mu_{j}}{2}\right)L(s).
      \]
      ...
      \[
        \frac{1}{\pi}\D_{\eta_{L}}\arg\xi_{L}(s) = \frac{T}{2\pi}\log\left(\frac{T^{d_{L}}}{(2\pi e)^{d_{L}}}\right)+O(\log{\mf{q}(iT,f)}),
      \]
      and therefore
      \[
        N'(T,f) = \frac{T}{2\pi}\log\left(\frac{T^{d_{L}}}{(2\pi e)^{d_{L}}}\right)+O(\log{\mf{q}(iT,f)}),
      \]
      The asymptotic formula follows from this estimate and that the same exact estimate holds for \(N_{L}'(T)\) by using the \(L\)-function \(L(s)\). We obtain the asymptotic equality at once from the asymptotic formula.
    \end{proof}
