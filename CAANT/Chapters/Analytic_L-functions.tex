\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Analytic Data}
    ...
    \[
      q_{L}^{\frac{s}{2}}\g(s,L)L(s) = \e_{L}q_{L}^{\frac{1-s}{2}}\g(1-s,L)\conj{L}(1-s).
    \]
  ...
  \section{Logarithmic Derivatives}
    ...
    \end{equation}
    which we refer to as the \textbf{functional equation}\index{functional equation} for \(\xi(s,L)\). Observe that \(\xi(s,L)\) is just \(\L(s,L)\) with the potential poles at \(s = 0\) and \(s = 1\) removed. This means \(\xi(s,L)\) is entire. In fact, it admits a Hadamard factorization. To see this, first observe that \(\xi(s,L)\) is of order \(1\) by Stirling's formula (see \cref{sec:the_gamma_function}) and that \((s-1)^{m_{L}}L(s)\) is of order \(1\). Then the Hadamard factorization theorem implies
    \[
  ...
  \section{The Explicit Formula}
    ...
    \begin{theorem*}[Explict Formula]
      ...
      \begin{align*}
        \sum_{n \ge 1}\left(\L_{L}(n)\psi(n)+\L_{\conj{L}}(n)\phi(n)\right) &= \psi(1)\log(q_{L})+m_{L}\Psi(1)-\sum_{\rho}\Psi(\rho) \\
    ...
    \begin{proof}
      ...
      \[
        \frac{1}{2\pi i}\int_{(-\e)}-\frac{\conj{L}'}{\conj{L}}(1-s)\Psi(s)\,ds = \sum_{n \ge 1}\L_{\conj{L}}(n)\phi(n).
      \]
      Whence
      \begin{align*}
        \sum_{n \ge 1}\left(\L_{L}(n)\psi(n)+\L_{\conj{L}}(n)\phi(n)\right) &= \psi(1)\log(q_{L})+m_{L}\Psi(1)-\sum_{\rho}\Psi(\rho) \\
