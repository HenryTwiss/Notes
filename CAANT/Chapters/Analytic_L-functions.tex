\chapter{Analytic \texorpdfstring{\(L\)}{L}-functions}
  ...
  \section{Zero Density Estimates}
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label*=(\roman*)]
        ...
        \item Let \(\s > 1\). The estimate follows from direct computation since any nontrivial zero \(\rho\) satisfies \(\b \le 1\).
    ...
    \begin{proof}
      ...
      \]
      As the logarithmic derivative of \(L(s)\) is absolutely bounded for \(\s > 1+\t\), the integral is \(O(1)\) on \(\eta_{2}\). On \(\eta_{1}\) and \(\eta_{3}\), \cref{lem:powerful_L-function_approximation_lemma} (ii) and (iv) together imply that the integral is \(O(\log{\mf{q}(iT,L)})\). Whence
      \[
  ...
  \iffalse\section{A Zero-free Region}
