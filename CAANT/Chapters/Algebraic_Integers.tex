\chapter{Algebraic Integers}
  ...
  \section{Dedekind Domains}
   ...
   Now let \(\smc{O}\) be a Dedekind domain with field of fractions \(K\), \(L/K\) be a finite separable field extension, and \(\mc{O}\) be the integral closure of \(\smc{O}\) in \(L\). It turns out that \(\mc{O}\) is also a Dedekind domain.
  ...
  \section{Localization}
    ...
    In order for localization to be a useful tool in algebraic investigations, it must behave well with respect to what we have already developed. To this end, we will collect some useful properties about localization. Our first property is a generalization of the Chinese remainder theorem.

    \begin{proposition}
      Let \(\smc{O}\) be an integral domain with field of fractions \(K\). Then for any integral ideal \(\mf{a}\) of \(\smc{O}\), we have an isomorphism
      \[
    ...
    \end{proposition}
    \begin{proof}
      \todo{use chinese remainder theorem here}
    ...
    Our third property is that that localization behaves well with respect to Dedekind extensions.

    \begin{proposition}\label{prop:localization_of_integral_closure_is_integral_closure}
      Let \(\mc{O}/\smc{O}\) be Dedekind extension for a finite separable field extension \(L/K\). Then for any multiplicative set \(D\) of \(\smc{O}\), \(\mc{O}D^{-1}/\smc{O}D^{-1}\) is a Dedekind extension of \(L/K\).
    \end{proposition}
    \begin{proof}
      To show \(\mc{O}D^{-1}\) is integrally closed, we must prove \(\mc{O}D^{-1} = \conj{\smc{O}D^{-1}}\). For the forward inclusion, let \(\frac{\eta}{\d} \in \mc{O}D^{-1}\). As \(\mc{O}\) is the integral closure of \(\smc{O}\) in \(L\), \(\eta\) is integral over \(\smc{O}\) so that
      \[
    ...
    It follows from this result that the Dedekind extension \(\mc{O}_{\mf{p}}/\smc{O}_{\mf{p}}\) is an extension of principle ideal domains. Whence it admits an integral basis by \cref{thm:integral_basis_AKBL}. This is often a useful reduction for developing further results or applications in conjunction with \cref{prop:ring_is_intersection_of_all_localizations}.
  ...
  \section{\todo{Ramification}}
    We begin with an important claim regarding a Dedekind extension \(\mc{O}/\smc{O}\) of a finite separable field extension \(L/K\).
