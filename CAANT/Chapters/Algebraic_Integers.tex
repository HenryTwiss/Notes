\chapter{Algebraic Integers}
  ...
  \iffalse\section{\todo{Dedekind Extensions}}
    A ring extension \(\mc{O}/\smc{O}\) is said to be a \textbf{Dedekind extension}\index{Dedekind extension} of a finite separable extension \(L/K\) if \(\mc{O}\) and \(\smc{O}\) are Dedekind domains whose field of fractions are \(L\) and \(K\) respectively and \(\mc{O}\) is the integral closure of \(\smc{O}\) in \(L\). These Dedekind domains are related via the identity
    \[
      \mc{O} \cap K = \smc{O}.
    \]
    It also follows from \cref{prop:field_of_fractions_AKBL} that \(L\) is the field of fractions of \(\mc{O}\).

  \fi\iffalse
    If we merely assume \(\smc{O}\) is a Dedekind domain with field of fractions \(K\) and \(L/K\) is a finite separable extension then \(\mc{O}\) is also a Dedekind domain. The argument is similar to \cref{thm:ring_of_integers_is_Dedekind}.

    \begin{proposition}\label{prop:integral_closure_of_Dedekind_is_Dedekind}
      Let \(\smc{O}\) be a Dedekind domain with field of fractions \(K\), \(L/K\) be a finite separable extension, and \(\mc{O}\) be the integral closure of \(\smc{O}\) in \(L\). Then \(\mc{O}\) is a Dedekind domain. That is, \(\mc{O}/\smc{O}\) is a Dedekind extension of \(L/K\).
    \end{proposition}
    \begin{proof}
