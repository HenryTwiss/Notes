\chapter{Algebraic Integers}
  ...
  \section{Integrality}
    ...
    \begin{proposition}\label{prop:field_of_fractions_AKBL}
      ...
      \[
        \l = \frac{\b}{\a},
      \]
      for some \(\b \in \mc{O}\) and nonzero \(\a \in \smc{O}\). In particular, is an integrally closed domain with field of fractions \(L\). Moreover, \(\l \in L\) is integral over \(\smc{O}\) if and only if the minimal polynomial \(m_{\l}(x)\) of \(\l\) over \(K\) has coefficients in \(\smc{O}\).
    \end{proposition}
    \begin{proof}
  ...
  \section{Traces and Norms}
    ...
    \end{theorem}
    \begin{proof}
      Let \(\l_{1},\ldots,\l_{n}\) be a basis for \(L/K\). From \cref{lemma:unique_factorization_domains_are_integrally_closed} we see that \(\smc{O}\) is an integrally closed domain. Then by \cref{prop:field_of_fractions_AKBL}, we may multiply by a nonzero element of \(\smc{O}\), if necessary, to ensure that this basis belongs to \(\mc{O}\). Then \cref{lem:lemma_for_integral_basis_AKBL} implies
      \[
        d_{L/K}(\l_{1},\ldots,\l_{n})\mc{O} \subseteq \smc{O}\l_{1}+\cdots+\smc{O}\l_{n}.
      \]
  ...
  \section{Localization}
    ...
    \[
      \mf{F}(X) = \mf{F}\left(\smc{O}-\bigcup_{\mf{p} \in X}\mf{p}\right)^{-1}.
    \]
    If \(X\) consists of a single prime \(\mf{p}\) then \(\mc{O}(X) = \mc{O}_{\mf{p}}\) and \(\mf{F}(X) = \mf{F}_{\mf{p}}\). Moreover, \(\mc{O}_{\mf{p}}/\smc{O}_{\mf{p}}\) and \(\mc{O}(X)/\smc{O}(X)\) are Dedekind extensions of \(L/K\).
  ...
  \iffalse\section{\todo{Orders}}
    ...
    \end{proof}
  \fi\section{\todo{Ramification}}
    We begin with an important claim regarding a Dedekind extension \(\mc{O}/\smc{O}\) of a finite separable extension \(L/K\).
