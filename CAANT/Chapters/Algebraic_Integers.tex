\chapter{Algebraic Integers}
  ...
  \fi\section{Ramification}
    ...
    Another way to phrase this result is that the primes above \(\mf{p}\) are all conjugate to each other and their ramification indices and inertia degrees are all equal. We call the common ramification index \(e\) the \textbf{ramification index}\index{ramification index} of \(\mf{p}\) and the common inertia degree \(f\) the \textbf{inertia degree}\index{inertia degree} of \(\mf{p}\). If there are \(r\) primes above \(\mf{p}\), the fundamental equality takes the particularly simple form
    \[
      n = ref.
    \]
    ...
    We now turn to the setting of a number field \(K\). Every prime \(\mf{p}\) of \(K\) is above some prime \(p\). Then the residue class extension of \(\mc{O}_{K}/\Z\) for \(\mf{p}\) is \(\F_{\mf{p}}/\F_{p}\). Also, we write \(f_{p}(\mf{p})\) and \(e_{p}(\mf{p})\) for the inertia degree and ramification index of \(\mf{p}\) relative to \(p\) respectively. Moreover, all of the residue class extensions are separable since \(\F_{p}\) is perfect. This implies \(\mf{p}\) is ramified in \(\mc{O}_{K}/\Z\) if and only if \(e_{p}(\mf{p}) = 1\).
