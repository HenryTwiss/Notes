\chapter{Algebraic Integers}
  ...
  \fi\section{Ramification}
    ...
    Suppose \(\mc{O}'/\mc{O}\) and \(\mc{O}/\smc{O}\) are Dedekind extensions for finite separable extensions \(M/L\) and \(L/K\) respectively. Let \(\mf{P}'\), \(\mf{P}\), and \(\mf{p}\) be primes of \(\mc{O}'\), \(\mc{O}\), and \(\smc{O}\) respectively with \(\mf{P}'\) above \(\mf{P}\) and \(\mf{P}\) above \(\mf{p}\). We illustrate this relationship via the diagram
    \begin{center}
      \begin{tikzcd}[row sep=large, column sep=large]
      ...
      \end{tikzcd}
    \end{center} 
    Then \(\mf{P}^{e_{\mf{p}}(\mf{P})} \mid\mid \mf{p}\mc{O}\) and \(\mf{P}^{e_{\mf{P}}(\mf{P}')} \mid\mid \mf{P}\mc{O}'\). We also have the residue class field extensions \(\F_{\mf{P}'}/\F_{\mf{P}}\) and \(\F_{\mf{P}}/\F_{\mf{p}}\). It follows that
    ...
    \end{equation}
    In other words, the ramification indices and inertia degrees are multiplicative with respect to primes above other primes.
  ...
  \section{Galois Extensions}
    ...
    \]
    As \(\smc{O} \subseteq K\), \(\Gal(L/K)\) fixes \(\smc{O}\) pointwise and hence every fractional ideal of \(\smc{O}\) as well. In fact, \(\Gal(L/K)\) fixing \(\smc{O}\) pointwise implies that \(\Gal(L/K)\) permutes the roots of the monic polynomial in \(\smc{O}[x]\) which \(\a \in \mc{O}\) satisfies. Whence there is an action
    \[
      \Gal(L/K) \x \mc{O} \to \mc{O} \qquad (\s,\a) \mapsto \s(\a).
    \]
    In particular, each automorphism \(\s\) of \(\Gal(L/K)\) restricts to an automorphism of \(\mc{O}\) and therefore \(\s(\mf{P})\) is a prime of \(\mc{O}\) if \(\mf{P}\). Moreover, if \(\mf{P}\) is above \(\mf{p}\) then so is \(\s(\mf{P})\) as it is a prime containing \(\mf{p}\). Whence
    \[
    ...
    \iffalse
    \begin{proof}
      Let \(\mf{P}_{i}\) and \(\mf{P}_{j}\) be distinct primes above \(\mf{p}\). Assume by contraction that \(\s(\mf{P}_{i}) \neq \mf{P}_{j}\) for any \(\s \in \Gal(L/K)\). As distinct primes are relatively prime, the Chinese remainder theorem implies the existence of an \(\a \in \mc{O}\) such that
      \[
