\chapter{Arithmetic of Number Fields}
  ...
  \section{Dedekind Domains}
    ...
    \subsection*{Number Fields}
      ...
      Now suppose \(L/K/\Q\) is an extension of number fields. Then \(L/K/\Q\) is a finite separable tower. Moreover, \(\mc{O}_{L}/\mc{O}_{K}/\Z\) is a tower of Dedekind extension of \(L/K/\Q\) by \cref{prop:integral_closure_is_transitive,prop:integral_closure_of_Dedekind_is_Dedekind}. In particular, \(\mc{O}_{L}/\mc{O}_{K}\) is a Dedekind extension of \(L/K\).
  ...
  \section{Prime Splitting}
    ...
    \]
    That is, \(f_{\mf{p}}(\mf{P})\) is the degree of \(\F_{\mf{P}}/\F_{\mf{p}}\) which is the dimension of the residue field \(\F_{\mf{P}}\) as an \(\F_{\mf{p}}\)-vector space. In fact, something slightly more general is true. Let \(\mf{A}\) and \(\mf{B}\) be an integral ideals of \(\mc{O}\) with \(\mf{p}\mc{O} \subseteq \mf{B} \subseteq \mf{A}\). Consider the homomorphism
    \[
    ...
    \subsection*{Number Fields}
      We now turn to the setting where \(L/K\) is an extension of number fields and recall that the underlying Dedekind extension is \(\mc{O}_{L}/\mc{O}_{K}\). Then every residue class extension is separable as \(\F_{p}\) is perfect. A prime \(\mf{p}\) of \(K\) is said to be \textbf{inert}\index{inert}, \textbf{nonsplit}\index{nonsplit}, \textbf{split}\index{split}, or \textbf{totally split}\index{totally split} in \(L/K\) if it is in \(\mc{O}_{L}/\mc{O}_{K}\). Primes \(\mf{P}\) and \(\mf{p}\) of \(L\) and \(K\) are said to be \textbf{unramified}\index{unramified}, \textbf{ramified}\index{ramified}, or \textbf{totally ramified}\index{totally ramified} in \(L/K\) if they are in \(\mc{O}_{L}/\mc{O}_{K}\). In particular, if \(\mf{P}\) is above \(\mf{p}\) then it is unramified in \(L/K\) if and only if \(e_{\mf{p}}(\mf{P}) = 1\). Lastly, the \textbf{ideal norm}\index{ideal norm} \(\mf{N}_{L/K}\) of \(L/K\) is the ideal norm \(\mf{N}_{\mc{O}_{L}/\mc{O}_{K}}\).

      In the case of a number field \(K\), we can say much more as \(\mc{O}_{K}/\Z\) admits an integral basis and the reside class fields extension \(\F_{\mf{p}}/\F_{p}\) for a prime \(\mf{p}\) above \(p\) is an extension of finite fields. The \textbf{ideal norm}\index{ideal norm} \(\mf{N}_{K}\) of \(K\) is the ideal norm \(\mf{N}_{\mc{O}_{K}/\Z}\). As \(\Z\) is a principal ideal domain, every fractional ideal is principal. Therefore, \(\mf{N}_{K}(\mf{f})\) is generated by some \(r_{\mf{f}} \in \Q_{+}\) for every fractional ideal \(\mf{f}\) of \(\mc{O}_{K}\). As the ideal norm is completely multiplicative, this induces a homomorphism
      \[
      ...
      As a consequence of this result and Lagrange's theorem, we see that \(\Norm_{K}(\mf{a}) \in \mf{a}\) and therefore for every fractional ideal \(\mf{f}\) as well.
  ...
  \section{Differents and Discriminants}
    ...
    \]
    From this chain we see that the different \(\mf{D}_{\mc{O}/\smc{O}}\) is a measure of how much \(\mc{O}\) fails to be self-dual since \(\mf{D}_{\mc{O}/\smc{O}} = \mc{O}^{\vee}\) if and only if \(\mc{O}\) is self-dual. In terms of the different, the dual \(\mf{F}^{\vee}\) of a fractional ideal \(\mf{F}\) of \(\mc{O}\) may be expressed as
    \[
    ...
    \begin{proof}
      ...
      \[
        \mf{d}_{\mc{O}/\smc{O}} = d_{\mc{O}/\smc{O}}\smc{O}.
      \]
      Therefore \(\mf{p} \mid \mf{d}_{\mc{O}/\smc{O}}\) if and only if \(d_{\mc{O}/\smc{O}}(\a_{1},\ldots,\a_{n}) \subset \mf{p}\) which is to say
      \[
    ...
    \subsection*{Number Fields}
      ...
      Computing the norm of the discriminant \(\D_{K}\) is also an easy matter:
      ...
      As a consequence of \cref{prop:sum_of_divisors_growth_rate}, we have the slightly weaker estimate \(a_{K}(n) \ll_{\e} n^{\e}\).\fi