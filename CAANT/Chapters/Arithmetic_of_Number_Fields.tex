\chapter{Arithmetic of Number Fields}
  ...
  \section{Orders}
    ...
    \[
      \Pic(\smc{O}) = \Cl(\smc{O}).
    \]
    An element of \(\Pic(\smc{O})\) is called an \textbf{Picard class}\index{Picard class} of \(\smc{O}\). As every fractional ideal \(\mf{f}\) can be expressed as \(\mf{f} = \frac{1}{\d}\mf{a}\) for some nonzero \(\d\) and integral ideal \(\mf{a}\), we have \(\d\mf{f} = \mf{a}\) and it follows that every ideal class can be represented by an integral ideal. 
    ...
    \end{proposition}
    \begin{proof}
      As the second map is injective and the fourth map is surjective, the sequence is exact at \(K^{\ast}/\smc{O}^{\ast}\) and \(\Pic(\smc{O})\). So it suffices to prove exactness at \(I(\smc{O})\). This follows because the principal fractional ideals represent the identity class of \(\Pic(\smc{O})\) and these are generated by a complete set of representatives for \(K^{\ast}/\smc{O}^{\ast}\).
    \end{proof}
  ...
  \section{Prime Splitting}
    ...
    The ramification indices and inertia degrees satisfy a simple relationship to the degree of \(L/K\) that is fundamental to the study of Dedekind extensions. In order to state it, we will require a lemma. The idea is that \(\mc{O}/\mf{p}\mc{O}\) is of maximum dimension as an \(\F_{\mf{p}}\)-vector space, namely \(n\), and its dimension can be computed as a sum of products of the ramification indices and inertia degrees. This relationship is known as the \textbf{fundamental equality}\index{fundamental equality}.
    ...
    \begin{proof}
      ...
      \[
        \mc{O}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})} \supset \mf{P}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})} \supset \cdots \supset \mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})},
      \]
      of \(\F_{\mf{p}}\)-vector spaces. By the third isomorphism theorem, the successive quotients are of the form \(\mf{P}_{i}^{j}/\mf{P}_{i}^{j+1}\) which are all isomorphic to \(\F_{\mf{P}}\) by \cref{lem:isomorphism_of_quotient_by_prime_integral_ideals}. Whence \(\mf{P}/\mf{P}_{i}^{j}\) is an \(\F_{\mf{P}}\)-vector space of dimension \(j\). In particular, \(\mc{O}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})}\) is an \(\F_{\mf{P}}\)-vector space of dimension \(e_{\mf{p}}(\mf{P}_{i})\). Since the dimension of \(\F_{\mf{P}_{i}}\) as an \(\F_{\mf{p}}\)-vector space is \(f_{\mf{p}}(\mf{P}_{i})\), it follows that \(\mc{O}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})}\) is an \(\F_{\mf{p}}\)-vector space of dimension \(ef_{\mf{p}}(\mf{P}_{i})\). This completes the proof.
    \end{proof}
    ...
    \]
    Setting \(\mf{N}_{\mc{O}/\smc{O}}(\mc{O}) = \smc{O}\), unique factorization shows that we obtain a homomorphism
    \[
    ...
    \subsection*{Number Fields}
      ...
      \begin{proof}
        ...
        \[
          \mc{O}_{K} \supset \mf{p} \supset \cdots \supset \mf{p}^{e}.
        \]
        The successive quotients are of the form \(\mf{p}^{i}/\mf{p}^{i+1}\) which are all isomorphic to \(\F_{\mf{p}}\) by \cref{lem:isomorphism_of_quotient_by_prime_integral_ideals}. Repeated application of the third isomorphism theorem gives
        \[
          |\mc{O}_{K}/\mf{p}^{e}| = \prod_{i}|\mf{p}^{i}/\mf{p}^{i+1}|.
        \]
        As \(\F_{\mf{p}}\) is an \(\F_{p}\)-vector space of dimension \(f_{p}(\mf{p})\), we find that \(|\mf{p}^{i}/\mf{p}^{i+1}| = p^{f_{p}(\mf{p})}\). Whence \(|\mc{O}_{K}/\mf{p}^{e}| = p^{ef_{p}(\mf{p})}\). This proves the first statement. The second statement follows from the first together with \cref{prop:ideal_norm_of_principal_ideal}.
      \end{proof}
  ...
  \section{Differents and Discriminants}
    ...
    \end{theorem}
    \begin{proof}
      In view of \cref{prop:ring_is_intersection_of_all_localizations,prop:ramification_setup_respects_localization,prop:different_and_discriminant_respect_localization}, we may assume that \(\mc{O}/\smc{O}\) is a local Dedekind extension. Let \(e\) be a nonnegative integer with \(e \le e_{\mf{p}}(\mf{P})\) and write \(\mf{p}\mc{O} = \mf{P}^{e}\mf{A}\) for some integral ideal \(\mf{A}\) of \(\mc{O}\). By \cref{lem:different_containment_lemma}, \(\mf{P}^{e} \mid \mf{D}_{\mc{O}/\smc{O}}\) if and only if \(\Trace_{L/K}(\mf{P}^{e}) \subseteq \smc{O}\). As \(\mf{P}^{e} = \mf{p}^{-1}\mf{A}\), it it further equivalent to show \(\Trace_{L/K}(\mf{A}) \subseteq \mf{p}\). Since \(\mc{O}/\smc{O}\) admits an integral basis, we have \(\Trace_{K/L}(\mf{A}) = \Trace_{\mc{O}/\smc{O}}(\mf{A})\) from  \cref{prop:trace_and_norm_reduce_for_integral_basis} and so it is further equivalent to show
      \[
        \Trace_{\mc{O}/\smc{O}}(\mf{A}) \subseteq \mf{p}.
      \]
      Let \(\a \in \mf{A}\) and let \(\conj{\a} \in \mc{O}/\mf{p}\mc{O}\) be its image under natural projection. Then
      \[
        \Trace_{\mc{O}/\smc{O}}(\a) \equiv \Trace_{(\mc{O}/\mf{p}\mc{O})/\F_{\mf{p}}}(\conj{\a}) \pmod{\mf{p}}.
      \]
      Therefore it is yet further equivalent to show
      \[
        \Trace_{(\mc{O}/\mf{p}\mc{O})/\F_{\mf{p}}}(\conj{\a}) \equiv 0 \pmod{\mf{p}}.
      \]

      To show \(\mf{P}^{e_{\mf{p}}(\mf{P})-1} \mid \mf{D}_{\mc{O}/\smc{O}}\), take \(e = e_{\mf{p}}(\mf{P})-1\) so that \(\mf{P} \mid\mid \mf{A}\). Then the prime factors of \(\mf{A}\) and \(\mf{p}\mc{O}\) are the same. Whence there is a positive integer \(k\) such that \(\mf{A}^{k}\) is divisible by \(\mf{p}\mc{O}\). So \(\mf{p}\mc{O} \mid \mf{A}^{k}\) and therefore \(\mf{A}^{k} \subseteq \mf{p}\mc{O}\) as to divide is to contain. This implies that the \(\F_{\mf{p}}\)-linear operator \(T_{\conj{\a}}^{k}\) is the zero operator. Whence \(T_{\conj{\a}}\) is nilpotent. As nilpotent operators have trace zero, \(\Trace_{(\mc{O}/\mf{p}\mc{O})/\F_{\mf{p}}}(\conj{\a}) = 0\) which implies the desired congruence.

      To show \(\mf{P}^{e_{\mf{p}}(\mf{P})} \mid \mf{D}_{\mc{O}/\smc{O}}\) if and only if \(e_{\mf{p}}(\mf{P}) \equiv 0 \tmod{\mf{p}}\), take \(e = e_{\mf{p}}(\mf{P})\) so that \(\mf{P}\) and \(\mf{A}\) are relatively prime. The Chinese remainder theorem gives an isomorphism
      \[
        \mc{O}/\mf{p}\mc{O} \cong (\mc{O}/\mf{P}^{e_{\mf{p}}(\mf{P})}) \op (\mc{O}/\mf{A}).
      \]
      Since \(\a \in \mf{A}\), there exists \(\b,\g \in \mc{O}\) such that 
      \[
        \a \equiv \b \pmod{\mf{P}^{e_{\mf{p}}(\mf{P})}} \quad \text{and} \quad \a \equiv \g \pmod{\mf{A}}.
      \]
      Let \(\conj{\b} \in \mc{O}/\mf{P}^{e_{\mf{p}}(\mf{P})}\) and \(\conj{\g} \in \mc{O}/\mf{A}\mc{O}\) be the images of \(\b\) and \(\g\) under the respective natural projection. Then
      \[
        \Trace_{(\mc{O}/\mf{p}\mc{O})/\F_{\mf{p}}}(\conj{\a}) = \Trace_{(\mc{O}/\mf{P}^{e_{\mf{p}}(\mf{P})})/\F_{\mf{p}}}(\conj{\b})+\Trace_{(\mc{O}/\mf{A})/\F_{\mf{p}}}(\conj{\g}).
      \]
      But as \(\a \in \mf{A}\), we have \(\Trace_{(\mc{O}/\mf{A})/\F_{\mf{p}}}(\conj{\g}) = 0\) and it is further equivalent to show
      \[
        \Trace_{(\mc{O}/\mf{P}^{e_{\mf{p}}(\mf{P})})/\F_{\mf{p}}}(\conj{\b}) = 0.
      \]
      We will prove that this occurs if and only if \(e_{\mf{p}}(\mf{P}) \equiv 0 \tmod{\mf{p}}\). \todo{xxx}
      \iffalse
      As the residue class extensions are assumed to be separable, \cref{lem:trace_is_nondegenerate} implies that \(\Trace_{\F_{\mf{P}}/\F_{\mf{p}}}(\conj{\b})\) cannot be zero for all \(\b \in \mc{O}\). So it must be the case that \(\Trace_{(\mc{O}/\mf{P}^{e_{\mf{p}}(\mf{P})})/\F_{\mf{p}}}(\conj{\b}) = 0\) for all \(\b \in \mc{O}\) if and only if \(e_{\mf{p}}(\mf{P}) \equiv 0 \tmod{\mf{p}}\).

       As all residue class extensions are separable by assumption, \(\mf{P}\) ramifies in \(\mc{O}/\smc{O}\) if and only if \(e_{\mf{p}}(\mf{P}) \ge 2\). But (i) and (ii) together imply that \(\mf{P} \mid \mf{D}_{\mc{O}/\smc{O}}\) if and only if \(e_{\mf{p}}(\mf{P}) \ge 2\). Whence \(\mf{P}\) ramifies in \(\mc{O}/\smc{O}\) if and only if \(\mf{P} \mid \mf{D}_{\mc{O}/\smc{O}}\). So it remains to prove (i) and (ii).
       \fi
    \end{proof}
