\chapter{Arithmetic of Number Fields}
  ...
  \section{Dedekind Domains}
    ...
    It follows from this result that every fractional ideal is invertible in a Dedekind domain. We will use this fact so often that we suppress it for legibility. 
  ...
  \section{\todo{Orders}}
    ...
    \end{proposition}
    \begin{proof}
      For the forward implication, suppose \(\mf{q}_{\smc{O}}\) is an integral ideal of \(\smc{O}\). Then there is some nonzero \(\a \in \mf{q}_{\smc{O}}\) with \(\a\conj{\smc{O}} \subseteq \smc{O}\). Whence \(\a\conj{\smc{O}}\) is an ideal of \(\smc{O}\) and hence a finitely generated \(\smc{O}\)-module as \(\smc{O}\) is noetherian. But then \(\conj{\smc{O}}\) is a finitely generated \(\smc{O}\)-module. For the reverse implication, let \(\frac{\a_{1}}{\d_{1}},\ldots,\frac{\a_{r}}{\d_{r}}\) be generators of \(\conj{\smc{O}}\) as a \(\smc{O}\)-module. Then \(\d = \prod_{i}\d_{i}\) is nonzero with \(\d\conj{\smc{O}} \subset \smc{O}\). Whence \(\d \in \mf{q}_{\smc{O}}\).
    \end{proof}
    
    Accordingly, an \textbf{order}\index{order} \(\smc{O}\) is a one-dimensional noetherian domain whose conductor is an integral ideal of \(\smc{O}\). If \(\smc{O}\) has field of fractions \(K\), then this last condition is equivalent to the normalization \(\conj{\smc{O}}\) being a finitely generated \(\smc{O}\)-module. This means \(\smc{O}\) is a Dedekind domain if and only if it is integrally closed which happens if and only if the conductor is \(\smc{O}\) itself. In fact, the normalization of an order is always a Dedekind domain.
