\chapter{Arithmetic of Number Fields}
  ...
  \section{Dedekind Domains}
    ...
    \begin{proof}
      ...
      \]
      for some positive integer \(n\) and \(\a_{i} \in \smc{O}\). Taking \(n\) minimal ensures \(\a_{0} \neq 0\) and isolating \(\a_{0}\) shows \(\a_{0} \in \mf{P}\) whence \(\a_{0} \in \mf{P} \cap \smc{O}\). Therefore \(\mf{P} \cap \smc{O} = \mf{p}\) for some prime \(\mf{p}\). This means \(\ker\phi = \mf{p}\). By the first isomorphism theorem, \(\phi\) induces an embedding \(\F_{\mf{p}} \to \mc{O}/\mf{P}\). Therefore \(\mc{O}/\mf{P}\) is a finite dimensional \(\F_{\mf{p}}\)-vector space as \(\mc{O}\) is a finitely generated \(\smc{O}\)-module. By primality of \(\mf{P}\), \(\mc{O}/\mf{P}\) is a finite integral domain and thus must be a field. This proves the first statement and the second is equivalent.
    \end{proof}
  ...
  \section{Localization}
    ...
    \end{proposition}
    \begin{proof}
      We see from \cref{prop:localization_of_integral_closure_is_integral_closure} that \(\smc{O}D^{-1}\) is an integrally closed domain. To show it is noetherian, let \(\mf{A}\) be an integral ideal of \(\smc{O}D^{-1}\) and set \(\mf{a} = \mf{A} \cap \smc{O}\). Then
      \[
        \mf{A} = \mf{a}D^{-1}.
      \]
  ...
  \section{\todo{Orders}}
    ...
    \]
    Then \(\mf{q}_{\smc{O}}\) is an ideal of \(\smc{O}\) and it is the largest ideal which is also an ideal of \(\conj{\smc{O}}\). Clearly \(\smc{O}\) is an integrally closed domain if and only if the conductor is \(\smc{O}\). In the case \(\smc{O}\) is a noetherian domain, the conductor is an integral ideal of \(\smc{O}\), which is to say it is nonzero, precisely when \(\conj{\smc{O}}\) is a finitely generated \(\smc{O}\)-submodule of \(K\).

    \begin{proposition}\label{prop:conductor_integral_ideal_equivalence}
      Let \(\smc{O}\) be a noetherian domain with field of fractions \(K\). Then the conductor \(\mf{q}_{\smc{O}}\) is an integral ideal of \(\smc{O}\) if and only if \(\conj{\smc{O}}\) is a finitely generated \(\smc{O}\)-submodule of \(K\).
    \end{proposition}
    \begin{proof}
      For the forward implication, suppose \(\mf{q}_{\smc{O}}\) is an integral ideal of \(\smc{O}\). Then there is some nonzero \(\a \in \mf{q}_{\smc{O}}\) with \(\a\conj{\smc{O}} \subseteq \smc{O}\). Whence \(\a\conj{\smc{O}}\) is an ideal of \(\smc{O}\) and hence a finitely generated \(\smc{O}\)-module as \(\smc{O}\) is noetherian. But then \(\conj{\smc{O}}\) is a finitely generated \(\smc{O}\)-submodule of \(K\). For the reverse implication, let \(\frac{\a_{1}}{\d_{1}},\ldots,\frac{\a_{r}}{\d_{r}}\) be generators of \(\conj{\smc{O}}\) as a \(\smc{O}\)-submodule of \(K\). Then \(\d = \prod_{i}\d_{i}\) is nonzero with \(\d\conj{\smc{O}} \subset \smc{O}\). Whence \(\d \in \mf{q}_{\smc{O}}\).
    \end{proof}
    
    Accordingly, an \textbf{order}\index{order} \(\smc{O}\) is a one-dimensional noetherian domain whose conductor is an integral ideal of \(\smc{O}\). If \(\smc{O}\) has field of fractions \(K\), then this last condition is equivalent to \(\conj{\smc{O}}\) being a finitely generated \(\smc{O}\)-submodule of \(K\). This means \(\smc{O}\) is a Dedekind domain if and only if it is integrally closed which happens if and only if the conductor is \(\smc{O}\) itself.
