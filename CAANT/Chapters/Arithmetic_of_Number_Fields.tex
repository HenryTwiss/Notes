\chapter{Arithmetic of Number Fields}
  ...
  \section{Prime Splitting}
    ...
    \]
    called the \textbf{ideal norm}\index{ideal norm} of \(\mc{O}/\smc{O}\). This means that the ideal norm is multiplicative. It follows from the fundamental equality that
    \[
    ...
    \]
    In any case, the ideal norm is aptly named because it is compatible with the field norm \(\Norm_{L/K}\).
    ...
    \end{proposition}
    \begin{proof}
      Since both \(\mf{N}_{\mc{O}/\smc{O}}\) and \(\Norm_{L/K}\) are multiplicative and \(L\) is the field of fractions of \(\mc{O}\), it is enough to prove the claim when \(\l \in \mc{O}\). By prime factorization, we may write
      \[
        \l\mc{O} = \prod_{\mf{P}}\mf{P}^{v_{\mf{P}}(\l)},
      \]
      where all but finitely many of the valuations \(v_{\mf{P}}(\l)\) are zero. In view of \cref{prop:ring_is_intersection_of_all_localizations}, it suffices to show
      \[
        \mf{N}_{\mc{O}/\smc{O}}(\l\mc{O})_{\mf{p}} = \Norm_{L/K}(\l)\smc{O}_{\mf{p}},
      \]
      for all primes \(\mf{p}\) of \(\smc{O}\). In view of the prime factorization of \(\l\mc{O}\) and that \(\smc{O}_{\mf{p}}\) is a discrete valuation ring by \cref{thm:Dedekind_if_and_only_if_all_localizations_are_discrete_valuation_rings}, we have
      \[
        \mf{N}_{\mc{O}/\smc{O}}(\l\mc{O})_{\mf{p}} = \prod_{\mf{P} \mid \mf{p}\mc{O}}\mf{p}_{\mf{p}}^{v_{\mf{P}}(\l)f_{\mf{p}}(\mf{P})} \quad \text{and} \quad \Norm_{L/K}(\l)\smc{O}_{\mf{p}} = \mf{p}_{\mf{p}}^{v_{\mf{p}}(\Norm_{L/K}(\l))}.
      \]
      Therefore we are reduced to proving
      \[
        \sum_{\mf{P} \mid \mf{p}\mc{O}}v_{\mf{P}}(\l)f_{\mf{p}}(\mf{P}) = v_{\mf{p}}(\Norm_{L/K}(\l)).
      \]
      We will show this via a dimension computation. Recall that \(\mc{O}_{\mf{p}}/\smc{O}_{\mf{p}}\) is a Dedekind extension of principal ideal domains for \(L/K\) and admits an integral basis. We will compute the dimension of \(\mc{O}_{\mf{p}}/\l\mc{O}_{\mf{p}}\) as an \(\F_{\mf{p}}\)-vector space in two ways. First, the Chinese remainder theorem gives an isomorphism
      \[
        \mc{O}_{\mf{p}}/\l\mc{O}_{\mf{p}} \cong \bigop_{\mf{P} \mid \mf{p}\mc{O}}\mc{O}_{\mf{p}}/\mf{P}_{\mf{p}}^{v_{\mf{P}}(\l)}.
      \]
      By \cref{lem:quotient_by_prime_power_isomorphism,prop:ramification_setup_respects_localization}, \(\mc{O}_{\mf{p}}/\mf{P}_{\mf{p}}^{v_{\mf{P}}(\l)}\) is an \(\F_{\mf{p}}\)-vector space of dimension \(v_{\mf{P}}(\l)f_{\mf{p}}(\mf{P})\). Therefore \(\mc{O}_{\mf{p}}/\l\mc{O}_{\mf{p}}\) is an \(\F_{\mf{p}}\)-vector space of dimension \(\sum_{\mf{P} \mid \mf{p}\mc{O}}v_{\mf{P}}(\l)f_{\mf{p}}(\mf{P})\). On the other hand, \(\mc{O}_{\mf{p}}\) is a free \(\smc{O}_{\mf{p}}\) of rank \(n\) as \(\mc{O}_{\mf{p}}/\smc{O}_{\mf{p}}\) admits an integral basis. By the structure theorem for finitely generated modules over principal ideal domains and \cref{thm:integral_basis_AKBL}, we have isomorphisms
      \[
        \mc{O}_{\mf{p}} \cong \bigop^{n}\smc{O}_{\mf{p}} \quad \text{and} \quad \l\mc{O}_{\mf{p}} \cong \bigop_{i}\pi^{k_{i}}\smc{O}_{\mf{p}},
      \]
      for some nonnegative integers \(k_{1},\ldots,k_{n}\) and a uniformizer \(\pi\) for \(\smc{O}_{\mf{p}}\). Whence there is another isomorphism
      \[
        \mc{O}_{\mf{p}}/\l\mc{O}_{\mf{p}} \cong \bigop_{i}\smc{O}_{\mf{p}}/\pi^{k_{i}}\smc{O}_{\mf{p}}.
      \]
      Moreover, \(\Norm_{L/K}(\l) = \Norm_{\mc{O}_{\mf{p}}/\smc{O}_{\mf{p}}}(\l)\) by \cref{prop:trace_and_norm_reduce_for_integral_basis}. Recall that \(\Norm_{\mc{O}_{\mf{p}}/\smc{O}_{\mf{p}}}(\l)\) is the determinant of the \(\smc{O}_{\mf{p}}\)-linear operator
      \[
        T_{\l}:\mc{O}_{\mf{p}} \to \mc{O}_{\mf{p}} \qquad x \mapsto \l x. 
      \]
      So under the aforementioned isomorphisms, it follows that \(\Norm_{\mc{O}_{\mf{p}}/\smc{O}_{\mf{p}}}(\l) = \pi^{\sum_{i}k_{i}}\) whence \(v_{\mf{p}}(\Norm_{L/K}(\l)) = \sum_{i}k_{i}\). The proof will be complete if the dimension of \(\mc{O}_{\mf{p}}/\l\mc{O}_{\mf{p}}\) as an \(\F_{\mf{p}}\)-vector space is \(\sum_{i}k_{i}\). From \cref{lem:quotient_by_prime_power_isomorphism,prop:ramification_setup_respects_localization} we see that \(\smc{O}_{\mf{p}}/\pi^{k_{i}}\smc{O}_{\mf{p}}\) is an \(\F_{\mf{p}}\)-vector space of dimension \(k_{i}\). The claim follows from the last isomorphism above.
    \end{proof}
  ...
  \section{Differents and Discriminants}
    ...
    \]
    As \(\mc{O} \subseteq \mc{O}^{\vee}\) by \cref{prop:trace_and_norm_AKBL}, the definition of the inverse shows \(\mf{D}_{\mc{O}/\smc{O}} \subseteq \mc{O}\). Therefore the different \(\mf{D}_{\mc{O}/\smc{O}}\) is an integral ideal of \(\mc{O}\) and we have a chain of inclusions
    \[
    ...
    \]
    As expected, the different respects localization.
    ...
    Similarly to the different, the discriminant also respects localization.
