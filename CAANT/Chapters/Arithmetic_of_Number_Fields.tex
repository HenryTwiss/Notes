\chapter{Arithmetic of Number Fields}
  ...
  \section{Differents and Discriminants}
    ...
    \begin{proof}
      ...
      \[
        \Trace_{(\mc{O}/\mf{p}\mc{O})/\F_{\mf{p}}}(\mf{A}/\mf{p}\mc{O}) \equiv 0 \pmod{\mf{p}}.
      \]

      To prove \(\mf{P}^{e_{\mf{p}}(\mf{P})-1} \mid \mf{D}_{\mc{O}/\smc{O}}\), take \(e = e_{\mf{p}}(\mf{P})-1\) so that \(\mf{P} \mid\mid \mf{A}\). Then the prime factors of \(\mf{A}\) and \(\mf{p}\mc{O}\) are the same. Whence there is a positive integer \(k\) such that \(\mf{A}^{k}\) is divisible by \(\mf{p}\mc{O}\). So \(\mf{p}\mc{O} \mid \mf{A}^{k}\) and therefore \(\mf{A}^{k} \subseteq \mf{p}\mc{O}\) as to divide is to contain. Now let \(\a \in \mf{A}\) with \(\conj{\a} \in \mf{A}/\mf{p}\mc{O}\) its image under natural projection. It follows that the \(k\)-th power of the \(\F_{\mf{p}}\)-linear operator \(T_{\conj{\a}}\) on \(\mc{O}/\mf{p}\mc{O}\) is the zero operator. This means \(T_{\conj{\a}}\) is nilpotent. As nilpotent operators have trace zero, \(\Trace_{(\mc{O}/\mf{p}\mc{O})/\F_{\mf{p}}}(\conj{\a}) = 0\). As \(\a\) was arbitrary, this implies the desired congruence proving the first statement.
      ...
      \[
        \Trace_{(\mc{O}/\mf{P}^{e_{\mf{p}}(\mf{P})})/\F_{\mf{p}}}(\mc{O}/\mf{P}^{e_{\mf{p}}(\mf{P})}) \equiv 0 \pmod{\mf{p}}.
      \]
      We will prove that this occurs if and only if \(e_{\mf{p}}(\mf{P}) \equiv 0 \tmod{\mf{p}}\). To this end, consider the filtration
      \[
      ...
      \]
      induced by natural projection of the adapted basis. Now let \(\a \in \mc{O}\) where \(\conj{\a} \in \mc{O}/\mf{P}^{e_{\mf{p}}(\mf{P}_{i})}\) is its image under natural projection. Also let \(\conj{\a}_{i} \in \mf{P}^{i}/\mf{P}^{i+1}\) be the images of \(\a\) under the aforementioned isomorphism. Then
      \[
        \Trace_{(\mc{O}/\mf{P}^{e_{\mf{p}}(\mf{P})})/\F_{\mf{p}}}(\conj{\a}) = \sum_{i}\Trace_{(\mf{P}^{i}/\mf{P}^{i+1})/\F_{\mf{p}}}(\conj{\a}_{i}).
      \]
      The quotients \(\mf{P}^{i}/\mf{P}^{i+1}\) are all isomorphic to \(\F_{\mf{P}}\) by \cref{lem:isomorphism_of_quotient_by_prime_integral_ideals} which is induced by multiplication and natural projection. This implies the the trace in the summands are all \(\Trace_{\F_{\mf{P}}/\F_{\mf{p}}}(\conj{\a}_{0})\). So
      \[
        \Trace_{(\mc{O}/\mf{P}^{e_{\mf{p}}(\mf{P})})/\F_{\mf{p}}}(\conj{\a}) = e_{\mf{p}}(\mf{P})\Trace_{\F_{\mf{P}}/\F_{\mf{p}}}(\conj{\a}_{0}).
      \]
      As the residue class extension \(\F_{\mf{P}}/\F_{\mf{p}}\) is assumed to be separable, the trace form of \(\F_{\mf{P}}/\F_{\mf{p}}\) is nondegenerate. Therefore the desired congruence holds if and only if \(e_{\mf{p}}(\mf{P}) \equiv 0 \tmod{\mf{p}}\). This proves the second statement.
