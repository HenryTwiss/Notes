\chapter{Arithmetic of Number Fields}
  ...
  \section{Dedekind Domains}
    ...
    \begin{proof}
      ...
      \]
      Thus the determinant of the matrix on the left-hand side must be zero. But this means \(\a\) is a root of the characteristic polynomial \(\det(xI-(\a_{i,j}))\) which is a monic polynomial with coefficients \(\smc{O}\). As \(\smc{O}\) is an integrally closed domain, \(\a \in \smc{O}\) which is a contraction. Thus \(\mf{p}\mf{p}^{-1} = \smc{O}\).
    \end{proof}
  ...
  \section{\todo{Orders}}
    ...
    \]
    for some primes \(\mf{p}_{1},\ldots,\mf{p}_{k}\). Since \(\smc{O}\) is one-dimensional, every maximal ideal containing \(\mf{q}_{\smc{O}}\) is a prime and thus one of the \(\mf{p}_{i}\). In view of \cref{prop:regular_primes_relative_to_conductor}, this means that only finitely many primes of \(\smc{O}\) are irregular and they are exactly the primes which are not invertible. This is a quite concrete description of the failure to which \(\smc{O}\) is not integrally closed in terms of the conductor \(\mf{q}_{\smc{O}}\).
