\chapter{Arithmetic of Number Fields}
  ...
  \section{Localization}
    ...
    We can now say more about the Dedekind extension \(\mc{O}_{\mf{p}}/\smc{O_{\mf{p}}}\). The previous result implies \(\smc{O}_{\mf{p}}\) is a discrete valuation ring whence a principal ideal domain. This forces \(\mc{O}_{\mf{p}}/\smc{O}_{\mf{p}}\) to admit an integral basis, but we can say more. Observe that \(\mf{P}\) is not above \(\mf{p}\) if and only if \(\mf{P}\) contains an element of \(\smc{O}-\mf{p}\). In view of \cref{prop:localization_prime_bijection}, it follows that the only primes of \(\mc{O}_{\mf{p}}\) are those above the unique prime of \(\smc{O}_{\mf{p}}\) and they correspond to the primes of \(\mc{O}\) above \(\mf{p}\). In particular, there are finitely many such primes. By \cref{prop:Dedekind_with_finite_primes_is_PID}, we find that \(\mc{O}_{\mf{p}}\) is also a principal ideal domain. In particular, \(\mc{O}_{\mf{p}}/\smc{O}_{\mf{p}}\) is an extension of principal ideal domains. Moreover, suppose \(\mf{A}\) is an integral ideal of \(\mc{O}\). By prime factorization, we may write
    \[
    ...
    \[
      \mf{A}_{\mf{p}} = \prod_{\mf{P} \mid \mf{p}\smc{O}}\mf{P}_{\mf{p}}^{e_{\mf{P}}}.
    \]
    In other words, localizing an integral ideal of \(\mc{O}\) at \(\mf{p}\) removes all of the prime factors that are not above \(\mf{p}\) and localizes the primes above \(\mf{p}\). In view of \cref{prop:ring_is_intersection_of_all_localizations}, all of this suggests that localizing a Dedekind extension \(\mc{O}/\smc{O}\) at a prime \(\mf{p}\) is a useful simplification technique for developing further results or applications.
