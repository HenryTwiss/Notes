\chapter{Arithmetic of Number Fields}
  ...
  \section{Differents and Discriminants}
    ...
    \]
    with \(\k_{j}^{\vee} \in K\). As the dual basis is determined by \(\Trace_{L/K}(\l_{i}\l_{j}^{\vee}) = \d_{i,j}\), it follows that \(\l \in \L^{\vee}\) if and only if \(\k_{j}^{\vee} \in \smc{O}\). Whence \(\l_{1}^{\vee},\ldots,\l_{n}^{\vee}\) is a basis for \(\L^{\vee}\) as a \(\smc{O}\)-submodule of \(L\) and thus is a complete \(\smc{O}\)-lattice of \(L\).

    Observe that an \(\smc{O}\)-lattice need not be a fractional ideal of \(\mc{O}\) as it need not be a \(\mc{O}\)-submodule of \(L\). However, very fractional ideal \(\mf{F}\) of \(\mc{O}\) is a complete \(\smc{O}\)-lattice. Indeed, \(\mf{F}\) is a finitely generated \(\smc{O}\)-submodule of \(L\) as \(\mc{O}\) is a finitely generated \(\smc{O}\)-module. Moreover, \(\mf{F}\) contains a basis of \(L/K\). To see this, let \(\l_{1},\ldots,\l_{n}\) be a basis for \(L/K\) contained in \(\mc{O}\) and let \(\a \in \mf{F}\) be nonzero. Then \(\a\l_{1},\ldots,\a\l_{n}\) is a basis for \(L/K\) contained in \(\mf{F}\).
