\chapter{Arithmetic of Number Fields}
  ...
  \section{Differents and Discriminants}
    ...
    \]
    For the forward inclusion, observe that \(\Trace_{L/K}(\mf{F}^{\vee}\mf{F}\mc{O}) \subseteq \smc{O}\). Whence \(\mf{F}^{\vee}\mf{F} \subseteq \mc{O}^{\vee}\) which is equivalent to \(\mf{F}^{\vee} \subseteq \mf{F}^{-1}\mc{O}^{\vee}\). This proves the forward inclusion. For the reverse inclusion, we have \(\Trace_{L/K}(\mf{F}^{-1}\mc{O}^{\vee}\mf{F}) \subseteq \smc{O}\). Thus \(\mf{F}^{-1}\mc{O}^{\vee} \subseteq \mf{F}^{\vee}\) proving the reverse inclusion.
    ...
    \iffalse
    As we might hope, localization respects duals:
