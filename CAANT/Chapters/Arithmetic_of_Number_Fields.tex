\chapter{Arithmetic of Number Fields}
  ...
  \section{Localization}
    ...
    \begin{proposition}\label{prop:localization_of_inverse_is_inverse_of_localization}
      Let \(\smc{O}\) be a Dedekind domain with field of fractions \(K\) and \(D\) be a multiplicative subset of \(\smc{O}\). Then for any fractional ideal \(\mf{f}\) of \(\smc{O}\), we have
      \[
        \mf{f}^{-1}D^{-1} = (\mf{f}D^{-1})^{-1}.
      \]
    \end{proposition}
    \begin{proof}
      As \(\mf{f}\mf{f}^{-1} \subseteq \smc{O}\) we have \(\mf{f}D^{-1}\mf{f}^{-1}D^{-1} \subseteq \smc{O}D^{-1}\) which shows the forward inclusion \(\mf{f}^{-1}D^{-1} \subseteq (\mf{f}D^{-1})^{-1}\). For the reverse inclusion, \(\mf{f}D^{-1}(\mf{f}D^{-1})^{-1} \subseteq \smc{O}D^{-1}\) and multiplying by \(\mf{f}^{-1}\) implies \((\mf{f}D^{-1})^{-1} \subseteq \mf{f}^{-1}D^{-1}\) which is the reverse inclusion.
    \end{proof}
  ...
  \section{Differents and Discriminants}
    ...
    \]
    From this chain we see that the different \(\mf{D}_{\mc{O}/\smc{O}}\) is a measure of how much \(\mc{O}\) fails to be self-dual as \(\mf{D}_{\mc{O}/\smc{O}} = \mc{O}^{\vee}\) if and only if \(\mc{O}\) is self-dual. In terms of the different, the dual \(\mf{F}^{\vee}\) of a fractional ideal \(\mf{F}\) of \(\mc{O}\) may be expressed as
    \[
      \mf{F}^{\vee} = \mf{F}^{-1}\mf{D}_{\mc{O}/\smc{O}}^{-1}.
    \]
    Now for the discriminant. We define the \textbf{discriminant}\index{discriminant} \(\mf{d}_{\mc{O}/\smc{O}}\) of \(\mc{O}/\smc{O}\) to be the integral ideal of \(\smc{O}\) generated by the discriminants \(d_{L/K}(\a_{1},\ldots,\a_{n})\) as \(\a_{1},\ldots,\a_{n}\) run over all bases of \(L/K\) contained in \(\mc{O}\). Recall that if \(\mc{O}/\smc{O}\) admits an integral basis then \(\mc{O}\) is a free \(\smc{O}\)-module of rank \(n\). In this case, bases of \(L/K\) contained in \(\mc{O}\) are precisely the integral bases of \(\mc{O}/\smc{O}\). Then
    \[
    ...
    \begin{proof}
      ...
      \]
      It follows that \(\Trace_{L/K}(\mf{F}^{\vee}\mf{F}D^{-1}) \subseteq \smc{O}D^{-1}\) if and only if \(\Trace_{L/K}(\mf{F}^{\vee}\mf{F})D^{-1} \subseteq \smc{O}D^{-1}\). This proves the claim.
    \end{proof}
    ...
    The different and discriminant also respect localization:
