\chapter{Arithmetic of Number Fields}
  ...
  \section{Differents and Discriminants}
    ...
    \end{lemma}
    \begin{proof}
      \(\mf{A}\) divides \(\mf{D}_{\mc{O}/\smc{O}}\) if and only if \(\mf{D}_{\mc{O}/\smc{O}} \subseteq \mf{A}\). This equivalent to \((\mc{O}^{\vee})^{-1} \subseteq \mf{A}\) and inverting shows the further equivalence \(\mf{A}^{-1} \subseteq \mc{O}^{\vee}\). This last inclusion is equivalent to \(\Trace_{L/K}(\mf{A}^{-1}) \subseteq \smc{O}\).
    \end{proof}
