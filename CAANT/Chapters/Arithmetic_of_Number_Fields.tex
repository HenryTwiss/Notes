\chapter{Arithmetic of Number Fields}
  ...
  \section{Prime Splitting}
    ...
    The ramification indices and inertia degrees satisfy a simple relationship to the degree of \(L/K\) that is fundamental to the study of Dedekind extensions. The idea is that \(\mc{O}/\mf{p}\mc{O}\) is of maximum dimension as an \(\F_{\mf{p}}\)-vector space, namely \(n\), and its dimension can be computed as a sum of products of the ramification indices and inertia degrees. This relationship is known as the \textbf{fundamental equality}\index{fundamental equality}.
    ...
    \begin{proof}
      ...
      \]
      Hence \(\l_{1},\ldots,\l_{m}\) spans \(L/K\) whence \(\mc{O}/\mf{p}\mc{O}\) is an \(\F_{\mf{p}}\)-vector space of dimension \(n\). For \(\mc{O}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})}\), consider the filtration
      \[
        \mc{O}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})} \supset \mf{P}_{i}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})} \supset \cdots \supset \mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})-1}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})} \supset 0,
      \]
      of \(\F_{\mf{P}_{i}}\)-vector spaces. By the third isomorphism theorem, quotients of successive layers of the filtration are isomorphic to \(\mf{P}_{i}^{j}/\mf{P}_{i}^{j+1}\). Whence we obtain a short exact sequence
      \begin{center}
        \begin{tikzcd}
          0 \arrow{r} & \mf{P}_{i}^{j+1}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})} \arrow{r} & \mf{P}_{i}^{j}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})} \arrow{r} & \mf{P}_{i}^{j}/\mf{P}_{i}^{j+1} \arrow{r} & 0,
        \end{tikzcd}
      \end{center}
      induced by natural inclusions and projections. As this is a short exact sequence of \(\F_{\mf{P}_{i}}\)-vector spaces it splits. Choosing sections for all of these short exact sequences is equivalent to choosing a basis for \(\mc{O}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})}\) adapted to the filtration and we obtain an isomorphism
      \[
        \mc{O}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})} \cong \bigop_{j}\mf{P}_{i}^{j}/\mf{P}_{i}^{j+1},
      \]
      induced by natural projection of the adapted basis. The quotients \(\mf{P}_{i}^{j}/\mf{P}_{i}^{j+1}\) are all isomorphic to \(\F_{\mf{P}_{i}}\) by \cref{lem:isomorphism_of_quotient_by_prime_integral_ideals}. As \(\F_{\mf{P}_{i}}\) is an \(\F_{\mf{p}}\)-vector spaces of dimension \(f_{\mf{p}}(\mf{P}_{i})\), it follows that \(\mc{O}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})}\) is an \(\F_{\mf{p}}\)-vector space of dimension \(e_{\mf{p}}(\mf{P}_{i})f_{\mf{p}}(\mf{P_{i}})\).
    \end{proof}
  ...
  \section{Differents and Discriminants}
    ...
    \]
    so that the discriminant \(\mf{d}_{\mc{O}/\smc{O}}\) is generated by the discriminant \(d_{\mc{O}/\smc{O}}\). In particular, \(\mf{d}_{\mc{O}/\smc{O}}\) is principal if \(\mc{O}/\smc{O}\) admits an integral basis. In view of \cref{thm:integral_basis_AKBL} this will hold if \(\smc{O}\) is a principal ideal domain.
    ...
    \begin{proof}
      ...
      \]
      of \(\F_{\mf{p}}\)-vector spaces. By the third isomorphism theorem, quotients of successive layers of the filtration are isomorphic to \(\mf{P}^{i}/\mf{P}^{i+1}\). Whence we obtain a short exact sequence
      \begin{center}
        \begin{tikzcd}
      ...
      \]
      induced by natural projection of the adapted basis. Now \(\conj{\b}_{i} \in \mf{P}^{i}/\mf{P}^{i+1}\) be the images of \(\b\) under the aforementioned isomorphism. Then
      \[
    ...
    \begin{proof}
      ...
      \[
        \mf{d}_{\mc{O}/\smc{O}} = d_{\mc{O}/\smc{O}}\smc{O}.
      \]
      Therefore \(\mf{p}\) divides \(\mf{d}_{\mc{O}/\smc{O}}\) if and only if \(d_{\mc{O}/\smc{O}} \equiv 0 \tmod{\mf{p}}\). Since \(\conj{\a_{1}},\ldots,\conj{\a_{n}}\) is a basis for \(\mc{O}/\mf{p}\mc{O}\) as an \(\F_{\mf{p}}\)-vector space, it follows that
      \[
        d_{\mc{O}/\smc{O}} \equiv d_{\F_{\mf{p}}}(\mc{O}/\mf{p}\mc{O}) \pmod{\mf{p}}.
      \]
