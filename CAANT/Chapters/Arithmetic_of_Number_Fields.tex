\chapter{Arithmetic of Number Fields}
  ...
  \section{Traces and Norms}
    ...
    \begin{proof}
      ...
      \]
      By the structure theorem of finitely generated modules over principal ideal domains again, \(M\) is a free \(\smc{O}\)-module of rank at most \(n\). To see that the rank is at least \(n\), let \(\l \in M\) be nonzero. Then \(\l\l_{1},\ldots,\l\l_{n}\) is a basis for \(L/K\) contained in \(M\). Thus the rank of \(M\) is at least \(n\), and in particular it must be \(n\). This proves the second statement.
    \end{proof}
  ...
  \section{Orders}
    ...
    \subsection*{Number Fields}
      We now turn to the setting of a number field \(K\). Recall that \(\mc{O}_{K}/\Z\) is a Dedekind extension. In this setting, orders arise as finitely generated \(\Z\)-modules \(\smc{O}\) contained inside \(\mc{O}_{K}\). Such an order is of the form
      \[
        \smc{O} = \Z[\a_{1},\ldots,\a_{n}],
      \]
      with \(\a_{i} \in \mc{O}_{K}\) and necessarily satisfies
      \[
        \Z \subseteq \smc{O} \subseteq \mc{O}_{K}.
      \]
      However, with this generality the normalization of \(\smc{O}\) need not be \(\mc{O}_{K}\). Therefore we usually only consider orders \(\smc{O}\) whose normalization is \(\mc{O}_{K}\). For example, this always occurs if \(\a_{1},\ldots,\a_{n}\) is a basis for \(K/\Q\) as \(K\) is the field of fractions of \(\mc{O}_{K}\) and the latter is the integral closure of \(\Z\) in \(K\).
      
      More generally, suppose \(L/K\) is an extension of number fields. Then the most natural orders \(\mc{O}\) to consider are finitely generated \(\mc{O}_{K}\)-modules contained inside \(\mc{O}_{L}\). Analogously, such an order is of the form
      \[
      ...
      \]
      Again, we usually restrict to the setting where \(\a_{1},\ldots,\a_{n}\) is a basis for \(L/K\) for then the normalization of \(\mc{O}\) will be \(\mc{O}_{L}\).
  \section{Prime Splitting}
    ...
    Let us now discuss how ramification indices and inertia degrees are affected under localization. The truth of the matter is that they are preserved.
    ...
    \begin{proposition}
      Let \(\mc{O}/\smc{O}\) be a Dedekind extension of a finite separable extension \(L/K\). Then
      \[
    ...
    \end{proposition}
    \begin{proof}
      \iffalse Since both \(\mf{N}_{\mc{O}/\smc{O}}\) and \(\Norm_{L/K}\) are multiplicative and \(L\) is the field of fractions of \(\mc{O}\), it is enough to prove the claim when \(\l \in \mc{O}\). In view of \cref{prop:ring_is_intersection_of_all_localizations}, it suffices to show
      \[
        \mf{N}_{\mc{O}/\smc{O}}(\l\mc{O})_{\mf{p}} = \Norm_{L/K}(\l)\smc{O}_{\mf{p}},
      \]
      for all primes \(\mf{p}\) of \(\smc{O}\). By prime factorization, we may write 
      \[
        \l\mc{O} = \prod_{\mf{P}}\mf{P}^{v_{\mf{P}}(\l)},
      \]
      where all but finitely many of the valuations \(v_{\mf{P}}(\l)\) are zero. If \(\mf{P}\) is not above \(\mf{p}\) then \(\mf{P}\) contains an element of \(\smc{O}-\mf{p}\) whence the prime below it does as well and so the localizations become \(\mc{O}_{\mf{p}}\) and \(\smc{O}_{\mf{p}}\) respectively. So on the one hand, we find that
      \[
        \l\mc{O}_{\mf{p}} = \prod_{\mf{P} \mid \mf{p}\mc{O}}\mf{P}_{\mf{p}}^{v_{\mf{P}}(\l)},
      \]
      and
      \[
        \mf{N}_{\mc{O}/\smc{O}}(\l\mc{O})_{\mf{p}} = \prod_{\mf{P} \mid \mf{p}\mc{O}}\mf{p}_{\mf{p}}^{v_{\mf{P}}(\l)f_{\mf{p}}(\mf{P})}.
      \]
      On the other hand, \(\smc{O}_{\mf{p}}\) is a discrete valuation ring by \cref{thm:Dedekind_if_and_only_if_all_localizations_are_discrete_valuation_rings} whence
      \[
        \Norm_{L/K}(\l)\smc{O}_{\mf{p}} = \mf{p}_{\mf{p}}^{v_{\mf{p}}(\Norm_{L/K}(\l))}.
      \]
      Therefore we are reduced to proving
      \[
        \sum_{\mf{P} \mid \mf{p}\mc{O}}v_{\mf{P}}(\l)f_{\mf{p}}(\mf{P}) = v_{\mf{p}}(\Norm_{L/K}(\l)).
      \]
      We will show this equality via a dimension computation. Recall that \(\mc{O}_{\mf{p}}/\smc{O}_{\mf{p}}\) is a Dedekind extension that admits an integral basis. We will compute the dimension of \(\mc{O}_{\mf{p}}/\l\mc{O}_{\mf{p}}\) as a \(\F_{\mf{p}}\)-vector space in two ways. First, the chinese remainder theorem gives an isomorphism
      \[
        \mc{O}_{\mf{p}}/\l\mc{O}_{\mf{p}} \cong \bigop_{\mf{P} \mid \mf{p}\mc{O}}\mc{O}_{\mf{p}}/\mf{P}_{\mf{p}}^{v_{\mf{P}}(\l)}.
      \]
      By \cref{lem:quotient_by_prime_power_isomorphism}, \(\mc{O}_{\mf{p}}/\mf{P}_{\mf{p}}^{v_{\mf{P}}(\l)}\) is a \(\F_{\mf{p}}\)-vector space of dimension \(v_{\mf{P}}(\l)f_{\mf{p}}(\mf{P})\). Therefore \(\mc{O}_{\mf{p}}/\l\mc{O}_{\mf{p}}\) is an \(\F_{\mf{p}}\)-vector space of dimension \(\sum_{\mf{P} \mid \mf{p}\mc{O}}v_{\mf{P}}(\l)f_{\mf{p}}(\mf{P})\). On the other hand,\fi \todo{xxx}
    \end{proof}
    \subsection*{Number Fields}
  ...
  \fi\iffalse\section{\todo{The Ideal Norm}}
    We can now introduce the ideal norm of \(\mc{O}/\smc{O}\). Suppose \(\mf{F}\) is a fractional ideal of \(\mc{O}\) with prime factorization
    \[
      \mf{F} = \mf{P}_{1}^{e_{1}} \cdots \mf{P}_{r}^{e_{r}}.
    \]
    Let \(\mf{p}_{i}\) be the prime below \(\mf{P}_{i}\). We define the \textbf{norm}\index{norm} \(\mf{N}_{\mc{O}/\smc{O}}(\mf{F})\) of \(\mf{F}\) by
    ...
    \end{proposition}
    \begin{proof}
      Since both \(\mf{N}_{\mc{O}/\smc{O}}\) and \(\Norm_{L/K}\) are multiplicative and \(L\) is the field of fractions of \(\mc{O}\), it is enough to prove the claim when \(\l \in \mc{O}\). In view of \cref{prop:ring_is_intersection_of_all_localizations}, it suffices to show
      \[
        \mf{N}_{\mc{O}/\smc{O}}(\l\mc{O})_{\mf{p}} = \Norm_{L/K}(\l)\smc{O}_{\mf{p}},
      \]
      for all primes \(\mf{p}\) of \(\smc{O}\). Let
      \[
        \l\mc{O} = \mf{P}_{1}^{e_{1}} \cdots \mf{P}_{r}^{e_{r}},
      \]
      be the prime factorization of \(\l\mc{O}\). If \(\mf{P}_{i}\) is not above \(\mf{p}\) then \(\mf{P}_{i}\) contains an element of \(\smc{O}-\mf{p}\) whence the prime below it does as well and so its localization becomes \(\smc{O}_{\mf{p}}\). So on the one hand, we find that
      \[
        \mf{N}_{\mc{O}/\smc{O}}(\l\mc{O})_{\mf{p}} = \mf{p}_{\mf{p}}^{\sum_{\mf{P}_{i} \mid \mf{p}\mc{O}}e_{i}f_{\mf{p}}(\mf{P}_{i})}.
      \]
      On the other hand, \(\smc{O}_{\mf{p}}\) is a discrete valuation ring by \cref{thm:Dedekind_if_and_only_if_all_localizations_are_discrete_valuation_rings} whence
      \[
        \Norm_{L/K}(\l)\smc{O}_{\mf{p}} = \mf{p}_{\mf{p}}^{v_{\mf{p}}(\Norm_{L/K}(\l))}.
      ...
      Therefore we are reduced to proving
      \[
        \sum_{\mf{P}_{i} \mid \mf{p}\mc{O}}e_{i}f_{\mf{p}}(\mf{P}_{i}) = v_{\mf{p}}(\Norm_{L/K}(\l)).
      \]
      \todo{xxx}
    \end{proof}
