\chapter{Arithmetic of Number Fields}
  ...
  \section{Differents and Discriminants}
    ...
    \]
    with \(\k_{j}^{\vee} \in K\). As the dual basis is determined by \(\Trace_{L/K}(\l_{i}\l_{j}^{\vee}) = \d_{i,j}\), it follows that \(\l \in \L^{\vee}\) if and only if \(\k_{j}^{\vee} \in \smc{O}\). Whence \(\l_{1}^{\vee},\ldots,\l_{n}^{\vee}\) is a basis for \(\L^{\vee}\) as a \(\smc{O}\)-submodule of \(L\) and thus is a complete \(\smc{O}\)-lattice of \(L\). As the dual of the dual basis is the original basis, \((\mf{L}^{\vee})^{\vee} = \mf{L}\). The prototypical complete \(\smc{O}\)-lattice of \(L\) is \(\mc{O}\) whence it admits a dual \(\mc{O}^{\vee}\).
    ...
    There is a simple relationship between the duals \(\mf{F}^{\vee}\) and \(\mc{O}^{\vee}\).

    \begin{proposition}\label{prop:dual_lattice_is_fractional}
      Let \(\mc{O}/\smc{O}\) be a Dedekind extension of a degree \(n\) separable extension \(L/K\) and let \(\mf{F}\) be a fractional ideal of \(\mc{O}\). Then
      \[
    ...
    It turns out that it is exceptionally rare for primes of \(\mc{O}\) and \(\smc{O}\) to ramify. We will demonstrate this phenomena by constructs two integral ideals, one of \(\mc{O}\) and the other of \(\smc{O}\), which will encode the ramification of the corresponding primes. These integral ideals are the different and discriminant respectively. We define the \textbf{complement}\index{complement} \(\mf{C}_{\mc{O}/\smc{O}}\) of \(\mc{O}/\smc{O}\) by
    \[
    ...
    \begin{proof}
      ...
      \[
        \mf{d}_{\mc{O}/\smc{O}} = d_{\smc{O}}(\mc{O})\smc{O}.
      \]
      Therefore \(\mf{p}\) divides \(\mf{d}_{\mc{O}/\smc{O}}\) if and only if \(d_{\smc{O}}(\mc{O}) \equiv 0 \tmod{\mf{p}}\). Since \(\conj{\a_{1}},\ldots,\conj{\a_{n}}\) is a basis for \(\mc{O}/\mf{p}\mc{O}\) as an \(\F_{\mf{p}}\)-vector space, it follows that
      \[
