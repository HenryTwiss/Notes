\chapter{Arithmetic of Number Fields}
  ...
  \section{Localization}
    ...
    \]
    In particular, \(\mf{a}D^{-1}\) is an integral ideal of \(\smc{O}D^{-1}\) if \(\mf{a}\) is an integral ideal of \(\smc{O}\). Writing \(\mf{f} = \frac{1}{\d}\mf{a}\) for some nonzero \(\d\) and integral ideal \(\mf{a}\), it follows that \(\mf{f}D^{-1}\) is a fractional ideal of \(\smc{O}D^{-1}\).
    ...
    Let \(v_{\mf{p}}\) denote the valuation of \(\smc{O}_{\mf{p}}\). We call \(v_{\mf{p}}\) the \textbf{valuation}\index{valuation} associated to the prime \(\mf{p}\). These valuations are intimately connected to prime factorizations. Indeed, prime factorization implies that for any \(\k \in K^{\ast}\), we have
    \[
  ...
  \section{Orders}
    ...
    \begin{proof}
      ...
      \[
        \mf{p}_{1} \cdots \mf{p}_{k} \subseteq \mf{a},
      \]
      for some primes \(\mf{p}_{1},\ldots,\mf{p}_{k}\). Since \(\smc{O}\) is one-dimensional, every maximal ideal containing \(\mf{a}\) is a prime and thus one of the \(\mf{p}_{i}\). So one the one hand, \(\mf{a} \subsetneq \mf{p}\) for all but finitely many primes \(\mf{p}\). Whence \(\mf{a}\) contains an element of \(\smc{O}-\mf{p}\) for these primes. This forces \(\mf{a}_{\mf{p}} = \smc{O}_{\mf{p}}\) and so \(\wtilde{\mf{a}}_{\mf{p}} = \smc{O}\). On the other hand, if \(\mf{a} \subseteq \mf{p}\) then the inclusion-preserving bijections in \cref{prop:localization_prime_bijection} show that \(\mf{p}\) is the only prime containing \(\wtilde{\mf{a}}_{\mf{p}}\). As \(\smc{O}\) is one-dimensional, this means that if \(\mf{p}\) and \(\mf{q}\) are distinct primes then
      \[
  ...
  \section{Prime Splitting}
    ...
    \begin{proof}
      ...
      \]
      Prime factorization then forces \(e_{\mf{p}}(\mf{P}_{i}) \le e_{i}\). But then
      \[
