\chapter{Dirichlet Characters and Exponential Sums}
  ...
  \section{Ramanujan and Gauss Sums}
    ...
    For a Dirichlet character \(\chi\) modulo \(m\), we define the \textit{epsilon factor} \(\e_{\chi}\) by
    \[
  ...
  \section{Quadratic Gauss Sums}
    Our primary aim will be to evaluate the epsilon factor of the Gauss sum for quadratic Dirichlet characters defined by Jacobi symbols. To accomplish this we will study and auxiliary exponential sum. For integers \(b\) and \(m\) with \(m\) positive, the \textit{quadratic Gauss sum} \(g(b,m)\) is defined by
    \[
    ...
    For any integer \(m\), we define the \textit{epsilon factor} \(\e_{m}\) by
    \[
