\chapter{Dirichlet Characters and Exponential Sums}
  ...
  \section{Quadratic Gauss Sums}
    ...
    \begin{proof}
      ...
      \]
      Then \(f(x)\) is absolutely integrable and
      \[
        \sum_{n \in \Z}f(x+n),
      \]
      is piecewise continuously differentiable. Therefore Poisson summation applies where \(f(n)\) is understood to be the average of the left-hand and right-hand limits at jump discontinuities. On the one hand, we have
      \[
        \sum_{n \in \Z}f(n) = g(m).
      \]
