\chapter{Dirichlet Characters and Exponential Sums}
  We will make minor use of Poisson summation as developed in \cref{sec:Poisson_summation}. 
  ...
  \section{Quadratic Gauss Sums}
    ...
    \begin{proof}
      ...
      \]
      Then \(f(x)\) is absolutely integrable and piecewise smooth. Then
      \[
        \sum_{n \in \Z}f(x+n),
      \]
      is absolutely convergent and also piecewise smooth by the Weierstrass \(M\)-test. Therefore Poisson summation applies where \(f(n)\) is understood to be the average of the left-hand and right-hand limits at jump discontinuities. On the one hand, this means
      \[
        \sum_{n \in \Z}f(n) = g(m).
      \]
