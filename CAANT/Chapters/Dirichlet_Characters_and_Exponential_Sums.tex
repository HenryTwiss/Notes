\chapter{Dirichlet Characters and Exponential Sums}
  ...
  \[
    \psum_{a \tmod{m}} = \sum_{\substack{a \tmod{m} \\ (a,m) = 1}},
  \]
  ...
  \section{Dirichlet Characters}
    ...
    \[
      \psum_{a \tmod{m}}(\chi\conj{\psi})(a) = \vphi(m)\d_{\chi,\psi} \quad \text{and} \quad \sum_{\chi \tmod{m}}\chi(a\conj{b}) = \vphi(m)\d_{a,b}.
    \]
    In particular,
    \[
      \psum_{a \tmod{m}}\chi(a) = \vphi(m)\d_{\chi,\chi_{m,0}} \quad \text{and} \quad \sum_{\chi \tmod{m}}\chi(a) = \vphi(m)\d_{a,1}.
    \]
    \end{proposition*}
    \begin{proof}
      ...
      \[
        (\chi\conj{\psi})(b)\psum_{a \tmod{m}}(\chi\conj{\psi})(a) = \psum_{a \tmod{m}}(\chi\conj{\psi})(a),
      \]
    ...
    As not every Dirichlet character of a fixed modulus is primitive, it is natural to ask how many primitive Dirichlet characters there are for a given modulus. Let \(N(m)\) be the number of primitive Dirichlet characters modulo \(m\). Then \(N(m)\) is easily determined via the M\"obius inversion formula.
    ...
    \end{proposition}
    \begin{proof}
      To prove the first formula, the right-hand side counts the number of Dirichlet characters modulo \(m\) since every such Dirichlet character is induced from a unique primitive Dirichlet character whose modulus divides \(m\) by \cref{thm:Dirichlet_character_conductor_existance}. The left hand side also counts the number of Dirichlet characters modulo \(m\) as are \(\phi(m)\) many. This proves the first formula. The second follows by the M\"obius inversion formula.
    \end{proof}
  ...
  \section{\todo{Ramanujan and Gauss Sums}}
    For integers \(b\) and \(m\) with \(m\) positive, the \textit{Ramanujan sum} \(r(b,m)\) is defined by
    \[
      r(b,m) = \psum_{a \tmod{m}}e^{\frac{2\pi iab}{m}}.
    \]
    We call \(b\) the \textit{index} and \(m\) the \textit{modulus}. The Ramanujan sum is a finite sum of \(m\)-th roots of unity. When the index is zero the Ramanujan sum has the simple evaluation \(r(0,m) = \vphi(m)\). For a general idex the Ramanujan sums can be computed explicitly by means of the M\"obius function.

    \begin{proposition}\label{prop:Ramanujan_sum_evaluation}
      For any index \(b\) and frequency \(m\), we have
      \[
        r(b,m) = \sum_{d \mid (b,m)}d\mu\left(\frac{m}{d}\right).
      \]
    \end{proposition}
    \begin{proof}
      Every \(a\) modulo \(m\) is of the form \(a = a'd\) for some divisor \(d\) of \(m\) and \(a'\) modulo \(\frac{m}{d}\). So summing \(r(b,d)\) over the divisors \(d\) of \(m\) gives
      \[
        \sum_{d \mid m}r(b,d) = \sum_{a \tmod{m}}e^{\frac{2\pi iab}{m}},
      \]
      If \(m \mid b\) the latter sum is \(m\) and if \(m \nmid b\) the sum is zero because it is the sum of all \(m\)-th roots of unity. Thus
      \[
        \sum_{d \mid m}r(b,d) = \begin{cases} m & \text{if \(m \mid b\)}, \\ 0 & \text{if \(m \nmid b\)}. \end{cases}
      \]
      Now apply the M\"obius inversion formula.
    \end{proof}
    ...
    \]
    Again we call \(b\) the \textit{index}. This generalizes the previous Ramanujan sum as
    \[
      r(b,d) = \tau(b,\chi_{m,0}).
    \]
    In any case, when the index is \(1\) we simply write \(\tau(\chi) = \tau(1,\chi)\) and call \(\tau(\chi)\) the \textit{Gauss sum} associated to \(\chi\). When \(\chi\) is the principal Dirichlet character modulo \(m\), the Dirichlet orthogonality relations imply
    \[
      \tau(\chi) = \vphi(m)\d_{\chi,\chi_{m,0}}.
    \]
    The following proposition develops the basic properties of these Ramanujan sums:
    ...
    In light of \cref{thm:Gauss_sum_modulus} we define the \textit{epsilon factor} \(\e_{\chi}\) for a Dirichlet character \(\chi\) modulo \(m\) by
    \[
