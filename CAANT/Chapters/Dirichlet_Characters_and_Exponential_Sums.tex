\chapter{Dirichlet Characters and Exponential Sums}
  We will make minor use of Poisson summation for functions satisfying the Dirichlet-Jordan test as stated in \cref{sec:Poisson_summation}. 
  ...
  \section{Quadratic Gauss Sums}
    ...
    \begin{proof}
      ...
      \]
      Then \(f(x)\) satisfies the Dirichlet-Jordan test and therefore Poisson summation applies where \(f(n)\) is understood to be the average of the left-hand and right-hand limits at points of discontinuity (see \cref{sec:Poisson_summation}). On the one hand, we have
      \[
        \sum_{n \in \Z}f(n) = g(m).
      \]
