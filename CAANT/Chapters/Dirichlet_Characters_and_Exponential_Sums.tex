\chapter{Dirichlet Characters and Exponential Sums}
  We will make minor use of Poisson summation as developed in \cref{sec:Poisson_summation}. Also, for any complex \(z\) we set
  \[
    e(z) = e^{2\pi iz}.
  \]
  ...
  \section{Ramanujan and Gauss Sums}
    ...
    Gauss sums are a more general type of sum where we have introduced a Dirichlet character. Let \(\chi\) be a Dirichlet character modulo \(m\). For any integer \(b\), the \textbf{Gauss sum}\index{Gauss sum} \(\tau(b,\chi)\) associated to \(\chi\) is given by
    \[
    ...
    \]
    When \(b = 1\) we simply write \(\tau(\chi) = \tau(1,\chi)\). The following proposition develops the basic properties of Gauss sums:
