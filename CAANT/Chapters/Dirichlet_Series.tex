\chapter{Dirichlet Series}
  ...
  \section{Perron Formulas}
    ...
    \begin{proof}
      ...
      \] 
      and we see that \(\d(y) = \frac{1}{2}\). This proves everything when \(y = 1\). Now suppose \(y < 1\) and let \(d > c\). Let \(\eta = \eta_{1}+\cdots+\eta_{4}\) be the rectangular contour in \cref{fig:discontinuous_integral_contour_1} where the horizontal lines are along \(t = \pm T\) and the vertical lines are along \(\s = c\) and \(\s = d\). Consider
      \[
