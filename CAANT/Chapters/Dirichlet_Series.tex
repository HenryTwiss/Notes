\chapter{Dirichlet Series}
  Throughout, \(s = \s+it\) and \(s_{0} = \s_{0}+it_{0}\) will stand for complex variables with \(\s\), \(\s_{0}\), \(t\), and \(t_{0}\) real. We will also use the Mellin transform and the corresponding inversion theorem developed in \cref{sec:The_Mellin_transform}.
