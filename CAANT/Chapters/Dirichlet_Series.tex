\chapter{Dirichlet Series}
  ...
  \section{Convergence Properties}
    ...
    \end{proposition}
    \begin{proof}
      Set \(c(n) = a(n)-b(n)\) so that it suffices to prove \(c(n) = 0\). Take \(\s \ge \s_{a}\) so that \(D(s)\) converges absolutely. Letting \(\s \to \infty\), we have
      \[
        \lim_{\s \to \infty}D(\s) = a(1) \quad \text{and} \quad \lim_{\s \to \infty}D(\s) = b(1),
      \]
      upon interchanging the limit and the sum by absolute convergence. Whence \(c(1) = 0\). Assume by induction that \(c(n) = 0\) for \(n \le N\) and consider the Dirichlet series
      \[
      ...
      \]
      Take \(\s \ge \s_{a}\) so that sum on the right-hand side converges absolutely. Letting \(\s \to \infty\), we have
      \[
        \lim_{\s \to \infty}\left(-\sum_{n > N}c(n)\left(\frac{N}{n}\right)^{\s}\right) = 0,
      \]
      upon interchanging the limit and sum by absolute convergence. Hence \(c(N+1) = 0\) which completes the proof.
    \end{proof}
    ...
    \begin{proof}
      ...
      \]
      In view of the bound \(|A|(X) \ll_{\a} X^{\a}\), take the limit as \(X \to \infty\) to obtain
      \[
  ...
  \section{Perron Formulas}
    ...
    \]
    where the \(\ast\) indicates that the last term is multiplied by \(\frac{1}{2}\) if \(X\) is an integer. This slight modification of \(A(X)\) accounts for the fact that Mellin inversion returns the average of the left-hand and right-hand limits at a jump discontinuity. We would like to relate \(A^{\ast}(X)\) to the inverse Mellin transform of \(D(s)\). We will prove several variants of this basic idea. The first being \textbf{(classical) Perron's formula}\index{(classical) Perron's formula} which is a consequence of Abel summation and Mellin inversion applied to Dirichlet series.
    ...
    Since the integral in truncated Perron's formula is over a finite vertical line, the integral is automatically absolutely convergent. There is also a mor crude variant of truncated Perron's formula that follows as a corollary:
    ...
    \begin{proof}
      ...
      \]
      Also, \cref{prop:Dirichlet_series_polynomial_bound_from_absolute_convergence} guarantees \(a(X) \ll X^{c}\). These estimates and that \(D(s)\) is absolutely convergent at \(s = c\) together imply that error term in truncated Perron's formula is \(O\left(\frac{X^{c}}{T}\right)\). The claim follows.
    \end{proof}

    Having discussed truncated Perron's formula, we turn to the variant where we instead introduce a smoothing function. We call \(\psi(y)\) a \textbf{smooth weight}\index{smooth weight} if it is a positive bump function whose support is bounded away from zero. For any \(X > 0\), we set
    \[
