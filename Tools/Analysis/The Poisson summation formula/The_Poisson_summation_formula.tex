...
\begin{document}
  ...
  \section{The Fourier Transform}
    ...
    \end{proposition}
    \begin{proof}
      Property (i) is immediate from linearity of the integral while (ii) follows by applying the change of variables \(\mathbf{x} \mapsto \mathbf{x}-\boldsymbol{\a}\). Property (iii) is proved by performing the change of variables \(\mathbf{x} \mapsto A^{-1}\mathbf{x}\) to since the Jacobian matrix is \(A\).
    \end{proof}
  ...
  \section{Poisson Summation}
    ...
    \begin{proof}
      ...
      \]
      because the Jacobian matrix is \(P\) whose absolute determinant is \(V_{\L}\). This expression is simply
      \[
