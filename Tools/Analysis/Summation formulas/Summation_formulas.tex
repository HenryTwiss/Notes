...
\begin{document}
  ...
  \section{Summation Formulas}
    ...
    \]
    Note that \(A(X) = 0\) unless \(X \ge 1\). The most well-known summation formula is (classical) \textit{summation by parts}:
    ...
    Replacing the sequences \((a_{n})_{n \ge 1}\) and \((b_{n})_{n \ge 1}\) with \((b_{n})_{n \ge 0}\) and \((A(n-1))_{n \ge 1}\) respectively gives (partial sum) \textit{summation by parts}:
    ...
    Both classical and partial sum summation by parts give ways of rewriting finite sums. If we want to express a finite sum in the form of an integral, we need to do more work. One of the most well-known results of this type is known as \textit{Abel's summation formula}:
