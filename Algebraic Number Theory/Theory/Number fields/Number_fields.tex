...
\begin{document}
  ...
  \maketitle
  \section{Integrality}
    Let \(B/A\) be an extension of rings. We say that \(b \in B\) is \textit{integral} over \(A\) if \(\b\) is the root of a monic polynomial \(f(x) \in A[x]\). In other words, \(\b\) satisfies
    \[
      \b^{n}+\a_{n-1}\b^{n-1}+\cdots+\a_{0} = 0,
    \]
    for some \(n \ge 1\) and \(\a_{i} \in A\) for \(0 \le i \le n-1\). We say that \(B\) is \textit{integral} over \(A\) if every element of \(B\) is integral over \(A\). The following proposition shows that integral elements form a ring:
    ...
    In light of \cref{prop:integrality_is_transitive}, we define the \textit{integral closure} \(\overline{A}\) of \(A\) in \(B\) by
    \[
      \overline{A} = \{\b \in B:\b \text{ is integral over } A\}.
    \]
    Clearly \(A \subseteq \overline{A}\). Moreover, we say that \(A\) is \textit{integrally closed} in \(B\) if \(A = \overline{A}\). As \(A \subseteq \overline{A} \subseteq \overline{\overline{A}}\), \cref{prop:integrality_is_transitive} implies that \(\overline{A}\) is automatically integrally closed in \(B\). Now suppose \(A\) is an integral domain with field of fractions \(K\). Then we call the integral closure \(\overline{A}\) of \(A\) in \(K\) the \textit{normalization} of \(A\) and simply say that \(A\) is \textit{integrally closed} if \(A\) is equal to its normalization. It turns out that every unique factorization domain is integrally closed:
    ...
    We are now ready to introduce number fields. A \textit{number field} \(K\) is a finite extension of \(\Q\). That is, \(K\) is a finite dimensional \(\Q\)-vector space. In particular, \(K/\Q\) is a finite separable extension since \(\Q\) is perfect so that the primitive element theorem applies. Moreover, \(K/\Q\) is Galois if and only if it is normal. We say that the \textit{degree} of \(K\) is \([K:\Q]\) which is simply the degree of \(K\) as a \(\Q\)-vector space. If \(K\) is of degree \(2\), \(3\), etc.\ then we say it is \textit{quadratic}, \textit{cubic}, etc. Any \(\k \in K\) is called an \textit{algebraic number}. We define the \textit{ring of integers} \(\mc{O}_{K}\) of \(K\) to be the integral closure of \(\Z\) in \(K\). In other words,
    \[
      \mc{O}_{K} = \{\k \in K:\k \text{ is integral over } \Z\}.
    \]
    Any \(\a \in \mc{O}_{K}\) is called an \textit{algebraic integer}. Then \(\a\) is an algebraic integer if and only if it is the root of a monic polynomial \(f(x) \in \Z[x]\).
  ...
  \section{Traces and Norms}
    We will now introduce norms and traces of free modules. Let \(B/A\) be an extension of rings such that \(B\) is a free \(A\)-module of rank \(n\). Then the \textit{trace} and \textit{norm} of \(B\) over \(A\), denoted \(\Trace_{B/A}\) and \(\Norm_{B/A}\) respectively, are defined by
    \[
    ...
    \end{equation}
    In the case of a degree \(n\) extension \(L/K\), we call \(\Trace_{L/K}\) and \(\Norm_{L/K}\) the \textit{trace} and \textit{norm} of \(L/K\). Moreover, \(\Norm_{L/K}(\l) = 0\) if and only if \(\l = 0\) because otherwise \(T_{\l}\) has inverse \(T_{\l^{-1}}\) and hence nonzero determinant. Therefore we obtain homomorphisms
    \[
    ...
    Having introduced traces and norms, we discuss discriminants of free modules. Let \(B/A\) be an extension of rings such that \(B\) is a free \(A\)-module of rank \(n\). If \(\b_{1},\ldots,\b_{n}\) is a basis for \(B\), we define its \textit{trace matrix} \(\Trace_{B/A}(\b_{1},\ldots,\b_{n})\) by
    \[
    ...
    \]
    The trace matrix will play a prominent role in the theory. Most importantly is the \textit{discriminant} \(d_{B/A}(\b_{1},\ldots,\b_{n})\) of \(\b_{1},\ldots,\b_{n}\) defined by
    \[
    ...
    \end{equation}
    as claimed. Accordingly, we define the \textit{discriminant} \(d_{A}(B)\) of \(B/A\) to be the coset in \(A/(A^{\ast})^{2}\) represented by any discriminant \(d_{B/A}(\b_{1},\ldots,\b_{n})\). In other words,
    \[
    ...
    In addition to the discriminant \(d_{L/K}(\l_{1},\ldots,\l_{n})\) never vanishing, we can also write it in an alternative form. To do this, we define the \textit{embedding matrix} \(M(\l_{1},\ldots,\l_{n})\) of the basis \(\l_{1},\ldots,\l_{n}\) by
    \[
    ...
    Again suppose \(A\) is an integrally closed integral domain with field of fractions \(K\), \(L/K\) is a degree \(n\) separable extension, and \(B\) is the integral closure of \(A\) in \(L\). We say that \(\b_{1},\ldots,\b_{n}\) is an \textit{integral basis} for \(B/A\) if \(\b_{1},\ldots,\b_{n}\) is such that
    \[
    ...
    It is generally very difficult to write down an integral basis explicitly. However, there is one instance in which this is possible. We say that \(B\) is \textit{monogenic} over \(A\) if \(B = A[\b]\) for some \(\b \in B\). It follows immediately that \(1,\b,\ldots,\b^{n-1}\) is an integral basis for \(B/A\) since \(B\) is of rank \(n\). The discriminant of this basis is then given by \cref{equ:Vandermonde_determinant_for_discriminant}. In addition, the traces \(\Trace_{L/K}\) and \(\Trace_{B/A}\) and norms \(\Norm_{L/K}\) and \(\Norm_{B/A}\) agree:
    ...
    We now turn to the case of a number field \(K\) of degree \(n\). We write \(\Trace_{K} = \Trace_{K/\Q}\) and \(\Norm_{K} = \Norm_{K/\Q}\) and call these the \textit{field trace} and \textit{field norm} of \(K\) respectively. Moreover, for any \(\k \in K\) we call \(\Trace_{K}(\k)\) and \(\Norm_{K}(\k)\) the \textit{trace} and \textit{norm} of \(\k\) respectively. Then from \cref{prop:trace_and_norm_AKBL,prop:unit_if_and_only_if_AKBL} we see that the trace and norm of algebraic integers are themselves integers and \(\k \in \mc{O}_{K}\) is a unit if and only if \(\Norm_{K}(\k) = \pm 1\) (as these are the only units in \(\Z\)). Moreover, since \(\Z\) is a principal ideal domain \cref{thm:integral_basis_AKBL} implies that \(\mc{O}_{K}\) admits an integral basis over \(\Z\) of degree \(n\). That is, \(\mc{O}_{K}\) is a free abelian group of rank \(n\). Accordingly, we say that \(\a_{1},\ldots,\a_{n}\) is an \textit{integral basis} for \(K\) if it is an integral basis of \(\mc{O}_{K}\) over \(\Z\). Accordingly, we define the \textit{discriminant} \(\D_{K}\) of \(K\) to be the discriminant of \(\mc{O}/\smc{O}\). As \((\Z^{\ast})^{2} = \{1\}\), \(\D_{K}\) is a well-defined integer and satisfies
    \[
  ...
  \section{Dedekind Domains}
    ...
    \]
    with \(e_{i} \in \Z-\{0\}\) for all \(i\), called the \textit{prime factorization} of \(\mf{f}\) with \textit{prime factors} \(\mf{p}_{1},\ldots,\mf{p}_{r}\). In particular, the prime factorization of an integral ideal \(\mf{a}\) is of the form
    \[
      \mf{a} = \mf{p}_{1}^{e_{1}} \cdots \mf{p}_{r}^{e_{r}},
    \]
    with \(e_{i} \ge 1\) for all \(i\). Accordingly, for any two integral ideal \(\mf{a}\) and \(\mf{b}\) we say that \(\mf{a}\) \textit{divides} \(\mf{b}\) and write \(\mf{a} \mid \mf{b}\) if \(\mf{b} \subseteq \mf{a}\). Sometimes this condition is expressed as \textit{to contain is to divide} or \textit{to divide is to contain}. By the prime factorization of fractional ideals, this is equivalent to the fact that every prime power factor of \(\mf{a}\) appears in the prime factorization of \(\mf{b}\). We also say that \(\mf{a}\) \textit{exactly divides} \(\mf{b}\) and write \(\mf{a} \mid\mid \mf{b}\) if \(\mf{a}\) divides \(\mf{b}\) but no power of \(\mf{a}\) divides \(\mf{b}\). This is equivalent to the fact that every prime power factor of \(\mf{a}\) appears in the prime factorization of \(\mf{b}\) but not for any power of \(\mf{a}\). In the case of a prime \(\mf{p}\) and an integral ideal \(\mf{a}\), \(\mf{p} \mid \mf{a}\) if and only if \(\mf{p}\) is a prime factor of \(\mf{a}\) and \(\mf{p}^{e} \mid\mid \mf{a}\), for some \(e \ge 1\), if and only if \(\mf{p}^{e}\) is exactly the power of \(\mf{p}\) appearing in the prime factorization of \(\mf{a}\). Moreover, if \(\mf{a} \mid \mf{p}\) then \(\mf{a} = \mf{p}\). The \textit{greatest common divisor} \((\mf{a},\mf{b})\) of \(\mf{a}\) and \(\mf{b}\) is defined to be the integral ideal that all other common integral ideal divisors divide. Since to divide is to contain, \((\mf{a},\mf{b})\) is the smallest ideal that contains both \(\mf{a}\) and \(\mf{b}\). This is \(\mf{a}+\mf{b}\) and so \((\mf{a},\mf{b}) = \mf{a}+\mf{b}\). The \textit{least common multiple} \([\mf{a},\mf{b}]\) of \(\mf{a}\) and \(\mf{b}\) is defined to be the integral ideal that divides all other common multiples. Since to divide is to contain, \([\mf{a},\mf{b}]\) is the largest integral ideal that is contained in both \(\mf{a}\) and \(\mf{b}\). This is \(\mf{a} \cap \mf{b}\) and so \([\mf{a},\mf{b}] = \mf{a} \cap \mf{b}\). Moreover, we say that \(\mf{a}\) and \(\mf{b}\) are \textit{relatively prime} if \((\mf{a},\mf{b}) = \smc{O}\). In other words, we have
    \[
      \mf{a}+\mf{b} = \smc{O}.
    \]
    ...
    With the prime factorization in hand, we will discuss the group structure of the fractional ideals of \(\smc{O}\). Let \(I_{\smc{O}}\) denote the set of fractional ideals of \(\smc{O}\). We call \(I_{\smc{O}}\) the \textit{ideal group} of \(\smc{O}\). The following theorem shows that \(I_{\smc{O}}\) is indeed a group:
    ...
    \cref{prop:fractional_ideal_generated_by_two_elements} shows that while a Dedekind domain \(\smc{O}\) may not be a principal ideal domain, it is not far off from one since every integral ideal needs at most two generators. We can give a more refined interpretation of this using the ideal group \(I_{\smc{O}}\). Let \(P_{\smc{O}}\) denote the subgroup of principal fractional ideals of \(I_{\smc{O}}\). Since \(I_{\smc{O}}\) is abelian by \cref{thm:ideal_group_is_a_group}, \(P_{\smc{O}}\) is normal. The \textit{ideal class group} \(\Cl(\smc{O})\) of \(\smc{O}\) is defined to be the quotient group
    \[
      \Cl(\smc{O}) = I_{\smc{O}}/P_{\smc{O}},
    \]
    We call an element of \(\Cl(\smc{O})\) an \textit{ideal class} of \(\smc{O}\). As every fractional ideal \(\mf{f}\) can be expressed as \(\mf{f} = \frac{1}{\d}\mf{a}\) for some nonzero \(\d \in \smc{O}\) and integral ideal \(\mf{a}\), we have \(\d\mf{f} = \mf{a}\) and hence every ideal class can be represented by an integral ideal \(\mf{a}\). The \textit{class number} \(h_{\smc{O}}\) of \(\smc{O}\) is defined by
    \[
      h_{\smc{O}} = |\Cl(\smc{O})|.
    \]
    ...
    The \textit{unit group} of \(\smc{O}\) is defined to be \(\smc{O}^{\ast}\). That is, the unit group is the group of units in \(\smc{O}\). The ideal class group and unit group of \(\smc{O}\) are related via the following exact sequence:
    ...
    In view of the fact that \(\mc{O}_{K}\) is a Dedekind domain by \cref{thm:ring_of_integers_is_Dedekind}, we simplify some terminology. An \textit{integral ideal} of \(K\) is simply an integral ideal of \(\mc{O}_{K}\), a \textit{prime} of \(K\) is a prime of \(\mc{O}_{K}\), and a \textit{fractional ideal} of \(K\) is a fractional ideal of \(\mc{O}_{K}\). The \textit{ideal group} \(I_{K}\) of \(K\) is the ideal group of \(\mc{O}_{K}\), we write \(P_{K}\) for the subgroup of principal fractional ideals of \(K\), and the \textit{ideal class group} \(\Cl(K)\) of \(K\) is the ideal class group of \(\mc{O}_{K}\). In particular,
    \[
      \Cl(K) = I_{K}/P_{K}.
    \]
    The \textit{class number} \(h_{K}\) of \(K\) is the class number of \(\mc{O}_{K}\) and so
    \[
      h_{K} = |\Cl(K)|.
    \]
    The \textit{unit group} of \(K\) is the unit group of \(\mc{O}_{K}\) and we call any element of \(\mc{O}_{K}^{\ast}\) a \textit{unit} of \(K\) (with the understanding that every element of \(K\) is invertible in \(K\)). It follows from \cref{thm:ring_of_integers_is_Dedekind,thm:unique_product_prime_ideals} that integral ideals \(\mf{a}\) of \(K\) admit prime factorizations. One of our core investigations will be to understand how the principal integral ideal \(p\mc{O}_{K}\) factors into a product of primes of \(K\) for any prime \(p\). Moreover, we will be able to leverage geometric tools to show that the class number \(h_{K}\) is finite and completely describe the unit group \(\mc{O}_{K}^{\ast}\).
  \section{Localization}
    Let \(\smc{O}\) be a noetherian integral domain with field of fractions \(K\). If \(D \subseteq \smc{O}-\{0\}\) is a multiplicative subset (recall that necessarily \(1 \in D\)) then the \textit{localization} \(\smc{O}D^{-1}\) of \(\smc{O}\) at \(D\) is defined by
    \[
    ...
    \]
    Essentially, localizing at a prime \(\mf{p}\) removes all of the algebraic information about \(\smc{O}\) that has nothing to do with \(\mf{p}\). If \(\mf{f}\) is a fractional ideal of \(\smc{O}\) then the \textit{localization} \(\mf{f}_{\mf{p}}\) of \(\mf{f}\) at \(\mf{p}\) is defined to be
    \[
    ...
    \]
    and the \(\smc{O}-\mf{p}\) are. We define the \textit{localization} \(\smc{O}(X)\) of \(\smc{O}\) at \(X\) by
    \[
    ...
    \]
    If \(\mf{f}\) is a fractional ideal of \(\smc{O}\) then the \textit{localization} \(\mf{f}(X)\) of \(\mf{f}\) at \(X\) is defined to be
    \[
    ...
    A noetherian integral domain is said to be \textit{local} if it has a unique maximal integral ideal. As we might expect, localizing at a prime will produce a local noetherian integral domain and hence justifies the term localization. We claim that \(\smc{O}_{\mf{p}}\) is a local noetherian integral domain with maximal integral ideal \(\mf{p}_{\mf{p}}\). Indeed, \(\smc{O}_{\mf{p}}\) is already a noetherian integral domain and it is clear that \(\mf{p}_{\mf{p}}\) is an ideal of \(\smc{O}_{\mf{p}}\). By \cref{prop:localization_prime_bijection}, the map
    \[
      \mf{q} \to \mf{q}_{\mf{p}},
    \]
    ...
    \begin{remark}
      The valuation \(v_{\mf{p}}\) associated to the prime \(\mf{p}\) of \(\smc{O}\) is sometimes called an \textit{exponential valuation}.
    \end{remark}
    ...
    We now turn to the setting of a number field \(K\). We let \(S\) denote a finite set of primes of \(\mc{O}_{K}\) and let \(X\) denote the set of all primes that do not belong to \(S\). The \textit{ring of \(S\)-integers} \(\mc{O}_{K}^{S}\) of \(K\) is defined by
    \[
      \mc{O}_{K}^{S} = \mc{O}_{K}(X).
    \]
    We call any \(\a \in \mc{O}_{K}^{S}\) an \textit{algebraic \(S\)-integer}. The \textit{\(S\)-class group} \(\Cl^{S}(K)\) of \(K\) is the ideal class group of \(\mc{O}_{K}^{S}\). The \textit{\(S\)-class number} \(h_{K}^{S}\) of \(K\) is the class number of \(\Cl^{S}(K)\). The \textit{\(S\)-unit group} of \(K\) is the unit group \((\mc{O}_{K}^{S})^{\ast}\) of \(\mc{O}_{K}^{S}\) and we call any element of \((\mc{O}_{K}^{S})^{\ast}\) an \textit{\(S\)-unit} of \(K\).
  \section{Dedekind Extensions}
    ...
    We now describe the relationship between inertia degrees and ramification indices which is known as the \textit{fundamental equality}:
    ...
    We also classify primes according to extremal cases of the fundamental equality. We say \(\mf{p}\) is \textit{inert} in \(\mc{O}/\smc{O}\) if \(r = 1\) so that there is a single prime \(\mf{P}\) above \(\mf{p}\) so that \(e_{\mf{p}}(\mf{P}) = 1\) and \(f_{\mf{p}}(\mf{P}) = n\) by the fundamental equality. Then
    \[
      \mf{p}\mc{O} = \mf{P},
    \]
    which means \(\mf{p}\mc{O}\) is prime. We say \(\mf{p}\) is \textit{totally split} in \(\mc{O}/\smc{O}\) if \(r = n\) so that there are primes \(\mf{P}_{i}\) above \(\mf{p}\) with \(e_{\mf{p}}(\mf{P}_{i}) = f_{\mf{p}}(\mf{P}_{i}) = 1\) for \(1 \le i \le n\) by the fundamental equality. Hence
    \[
      \mf{p}\mc{O} = \mf{P}_{1} \cdots \mf{P}_{n}.
    \]
    In terms of the inertia degrees, being inert or totally split in \(\mc{O}/\smc{O}\) are antithetical properties. In particular, the smaller the inertia degrees are the greater the tendency for \(\mf{p}\mc{O}\) to factor into distinct primes. Now let us introduce ramification. If \(\mf{P}\) is a prime of \(\mc{O}\) above \(\mf{p}\), we say that \(\mf{P}\) is \textit{unramified} in \(\mc{O}/\smc{O}\) if \(e_{\mf{p}}(\mf{P}) = 1\) and the residue class extension \(\F_{\mf{P}}/\F_{\mf{p}}\) is separable. Otherwise, we say \(\mf{P}\) is \textit{ramified} in \(\mc{O}/\smc{O}\). Moreover, we say \(\mf{P}\) is \textit{totally ramified} in \(\mc{O}/\smc{O}\) if in addition to being ramified we have \(f_{\mf{p}}(\mf{P}) = 1\). Similarly, we say that a prime \(\mf{p}\) of \(\smc{O}\) is \textit{unramified} in \(\mc{O}/\smc{O}\) if every prime \(\mf{P}\) above it is unramified and is \textit{ramified} in \(\mc{O}/\smc{O}\) otherwise. The Dedekind extension \(\mc{O}/\smc{O}\) itself is said to be \textit{unramified} if every prime of \(\smc{O}\) is unramified in \(\mc{O}/\smc{O}\) and is said to be \textit{ramified} otherwise.
  ...
  \section{Galois Extensions}
    ...
    \]
    Accordingly, we say that \(\s(\mf{P})\) is \textit{conjugate} to \(\mf{P}\). It turns out that the Galois group acts transitively on the set of primes above a given prime:
    ...
    Another way to phrase \cref{prop:Galois_action_on_primes_is_transitive} is that the primes above \(\mf{p}\) are all conjugate to each other and their inertia degrees and ramification indices are all equal. We call the common inertia degree \(f\) the \textit{inertia degree} of \(\mf{p}\) and the common ramification index \(e\) the \textit{ramification index} of \(\mf{p}\). If there are \(r\) primes above \(\mf{p}\), the fundamental equality takes the particularly simple form
    \[
      n = ref.
    \]
    Continue to let \(\mf{P}\) be a prime above \(\mf{p}\). We define the \textit{decomposition group} \(D_{\mf{p}}(\mf{P})\) of \(\mf{P}\) by
    \[
      D_{\mf{p}}(\mf{P}) = \{\tau \in \Gal(L/K):\tau(\mf{P}) = \mf{P}\}.
    \]
    Equivalently, \(D_{\mf{p}}(\mf{P})\) is the stabilizer subgroup of \(\mf{P}\) in \(\Gal(L/K)\). The associated \textit{decomposition field} \(L^{D_{\mf{p}}(\mf{P})}\) of \(\mf{P}\) is defined to be
    \[
    ...
    Accordingly, we define the \textit{inertia group} \(I_{\mf{p}}(\mf{P})\) of \(\mf{P}\) by
    \[
      I_{\mf{p}}(\mf{P}) = \ker{T_{\mf{p}}^{\mf{P}}}.
    \]
    Then \(I_{\mf{p}}(\mf{P})\) is a normal subgroup of \(D_{\mf{p}}(\mf{P})\). The associated \textit{inertia field} \(L^{I_{\mf{p}}(\mf{P})}\) of \(\mf{P}\) is defined to be
    \[
  ...
  \section{The Different and Discriminant}
    ...
    \[
      \Trace_{L/K}:L \x L \to K \qquad (\l,\eta) \mapsto \Trace_{L/K}(\l\eta).
    \]
    We call this bilinear form the \textit{trace form} of \(L/K\). The trace form makes \(L\) into a nondegenerate inner product space over \(K\) and so every basis \(\l_{1},\ldots,\l_{n}\) admits a dual basis \(\l_{1}^{\vee},\ldots,\l_{n}^{\vee}\) with respect to \(\Trace_{L/K}\) defined by
    \[
      \Trace_{L/K}(\l_{i}\l_{j}^{\vee}) = \d_{i,j},
    \]
    for \(1 \le i,j \le n\). The trace form will allow us to introduce duals. Indeed, if \(\mf{F}\) is a fractional ideal \(\mc{O}\), the \textit{dual} \(\mf{F}^{\vee}\) of \(\mf{F}\) is defined by
    \[
      \mf{F}^{\vee} = \{\l \in L:\Trace_{L/K}(\l\mf{F}) \subseteq \smc{O}\}.
    \]
    We say that \(\mf{F}\) is \textit{self-dual} if \(\mf{F}^{\vee} = \mf{F}\). The following proposition shows that the dual \(\mf{F}^{\vee}\) is indeed a fractional ideal:
    ...
    We will now introduce the different and the discriminant. We define the \textit{complement} \(\mf{C}_{\mc{O}/\smc{O}}\) of \(\mc{O}/\smc{O}\) by
    \[
      \mf{C}_{\mc{O}/\smc{O}} = \mc{O}^{\vee}.  
    \]
    This is a fractional ideal by \cref{prop:dual_lattice_is_fractional}. The \textit{different} \(\mf{D}_{\mc{O}/\smc{O}}\) of \(\mc{O}/\smc{O}\) is defined to be the inverse of the complement \(\mf{C}_{\mc{O}/\smc{O}}\):
    \[
    ...
    Now consider the case of a number field \(K\) of degree \(n\). The \textit{complement} \(\mf{C}_{K}\) of \(K\) is the complement of \(\mc{O}_{K}/\Z\), the \textit{different} \(\mf{D}_{K}\) of \(K\) is the different of \(\mc{O}_{K}/\Z\), and the \textit{discriminant} \(\mf{d}_{K}\) of \(K\) is the discriminant of \(\mc{O}_{K}/\Z\). Since \(\Z\) is a principal ideal domain, the \(\mf{d}_{K}\) is related to the discriminant \(\D_{K}\) by
    \[
  ...
  \section{The Ideal Norm}
    Let \(\mc{O}/\smc{O}\) be a Dedekind extension of a degree \(n\) extension \(L/K\). If \(\mf{P}\) a prime of \(L\) is above the prime \(\mf{p}\) of \(K\), we define the \textit{norm} \(\Norm_{\mc{O}/\smc{O}}(\mf{P})\) of \(\mf{P}\) by
    \[
    ...
    \]
    called the \textit{ideal norm} of \(\mc{O}/\smc{O}\). It follows from the fundamental equality and multiplicatively of the ideal norm that
    \[
    ...
    \]
    called the \textit{norm} of \(K\). For an integral ideal \(\mf{a}\), there is a simple relationship between the norm \(\mf{a}\) and the quotient \(\mc{O}_{K}/\mf{a}\):
  ...
  \section{Minkowski Space}
    ...
    \[
      K_{\C} = \C^{n},
    \]
    we see that \(K_{\C}\) is a \(\C\)-algebra and also a complex Hilbert space with respect to \(\<\cdot,\cdot\>_{K_{\C}}\) which we take to be the standard complex inner product. We denote the associated Lebesgue measure by \(d\l_{K_{\C}}\). We define the \textit{canonical embedding} \(j\) of \(K\) to be the \(\Q\)-embedding
    \[
    ...
    \]
    where the last equality in each chain follow by \cref{prop:formulas_for_trace_and_norm}. We now define the \textit{Minkowski space} \(K_{\R}\) of \(K\) by
    \[
    ...
    \]
    so that \(\<\mathbf{z},\mathbf{w}\> \in \R\) is real. Accordingly, we call \(\<\cdot,\cdot\>_{K_{\R}}\) the \textit{Minkowski inner product}. We denote the restriction of the Lebesgue measure \(d\l_{\C}\) to \(K_{\R}\) by \(d\l_{K_{\R}}\) which is also the Lebesgue measure associated to \(\<\cdot,\cdot\>_{K_{\R}}\). We call \(d\l_{K_{\R}}\) the \textit{Minkowski measure}. Lastly, we denote the restrictions of \(\Trace_{K_{\C}}\) and \(\Norm_{K_{\C}}\) to \(K_{\R}\) by \(\Trace_{K_{\R}}\) and \(\Norm_{K_{\R}}\) respectively and call these maps the \textit{Minkowski trace} and \textit{Minkowski norm} respectively. We also have homomorphisms
    \[
    ...
    Define the \textit{Minkowski embedding} \(\s_{K}\) of \(K\) by
    \[
  ...
  \section{Dirichlet's Unit Theorem}
    Let \(K\) be a number field of signature \((r_{1},r_{2})\). We define the \textit{rank} \(r_{K}\) of \(K\) to be
    \[
      r_{K} = r_{1}+r_{2}-1.
    \]
    ...
    We define the \textit{logarithmic embedding} \(\log_{K}\) of \(K\) by
    \[
    ...
    \]
    called the \textit{norm-one hypersurface} and \textit{trace-zero hyperplane} of \(\R^{r_{K}+1}\) respectively. Note that \(H\) is an \(r_{K}\)-dimensional subspace of \(\R^{r_{K}+1}\). We will also make use of the subset
    \[
    ...
    By \cref{thm:unit_group_complete_lattice}, there exist elements \(\e_{1},\ldots,\e_{r_{K}}\) of \(\mc{O}_{K}^{\ast}\) such that \(\l(\e_{1}),\ldots,\l(\e_{r_{K}})\) is a basis for the unit lattice \(\L\) by taking the preimage under \(j\) of any basis for \(\L\). We say that \(\e_{1},\ldots,\e_{r_{K}}\) are a \textit{system of fundamental units} of \(K\) and we call any such element a \textit{fundamental unit} for \(K\). The structure theorem for \(\mc{O}_{K}^{\ast}\) known as \textit{Dirichlet's unit theorem} says that \(\mc{O}_{K}^{\ast}\) is a product of a root of unity in \(K\) and powers of fundamental units:
